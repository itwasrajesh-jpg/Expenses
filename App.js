...
function BubbleField({ cats, onSelect, selected, T, animLevel, fieldH, bSize, collapsed, onCollapseChange }) {
  ...
  useEffect(() => {
    if (animLevel === 'none') {
      if (collapsed) {
        bs.forEach(b => { b.x = CX; b.y = CY; b.sc = 0.10; b.op = 0.38; });
      } else {
    ...
    if (collapsed) {
      // Already collapsed — snap to center silently
      bs.forEach(b => { b.x = CX; b.y = CY; b.tx = CX; b.ty = CY; b.sc = 0.10; b.tsc = 0.10; b.op = 0.38; b.top = 0.38; });
      return;
    }
              ...
              <>
                ...
                <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
                  {catEx.length === 0
                    ...
                    : catEx.slice(0, 15).map((e, idx) => <Row key={e.id} expense={e} onPress={(exp) => { close(); setTimeout(() => { setSelBbl(-1); setSheetCat(null); onRow(exp); }, 460); }} T={T} delay={idx * 50} animLevel={animLevel} />)
                  }
                  <View style={{ height: 24 }} />
                </ScrollView>
              </>
...
function PieChart({ data, total, size, T, onSelect, selected }) {
  ...
  // Keep a map of anims by category name so they survive month switches
  ...
  useEffect(() => {
    ...
    const anims = flyAnims.map((a, i) =>
      Animated.spring(a, {
        toValue: 1, tension: 55, friction: 10,
        delay: i * 65, useNativeDriver: true,
      })
    );
  ...
  // Build donut segments as clipped wedges using border-radius trick
  // Each segment is a square View with two rounded corners removed to form a wedge
  // We layer them with overflow:hidden on a circle container
  const segments = data.map((c, i) => {
    ...
    const sa = cumAngle;
    cumAngle += sweep;
    return { ...c, frac, pct: Math.round(frac*100), sweep, mid, sa, ea: cumAngle };
  });

  // Draw using a canvas approach: each segment is a colored donut slice
  // built from positioned absolute Views using the "half circle" trick
  const THICK = R - innerR - 3;
  const CR = innerR + THICK / 2; // center radius of stroke
  ...
  return (
    <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
      {segments.map((seg, i) => {
        ...
        return (
          <TouchableOpacity key={seg.name} onPress={() => onSelect?.(i)} activeOpacity={0.8}>
          <Animated.View style={{
            position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
            transform: [
              { translateX: flyAnims[i].interpolate({ inputRange: [0,1], outputRange: [Math.cos(seg.mid)*240, 0] }) },
              { translateY: flyAnims[i].interpolate({ inputRange: [0,1], outputRange: [Math.sin(seg.mid)*240, 0] }) },
              { scale:      flyAnims[i].interpolate({ inputRange: [0, 0.7, 1], outputRange: selected===i?[0.15,1.14,1.1]:[0.15, 1.06, 1] }) },
            ],
            opacity: flyAnims[i].interpolate({ inputRange: [0, 0.25, 1], outputRange: [0, selected===-1||selected===i?1:0.45, selected===-1||selected===i?1:0.45] }),
          }}>
            {Array.from({ length: slices }, (_, s) => {
              const angle = seg.sa + s * sliceA + sliceA / 2;
              const x = R + Math.cos(angle) * CR - THICK / 2;
              const y = R + Math.sin(angle) * CR - THICK / 2;
              return (
                <View key={s} style={{
                  position: 'absolute',
                  width: THICK + 1,
      ...
      <View style={{
        ...
        <Text style={{ fontSize: selected>=0 && segments[selected] ? 18 : 18 }}>
          {selected>=0 && segments[selected] ? segments[selected].icon : '💸'}
        </Text>
        <Text style={{ color: selected>=0 && segments[selected] ? segments[selected].color : (T?.accent ?? '#B985FA'), fontSize: 9, fontWeight: '700', marginTop: 2 }}>
...
function DatePickerModal({ currentDate, onSelect, onClose, T }) {
  ...
  return (
    <Animated.View style={{ position: 'absolute', inset: 0, zIndex: 200, opacity: fadeAnim }}>
      ...
      <Animated.View style={{ backgroundColor: T.surface, borderRadius: 24, margin: 16, padding: 20, transform: [{ translateY: slideAnim }] }}>
        ...
        {/* Highlight bar */}
        <View style={{ position: 'absolute', top: 108, left: 20, right: 20, height: 44, backgroundColor: T.accent+'18', borderRadius: 12, borderWidth: 1, borderColor: T.accent+'33', pointerEvents: 'none' }} />
