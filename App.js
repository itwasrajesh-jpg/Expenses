...
function BubbleField({ cats, onSelect, selected, T, animLevel, fieldH, bSize, collapsed, onCollapseChange }) {
  ...
  useEffect(() => {
    ...
    withDist.forEach(({ i }, rank) => {
      setTimeout(() => { bs[i].tx = HOME[i].x; bs[i].ty = HOME[i].y; bs[i].tsc = 1; bs[i].top = 1; }, rank * 36 + 200);
    });
  }, []);
...
// ─── HOME SCREEN ─────────────────────────────────────────────────────────────
function HomeScreen({ state, dispatch, onRow, onQuickAdd, T, animLevel }) {
    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10 }}>
          ...
          <TouchableOpacity onPress={() => { setCollapsed(!collapsed); setSelBbl(-1); }}
              ...
              <>
                ...
                <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
                  {catEx.length === 0
                    ...
                    : catEx.slice(0, 15).map((e, idx) => <Row key={e.id} expense={e} onPress={() => {}} T={T} delay={idx * 50} animLevel={animLevel} />)
                  }
                  <View style={{ height: 24 }} />
                </ScrollView>
              </>
...
export default function App() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      <StatusBar barStyle="light-content" backgroundColor={T.bg} />
      {tab === 'home'     && <HomeScreen     state={state} dispatch={dispatch} onRow={handleRow} onQuickAdd={() => { setFormEx(null); setShowForm(true); }} T={T} animLevel={aL} />}
