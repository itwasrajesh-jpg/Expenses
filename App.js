...
function FeatEditModal({ entry, fid, dispatch, onClose, T, originY }) {
  ...
  return (
    <Modal transparent visible animationType="none" onRequestClose={() => close()}>
      {/* Backdrop — sibling to card, not parent */}
      ...
      {/* Card — centred, intercepts its own touches */}
          ...
          <View style={{ width:'100%', maxWidth:360, borderRadius:24, padding:22,
            ...
            <View style={{ position:'relative' }}>
              ...
              <TouchableOpacity onPress={() => close()} hitSlop={{ top:12, right:12, bottom:12, left:12 }}
                style={{ position:'absolute', top:-4, right:-4, width:36, height:36, borderRadius:18,
...
function StatsScreen({ expenses, featEntries, bills, dispatch, T, animLevel, selCat, setSelCat }) {
  ...
  const buildData = () => {
    ...
    const entries = (featEntries[selCat] || []).filter(e => rangeStart && rangeEnd
      ? e.date >= rangeStart && e.date <= rangeEnd
      : monthOf(e.date) === selM);
    const getAmt = e => isTargetCat ? (e.targetAmount || e.amount || 0) : (e.amount || 0);
      ...
      <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.section, { backgroundColor:T.surface }]}>
          ...
          <Text style={{ color:T.sub, fontSize:11, marginBottom:14 }}>
            {(selCat==='goals'||selCat==='emi') ? ((featEntries[selCat]||[]).length + ' goals') : catD.length + (selCat==='expenses'?' categories':' types')}
          </Text>
          ...
          {/* Goals / EMI — target cards below breakdown */}
          {(selCat==='goals'||selCat==='emi') && (
            <View style={{ marginTop: 8 }}>
              <Text style={[styles.secTitle, { color:T.text, marginBottom:10 }]}>
