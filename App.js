...
function SavedInsightsView({ savedInsights, dispatch, T, animLevel }) {
  ...
  // Layout positions — safe margins, no overlaps at any screen size
  // x: 0.15–0.85 (leaves 15% margin each side for label overflow)
  // y: 0.1–0.85 (leaves room for label below)
  // Minimum gap between any two stars: ~28% of field
  const LAYOUTS = {
    1:[[0.5,0.42]],
    2:[[0.28,0.35],[0.72,0.55]],
    3:[[0.5,0.18],[0.22,0.62],[0.78,0.62]],
    4:[[0.28,0.22],[0.72,0.22],[0.22,0.68],[0.78,0.68]],
    5:[[0.5,0.18],[0.18,0.48],[0.82,0.48],[0.32,0.78],[0.68,0.78]],
    6:[[0.5,0.15],[0.18,0.42],[0.82,0.42],[0.28,0.72],[0.72,0.72],[0.5,0.55]],
    7:[[0.5,0.12],[0.18,0.38],[0.82,0.38],[0.22,0.65],[0.78,0.65],[0.38,0.85],[0.62,0.85]],
    8:[[0.28,0.12],[0.72,0.12],[0.18,0.42],[0.82,0.42],[0.5,0.3],[0.28,0.7],[0.72,0.7],[0.5,0.85]],
  };
  ...
  const ly = LAYOUTS[Math.min(curItems.length,8)] || LAYOUTS[8];
