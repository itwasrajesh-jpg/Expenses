...
function PieExpanded({ data, total, T, onClose }) {
  ...
  const R = SIZE / 2, innerR = R * 0.38, THICK = R - innerR - 4, CR = innerR + THICK / 2;
  ...
  useEffect(() => {
    flyAnims.forEach(a => a.setValue(0));
    Animated.parallel([
      Animated.timing(bgAnim,    { toValue: 1, duration: 350, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
  ...
  const close = () => {
    Animated.stagger(35, [...flyAnims].reverse().map(a =>
      Animated.timing(a, { toValue: 0, duration: 250, easing: Easing.in(Easing.cubic), useNativeDriver: true })
    )).start();
    Animated.parallel([
      Animated.timing(bgAnim,    { toValue: 0, duration: 350, delay: 150, easing: Easing.in(Easing.cubic), useNativeDriver: true }),
      Animated.spring(scaleAnim, { toValue: 0.3, tension: 80, friction: 12, useNativeDriver: true }),
      Animated.timing(opAnim,    { toValue: 0, duration: 280, delay: 150, useNativeDriver: true }),
    ]).start(onClose);
  };
  ...
  return (
    <Modal transparent visible animationType="none">
      ...
      <Animated.View {...pan.panHandlers}
        style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', transform: [{ scale: scaleAnim }, { translateY: panY }], opacity: opAnim }}>
        <View style={{ width: SIZE, height: SIZE }}>
          {segments.map((seg, i) => {
            const slices = Math.max(Math.ceil((seg.sweep * 180 / Math.PI) / 3), 1);
            const sliceA = seg.sweep / slices;
            const isSel = selIdx === i, isDim = selIdx !== -1 && !isSel;
            const ex = flyAnims[i].interpolate({ inputRange: [0, 1], outputRange: [0, Math.cos(seg.mid) * SIZE * 0.07] });
            const ey = flyAnims[i].interpolate({ inputRange: [0, 1], outputRange: [0, Math.sin(seg.mid) * SIZE * 0.07] });
            const sc = flyAnims[i].interpolate({ inputRange: [0, 0.6, 1], outputRange: [0.05, 1.08 + (isSel ? 0.04 : 0), 1 + (isSel ? 0.04 : 0)] });
            const op = flyAnims[i].interpolate({ inputRange: [0, 0.3, 1], outputRange: [0, 1, isDim ? 0.3 : 1] });
            return (
              <TouchableOpacity key={seg.name} onPress={() => setSelIdx(selIdx === i ? -1 : i)} activeOpacity={1}
                style={{ position: 'absolute', width: SIZE, height: SIZE }}>
                <Animated.View style={{ position: 'absolute', width: SIZE, height: SIZE, transform: [{ translateX: ex }, { translateY: ey }, { scale: sc }], opacity: op }}>
                  {Array.from({ length: slices }, (_, s) => {
                    ...
                    const x = R + Math.cos(angle) * CR - (THICK + 1) / 2;
                    const y = R + Math.sin(angle) * CR - (THICK + 1) / 2;
                    return <View key={s} style={{ position: 'absolute', width: THICK + 1, height: THICK + 1, borderRadius: (THICK + 1) / 2, backgroundColor: seg.color, left: x, top: y, shadowColor: seg.color, shadowOpacity: 0.55, shadowRadius: 6, elevation: 4 }} />;
                  })}
                </Animated.View>
              </TouchableOpacity>
            );
          ...
          {/* Label pills */}
          {segments.map((seg, i) => {
            const lr = R * 1.28, lx = R + Math.cos(seg.mid) * lr, ly = R + Math.sin(seg.mid) * lr;
            const isDim = selIdx !== -1 && selIdx !== i;
            return (
              <Animated.View key={seg.name + 'l'} pointerEvents="none"
                style={{ position: 'absolute', left: lx - 28, top: ly - 12,
                  opacity: flyAnims[i].interpolate({ inputRange: [0, 0.75, 1], outputRange: [0, 0, isDim ? 0.25 : 1] }),
                  transform: [{ scale: flyAnims[i].interpolate({ inputRange: [0, 0.8, 1], outputRange: [0, 0, 1] }) }] }}>
                <View style={{ backgroundColor: 'rgba(25,22,35,0.92)', borderRadius: 99, paddingHorizontal: 6, paddingVertical: 3, borderWidth: 1, borderColor: seg.color + '55', alignItems: 'center' }}>
          ...
          {/* Center hole */}
          <View pointerEvents="none" style={{ position: 'absolute', left: R - innerR, top: R - innerR, width: innerR * 2, height: innerR * 2, borderRadius: innerR, backgroundColor: T.surface, alignItems: 'center', justifyContent: 'center', zIndex: 30 }}>
            {selIdx >= 0 && segments[selIdx] ? (
              <>
                <Text style={{ fontSize: 20 }}>{segments[selIdx].icon}</Text>
                <Text style={{ color: segments[selIdx].color, fontSize: 11, fontWeight: '800', marginTop: 2 }}>{fmt(segments[selIdx].amt)}</Text>
                <Text style={{ color: 'rgba(255,255,255,0.45)', fontSize: 9, marginTop: 1 }}>{segments[selIdx].pct}%</Text>
              </>
            ) : (
              <>
                <Text style={{ color: T.accent, fontSize: 13, fontWeight: '800' }}>{'₹' + countVal.toLocaleString('en-IN')}</Text>
                <Text style={{ color: T.sub, fontSize: 9, marginTop: 1 }}>total</Text>
              </>
            )}
          </View>
        </View>
        ...
        {selIdx >= 0 && segments[selIdx] && (
          <View style={{ position: 'absolute', bottom: 90, left: 32, right: 32, backgroundColor: T.surface, borderRadius: 20, padding: 16, borderWidth: 1.5, borderColor: segments[selIdx].color + '55' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: segments[selIdx].color + '28', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 24 }}>{segments[selIdx].icon}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: T.text, fontSize: 16, fontWeight: '800' }}>{segments[selIdx].name}</Text>
                <Text style={{ color: segments[selIdx].color, fontSize: 20, fontWeight: '800' }}>{fmt(segments[selIdx].amt)}</Text>
              </View>
              <Text style={{ color: segments[selIdx].color, fontSize: 26, fontWeight: '900' }}>{segments[selIdx].pct}%</Text>
            </View>
          </View>
        )}
        <View style={{ position: 'absolute', bottom: 44, alignItems: 'center' }}>
          <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.18)', marginBottom: 8 }} />
          <Text style={{ color: 'rgba(255,255,255,0.28)', fontSize: 11 }}>Swipe down or tap outside to close</Text>
        </View>
      </Animated.View>
    </Modal>
  );
...
function StatsScreen({ expenses, T, animLevel }) {
  ...
  const mEx   = expenses.filter(e => rangeStart && rangeEnd
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity onPress={() => setShowDateRange(true)} style={[styles.filterPill, { backgroundColor: rangeStart ? T.grad1 : T.surface }]}><Text style={{ color: rangeStart ? '#fff' : T.accent, fontSize: 11 }}>{rangeStart ? dispDate(rangeStart)+' – '+dispDate(rangeEnd) : '📅 Range'}</Text></TouchableOpacity>
            {rangeStart && (<TouchableOpacity onPress={() => { setRangeStart(null); setRangeEnd(null); }} style={[styles.filterPill, { backgroundColor: T.surface }]}><Text style={{ color: T.sub, fontSize: 12 }}>✕</Text></TouchableOpacity>)}
        ...
        {expenses.length === 0 ? (
          ...
          <>
            ...
            {!rangeStart && <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingHorizontal: 16, marginBottom: 12 }}>
              ...
              {catD.length > 0 && (
                <View style={{ flexDirection: 'row', gap: 16, alignItems: 'center', marginBottom: 16 }}>
                  <TouchableOpacity onPress={() => setPieExpanded(true)} activeOpacity={0.85}>
                    <PieChart data={catD} total={catD.reduce((a,c)=>a+c.amt,0)} size={130} T={T} onSelect={idx => setSelSeg(selSeg===idx?-1:idx)} selected={selSeg} />
                  </TouchableOpacity>
                  <Text style={{ color: T.sub, fontSize: 9, textAlign: 'center', marginTop: 4 }}>Tap to expand</Text>
                  <View style={{ flex: 1 }}>
                    {catD.slice(0, 5).map((c, idx) => (
                      <TouchableOpacity key={c.name} onPress={() => setSelSeg(selSeg===idx?-1:idx)}
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 7, opacity: selSeg===-1||selSeg===idx?1:0.4 }}>
                        <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: c.color, transform:[{scale:selSeg===idx?1.3:1}] }} />
                        <Text style={{ color: T.sub, fontSize: 11, flex: 1 }}>{c.icon} {c.name}</Text>
                        <Text style={{ color: selSeg===idx?c.color:T.text, fontSize: 11, fontWeight: '700' }}>{c.pct}%</Text>
                      </TouchableOpacity>
                    ))}
                    {selSeg>=0 && catD[selSeg] && (
                      <View style={{ backgroundColor: catD[selSeg].color+'22', borderRadius: 10, padding: 8, marginTop: 4 }}>
                        <Text style={{ color: catD[selSeg].color, fontSize: 12, fontWeight: '700' }}>{catD[selSeg].icon} {fmt(catD[selSeg].amt)}</Text>
                      </View>
                    )}
                  </View>
                </View>
      ...
      {pieExpanded && catD.length > 0 && (
        <PieExpanded data={catD} total={catD.reduce((a,c)=>a+c.amt,0)} T={T} onClose={() => setPieExpanded(false)} />
      )}
      {showDateRange && (
