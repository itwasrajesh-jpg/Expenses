...
import {
  ...
  Alert, KeyboardAvoidingView, BackHandler, PanResponder, AppState,
  Easing, Share, Modal
} from 'react-native';
...
// ─── HOME SCREEN ─────────────────────────────────────────────────────────────
function HomeScreen({ state, dispatch, onRow, onEdit, onQuickAdd, T, animLevel, collapsed, setCollapsed, onSaveInsights }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Smart Insights — horizontal swipeable cards */}
        {insights.length > 0 && (
          <View style={{ marginBottom: 4 }}>
            <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingHorizontal:16, marginBottom:8 }}>
              <Text style={[styles.secTitle, { color: T.text }]}>💡 Insights</Text>
              <Text style={{ color: T.sub, fontSize: 10 }}>Swipe for more</Text>
            </View>
      ...
      <LottieView source={require('./lottie-shapes.json')} autoPlay loop style={{ width:110, height:110 }} speed={0.7} />
                    ...
                    : catEx.slice(0, 15).map((e, idx) => (
                      <View key={e.id} style={{ marginHorizontal: 16, marginBottom: 7 }}>
                        <View style={[styles.row, { backgroundColor: T.surface2, marginHorizontal: 0, marginBottom: 0 }]}>
                          ...
                          <TouchableOpacity
                            onPress={() => dispatch({ type: 'DELETE', id: e.id })}
  ...
  return (
    <View style={{ flex:1 }}>
      <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
        ...
        {selCat === 'insights' ? null : (<>{/* View toggle - hidden for insights */}
        ...
        <View style={[styles.section, { backgroundColor:T.surface }]}>
          ...
          {selCat === 'insights' && (() => {
            ...
            const now = new Date();
            const sections = [
              { key:'week',    label:'This Week',    items:saved.filter(s=>s.weekKey===`${now.getFullYear()}-W${String(Math.ceil(now.getDate()/7)).padStart(2,'0')}-${String(now.getMonth()+1).padStart(2,'0')}`) },
              { key:'month',   label:'This Month',   items:saved.filter(s=>s.monthKey===now.toISOString().slice(0,7)) },
              { key:'quarter', label:'This Quarter', items:saved.filter(s=>s.quarterKey===`${now.getFullYear()}-Q${Math.ceil((now.getMonth()+1)/3)}`) },
              { key:'year',    label:'This Year',    items:saved.filter(s=>s.yearKey===String(now.getFullYear())) },
            ];
            // Deduplicate by title+savedAt
            const dedup = arr => { const seen=new Set(); return arr.filter(i=>{const k=i.title+i.savedAt; if(seen.has(k))return false; seen.add(k);return true;}); };
            return (
              <View>
                {saved.length === 0
                  ? <View style={styles.emptyBox}>
                      ...
                      <Text style={{color:T.sub,fontSize:12,marginTop:6,textAlign:'center'}}>Insights are saved automatically each day when you open the app</Text>
                    </View>
                  : sections.map(sec => {
                      ...
                      return (
                        <View key={sec.key} style={{ marginBottom:8 }}>
                          <Text style={[styles.secTitle,{color:T.text,marginBottom:10}]}>{sec.label}</Text>
                          {items.map((ins,i) => (
                            <View key={i} style={{ borderRadius:16, padding:14, marginBottom:8, backgroundColor:ins.color+'15', borderWidth:1.5, borderColor:ins.color+'40' }}>
                              <View style={{ flexDirection:'row', alignItems:'center', gap:10, marginBottom:6 }}>
                                ...
                                <Text style={{ color:ins.color, fontSize:13, fontWeight:'800', flex:1 }}>{ins.title}</Text>
                                <Text style={{ color:T.sub, fontSize:10 }}>{ins.savedAt}</Text>
                              </View>
                ...
                {saved.length > 0 && (
                  <TouchableOpacity onPress={() => dispatch({type:'CLEAR_INSIGHTS'})}
                    style={{ marginTop:8,padding:14,borderRadius:14,backgroundColor:'rgba(255,107,107,0.1)',alignItems:'center',borderWidth:1,borderColor:'rgba(255,107,107,0.25)' }}>
...
// ─── SETTINGS SCREEN ──────────────────────────────────────────────────────────
function SettingsScreen({ state, dispatch, T, animLevel, onRecurring, onSubscriptions, onTaxSummary }) {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {tab === 'home'     && <HomeScreen     state={state} dispatch={dispatch} onRow={handleRow} onEdit={handleEdit} onQuickAdd={() => { setFormEx(null); setShowForm(true); }} T={T} animLevel={aL} collapsed={homeCollapsed} setCollapsed={setHomeCollapsed}
        onSaveInsights={insightsBatch => {
          const now = new Date();
          const weekKey = `${now.getFullYear()}-W${String(Math.ceil(now.getDate()/7)).padStart(2,'0')}-${String(now.getMonth()+1).padStart(2,'0')}`;
          const alreadySaved = (state.savedInsights||[]).some(s => s.weekKey === weekKey && s.savedAt === todayStr());
          if (!alreadySaved) {
            insightsBatch.forEach(ins => dispatch({ type:'SAVE_INSIGHT', insight:{ ...ins, weekKey, monthKey:now.toISOString().slice(0,7), quarterKey:`${now.getFullYear()}-Q${Math.ceil((now.getMonth()+1)/3)}`, yearKey:String(now.getFullYear()), savedAt:todayStr() } }));
          }
        }} />
      ...
      {tab === 'settings' && <SettingsScreen state={state} dispatch={dispatch} T={T} animLevel={aL} onRecurring={() => setShowRecurring(true)} onSubscriptions={() => setShowSubscriptions(true)} onTaxSummary={() => setShowTaxSummary(true)} />}
      {showSubscriptions && (
