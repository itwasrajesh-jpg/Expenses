...
const INIT = {
  ...
  accentKey: 'purple', voiceEnabled: false, voiceKeywords: [],
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'ADD_VOICE_KW':   return { ...state, voiceKeywords: [...(state.voiceKeywords||[]), action.kw] };
...
function FormScreen({ existing, onSave, onClose, T, animLevel, voiceKeywords }) {
  ...
  const handleTitleChange = (t) => {
    ...
    if (!existing) {
      ...
      const userMatch = (voiceKeywords||[]).find(k => tl.includes(k.word.toLowerCase()));
...
function parseVoice(text, userKeywords) {
  if (!text) return null;
  ...
  // Step 4 — Check user-taught keywords FIRST — they always win
  if (userKeywords && userKeywords.length > 0) {
    for (const uk of userKeywords) {
      if (cleaned.includes(uk.word.toLowerCase()) || t.includes(uk.word.toLowerCase())) {
        const title = uk.word.charAt(0).toUpperCase() + uk.word.slice(1);
        return { title, amount, category: uk.cat, date: new Date().toISOString().slice(0,10), note: 'Added by voice' };
      }
    }
  }
...
function VoiceListenOverlay({ T, animLevel, onClose, onAdd, userKeywords }) {
  ...
  React.useEffect(() => {
    ...
    const onResult = ExpoSpeechRecognitionModule.addListener('result', event => {
      ...
      if (event.results[0]?.isFinal || event.isFinal) {
        const result = parseVoice(text, userKeywords);
        ...
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24 }}>
          ...
          <View style={{ flexDirection:'row', gap:10 }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setParsed(null); ExpoSpeechRecognitionModule?.start({ lang:'hi-IN', interimResults:true, continuous:false, androidIntentOptions:{ EXTRA_LANGUAGE_MODEL:'web_search', EXTRA_LANGUAGE:'hi-IN', EXTRA_SUPPORTED_LANGUAGES:'hi-IN,en-IN' } }); }}
              ...
              <Text style={{ color:T.sub, fontSize:14, fontWeight:'700' }}>Try Again</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => onAdd(parsed)}
              style={{ flex:2, padding:14, borderRadius:14, backgroundColor:T.accent, alignItems:'center' }}>
              <Text style={{ color:'#fff', fontSize:14, fontWeight:'800' }}>✓ Add Expense</Text>
            </TouchableOpacity>
          </View>
...
function TrainScreen({ state, dispatch, T, animLevel, onClose }) {
  ...
  const [sugCard, setSugCard] = useState(null); // { word, cat }
  const [showPicker, setShowPicker] = useState(false);
  const [pickerFor, setPickerFor] = useState(null); // 'chip' | 'teach'
  ...
  // ── Save keyword ────────────────────────────────────────────────────────────
  const saveKeyword = (word, cat) => {
    const w = word.toLowerCase().trim();
    if (!w || keywords.some(k => k.word === w)) return;
    dispatch({ type:'ADD_VOICE_KW', kw: { word: w, cat: cat.name, icon: cat.icon, color: cat.color } });
    doFlash();
    showToast('🧠 Learned "' + word + '" → ' + cat.name);
  };
      ...
      {close => (
        <>
          ...
          <ScrollView style={{ flex:1 }} contentContainerStyle={{ padding:16, paddingBottom:60 }}
            ...
            {tab === 1 && (
              <>
                <View style={{ ...neuSurface(T), padding:14, marginBottom:12 }}>
                  <Text style={{ fontSize:10, fontWeight:'700', color:T.accent+'80',
                    letterSpacing:0.8, marginBottom:8 }}>WHAT SHOULD I LEARN?</Text>
                  <View style={{ flexDirection:'row', gap:10, alignItems:'center' }}>
                    <TextInput
                      value={teachInput}
                      onChangeText={setTeachInput}
                      placeholder={'e.g. cigarette, chai, sabzi...'}
                ...
                {sugCard && (
                  <Animated.View style={{ ...neuSurface(T), padding:16, marginBottom:12,
                    ...
                    <View style={{ flexDirection:'row', alignItems:'center', gap:12, marginBottom:14 }}>
                      ...
                      <View style={{ flex:1 }}>
                        <Text style={{ color:T.text, fontSize:17, fontWeight:'800' }}>
                          {sugCard.word.charAt(0).toUpperCase()+sugCard.word.slice(1)}
                        </Text>
                        <Text style={{ color:T.sub, fontSize:12, marginTop:2 }}>
                          {'I think this is → '+sugCard.cat.name}
                        </Text>
                      </View>
                    </View>
                    {/* Confidence bar */}
                    ...
                    <View style={{ flexDirection:'row', gap:8 }}>
                      ...
                      <TouchableOpacity onPress={() => { saveKeyword(sugCard.word, sugCard.cat); setSugCard(null); }}
                ...
                <Text style={{ color:T.sub, fontSize:11, textAlign:'center', lineHeight:18 }}>
                  {'Type any word — the app will suggest a category.\nConfirm or pick a different one.'}
                </Text>
              </>
            )}
                ...
                {keywords.length === 0 ? (
                  <View style={{ alignItems:'center', paddingVertical:48 }}>
                    ...
                    <Text style={{ color:T.sub, fontSize:12, textAlign:'center', lineHeight:18 }}>
                      {'Use Quick Add or Teach tab\nto add your personal words'}
                    </Text>
                  </View>
                ) : (
                  keywords.map((kw, i) => (
                    <View key={i} style={{ ...neuSurface(T), flexDirection:'row', alignItems:'center',
                      ...
                      <View style={{ flex:1 }}>
                        <Text style={{ color:T.text, fontSize:14, fontWeight:'700' }}>
                          {kw.word.charAt(0).toUpperCase()+kw.word.slice(1)}
                        </Text>
  ...
  return (
    <Animated.View style={[styles.lockScreen, { backgroundColor: T.bg, opacity: fadeAnim }]}>
      ...
      <LottieView
        ...
        autoPlay loop
        style={{ width: 200, height: 150, marginBottom: 8, backgroundColor:'transparent' }}
        speed={0.9}
        renderMode="SOFTWARE"
      />
...
function AppInner() {
  ...
  const liveInsights = React.useMemo(() => {
    ...
    const savingsRate = (() => { const inc=(state.featEntries?.income||[]).filter(e=>monthOf(e.date)===curMonth()).reduce((a,e)=>a+(e.amount||0),0); if(!inc)return null; return Math.round(((inc-mTotal)/inc)*100); })();
    ...
    const sub = AppState.addEventListener('change', nextState => {
      ...
      if (nextState === 'background') {
        ...
        const timeSinceUnlock = lastUnlockTime.current
          ...
          : Infinity;
        if (elapsed >= threshold && timeSinceUnlock > Math.max(threshold, 5)) {
          setLocked(true);
        }
        bgTime.current = null;
      }
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {showVoiceListen && <VoiceListenOverlay T={T} animLevel={aL} onClose={() => { suppressLock.current = false; setShowVoiceListen(false); }} onAdd={payload => { suppressLock.current = false; dispatch({ type:'ADD', payload }); setShowVoiceListen(false); }} userKeywords={state.voiceKeywords||[]} />}
