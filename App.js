              ...
              <>
                ...
                <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
                  {catEx.length === 0
                    ...
                    : catEx.slice(0, 15).map((e, idx) => <Row key={e.id} expense={e} onPress={(exp) => { close(); setTimeout(() => { setSelBbl(-1); setSheetCat(null); onRow(exp); }, 420); }} T={T} delay={idx * 50} animLevel={animLevel} />)
                  }
                  <View style={{ height: 24 }} />
                </ScrollView>
              </>
...
// ─── PIE CHART ────────────────────────────────────────────────────────────────
function PieChart({ data, total, size, T }) {
  ...
  const segments = data.map((c, i) => {
    ...
    const sa = cumAngle;
    cumAngle += sweep;
    return { ...c, frac, sweep, mid, sa, ea: cumAngle };
  });
  ...
  return (
    <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
      {segments.map((seg, i) => {
        ...
        return (
          <Animated.View key={seg.name} style={{
            ...
            transform: [
              ...
              { scale:      flyAnims[i].interpolate({ inputRange: [0, 0.7, 1], outputRange: [0.15, 1.06, 1] }) },
            ],
            opacity: flyAnims[i].interpolate({ inputRange: [0, 0.25, 1], outputRange: [0, 1, 1] }),
          }}>
      ...
      <View style={{
        ...
        <Text style={{ fontSize: 18 }}>💸</Text>
        <Text style={{ color: T?.accent ?? '#B985FA', fontSize: 9, fontWeight: '700', marginTop: 2 }}>total</Text>
      </View>
    </View>
  );
}
            ...
            <View style={[styles.section, { backgroundColor: T.surface }]}>
              ...
              {catD.length > 0 && (
                <View style={{ flexDirection: 'row', gap: 16, alignItems: 'center', marginBottom: 16 }}>
                  {/* Simple donut chart using stacked arc segments */}
                  <PieChart data={catD} total={catD.reduce((a,c)=>a+c.amt,0)} size={130} T={T} />
                  {/* Legend */}
                  <View style={{ flex: 1 }}>
                    {catD.slice(0, 5).map(c => (
                      <View key={c.name} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                        ...
                        <Text style={{ color: T.text, fontSize: 11, fontWeight: '700' }}>{c.pct}%</Text>
                      </View>
                    ))}
                  </View>
                </View>
