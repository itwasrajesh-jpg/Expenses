  ...
  return (
    <TouchableOpacity onPress={() => onPress(expense)}
      ...
      <Animated.View style={[styles.row, { backgroundColor: T.surface, transform: [{ scale }, { translateX: tx }], opacity: op }]}>
        <View style={[styles.rowIcon, T.white ? { backgroundColor:'#ECEEF3', borderTopWidth:1, borderLeftWidth:1, borderTopColor:'rgba(255,255,255,0.95)', borderLeftColor:'rgba(255,255,255,0.85)', borderBottomWidth:1, borderRightWidth:1, borderBottomColor:'rgba(166,172,189,0.5)', borderRightColor:'rgba(166,172,189,0.4)', elevation:3 } : { backgroundColor: cat.color + '28' }]}>
...
export default function App() {
  ...
  const T = (state.theme === 'default' || state.theme === 'amoled')
    ...
    : {
        ...
        grad2:  _ap.grad2,
        card:   T_base.white
          // white magic: card is the gradient background of spend card
          // use grad1 so it matches the accent (same as default purple behaviour)
          ? _ap.grad1
          : T_base.dark
            // amoled magic: deep dark card tinted with accent
            ? _ap.card
            : _ap.card,
      };
