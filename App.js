...
// WebView for voice recognition — loaded dynamically to avoid import issues
let WebView = null;
...
function VoiceListenOverlay({ T, animLevel, onClose, onAdd }) {
  const [phase, setPhase] = React.useState('listening'); // listening | result | error
  ...
  // Safe WebView load
  const [WebViewComp] = React.useState(() => {
    try { return require('react-native-webview').WebView; } catch(e) { return null; }
  });
  ...
  const handleMessage = React.useCallback(e => {
    try {
      ...
      if (msg.type === 'interim') {
        ...
        setTranscript(msg.transcript);
        const result = parseVoice(msg.transcript);
        if (result) { setParsed(result); setPhase('result'); }
        else { setErrMsg('Could not understand. Try: "Auto 200 transport"'); setPhase('error'); }
      } else if (msg.type === 'error') {
        setErrMsg(msg.message === 'not-allowed' ? 'Microphone permission denied. Please allow in Settings.' : 'Speech error: ' + msg.message);
        setPhase('error');
      } else if (msg.type === 'end' && phase === 'listening') {
        if (!transcript) { setErrMsg('No speech detected. Please try again.'); setPhase('error'); }
      }
    } catch(ex) {}
  }, [phase, transcript]);
    ...
    <View style={{ position:'absolute', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.88)', zIndex:999, alignItems:'center', justifyContent:'center' }}>
      ...
      {phase === 'listening' && (
        <>
          ...
          <TouchableOpacity onPress={onClose} style={{ marginTop:36, paddingHorizontal:28, paddingVertical:12, borderRadius:99, borderWidth:1, borderColor:'rgba(255,255,255,0.2)' }}>
      ...
      {phase === 'result' && parsed && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24 }}>
          ...
          <View style={{ flexDirection:'row', gap:10 }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setParsed(null); }}
      ...
      {phase === 'error' && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24, alignItems:'center' }}>
          ...
          <View style={{ flexDirection:'row', gap:10, width:'100%' }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setErrMsg(''); }}
