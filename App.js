...
function SlideScreen({ children, onClose, zIndex = 50, T, animLevel, originX, originY }) {
  ...
  useEffect(() => {
    ...
    if (!hasOrigin) {
      ...
      Animated.parallel([
        Animated.timing(opac, { toValue:1, duration:120, useNativeDriver:true }),
        // tx: shift from bubble's horizontal position to screen centre
        Animated.timing(tx, { toValue:0, duration:460, easing:Easing.out(Easing.cubic), useNativeDriver:true }),
        // ty: rise from bubble Y position to screen top
        Animated.timing(ty, {
          toValue:0,
          duration:460,
        ...
        Animated.sequence([
          // scaleX: start very narrow (spaghetti), widen as it rises
          Animated.timing(scaleX, { toValue:0.08, duration:80,  easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.35, duration:140, easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.75, duration:120, easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.spring(scaleX, { toValue:1, tension:60, friction:9, useNativeDriver:true }),
        ]),
        Animated.sequence([
          // scaleY: elongate like spaghettification — tall and thin first
          Animated.timing(scaleY, { toValue:1.18, duration:220, easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.spring(scaleY, { toValue:1, tension:70, friction:9, useNativeDriver:true }),
        ]),
      ]).start();
    }
  }, []);
  ...
  const genieClose = useCallback(() => {
    ...
    if (!hasOrigin) {
      ...
      Animated.parallel([
        Animated.timing(opac, { toValue:0, duration:320, delay:60, useNativeDriver:true }),
        // Shift back toward bubble's X
        Animated.timing(tx, { toValue:offX, duration:420, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        // Drop toward bubble's Y
        Animated.timing(ty, {
          toValue: Math.abs(offY) > 20 ? offY * 0.4 : SH * 0.5,
          duration:420, easing:Easing.in(Easing.cubic), useNativeDriver:true,
        }),
        Animated.sequence([
          // scaleY elongates first (spaghetti stretch)
          Animated.timing(scaleY, { toValue:1.3,  duration:160, easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.timing(scaleY, { toValue:0.04, duration:260, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        Animated.sequence([
          // scaleX narrows — becomes a stream
          Animated.timing(scaleX, { toValue:0.5,  duration:180, easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.04, duration:240, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
      ]).start(onClose);
    }
