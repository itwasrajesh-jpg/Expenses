...
function LiquidBudgetBar({ pct, animLevel }) {
  ...
  const waveTranslate = waveAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -44] });

  return (
    <View style={{ height: 10, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 99, overflow: 'hidden', marginTop: 12, marginBottom: 4 }}>
      <Animated.View style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: fillWidth, overflow: 'hidden', borderRadius: 99 }}>
        {/* Solid base */}
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: fillColor, opacity: 0.85 }} />
        {/* Wave overlay */}
        <Animated.View style={{ position: 'absolute', top: -4, left: 0, right: -44, height: 18, transform: [{ translateX: waveTranslate }] }}>
          {/* Two sine wave approximations using border radius trick */}
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 0 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 22 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 44 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 66 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 88 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 110 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 132 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 154 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 176 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 198 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 220 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 242 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 264 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 286 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 308 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 330 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 352 }} />
          <View style={{ position: 'absolute', width: 22, height: 10, borderRadius: 99, backgroundColor: 'rgba(255,255,255,0.25)', top: 0, left: 374 }} />
        </Animated.View>
        {/* Shimmer */}
        <Animated.View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, opacity: waveAnim.interpolate({ inputRange:[0,0.5,1], outputRange:[0.15,0.35,0.15] }), backgroundColor: '#fff' }} />
      </Animated.View>
    </View>
  );
}
  ...
  return (
    <Animated.View style={{ transform:[{ scale:pulseAnim }], marginBottom: 12 }}>
      ...
      <Animated.View style={{
        ...
        borderRadius: 20, backgroundColor:'#2ECC8A',
      ...
      {sparkAnims.map((spark, i) => (
        <Animated.View key={i} style={{
          ...
          transform:[{ translateX: spark.x }, { translateY: spark.y }, { scale: spark.sc }],
          opacity: spark.op,
        }} />
      ))}
      {/* Badge */}
      <View style={{ backgroundColor:'#2ECC8A', borderRadius:14, padding:12,
        ...
        shadowColor:'#2ECC8A', shadowOpacity:0.6, shadowRadius:12, elevation:8 }}>
...
function StatsScreen({ expenses, featEntries, bills, dispatch, T, animLevel, selCat, setSelCat }) {
  ...
  const buildData = () => {
    if (selCat === 'income') {
      ...
      const totalOutflow = totalExpenses + totalGoals + totalEMI + totalInvestments;
  ...
  const { breakdown: catD, total: catTotal, incomeAlloc, goalAlloc } = buildData();
...
// ─── APP ──────────────────────────────────────────────────────────────────────
// ─── PARTICLE BURST (fires when expense is added) ────────────────────────────
function ParticleBurst({ color, onDone }) {
  const N = 12;
  ...
  useEffect(() => {
    const anims = particles.map((p, i) => {
      ...
      return Animated.parallel([
        Animated.timing(p.x,  { toValue: tx,  duration: 600, easing: Easing.out(Easing.quad), useNativeDriver: true }),
        Animated.timing(p.y,  { toValue: ty,  duration: 600, easing: Easing.out(Easing.quad), useNativeDriver: true }),
        Animated.timing(p.sc, { toValue: 0,   duration: 600, easing: Easing.in(Easing.quad),  useNativeDriver: true }),
        Animated.timing(p.op, { toValue: 0,   duration: 500, delay: 100,                       useNativeDriver: true }),
      ]);
    });
    Animated.parallel(anims).start(() => onDone && onDone());
  }, []);

  const COLORS = [color, '#FFD700', '#fff', color+'cc', '#FF6B6B', '#B985FA'];

  return (
    <View style={{ position: 'absolute', top: '50%', left: '50%', width: 0, height: 0, zIndex: 999 }} pointerEvents="none">
      {particles.map((p, i) => (
        <Animated.View key={i} style={{
          position: 'absolute',
          width:  i % 3 === 0 ? 8 : 5,
          height: i % 3 === 0 ? 8 : 5,
          borderRadius: i % 3 === 0 ? 4 : 2.5,
...
export default function App() {
  ...
  const [burst,         setBurst]         = useState(null); // {color} when a new expense is added
  ...
  const handleSave   = p  => {
    if (p.id) {
      ...
      const cat = CM[p.category] ?? CM['Other'];
      setBurst({ id: Date.now(), color: cat.color });
    }
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {burst && <ParticleBurst key={burst.id} color={burst.color} onDone={() => setBurst(null)} />}
