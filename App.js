...
function HomeScreen({ state, dispatch, onRow, onEdit, onQuickAdd, T, animLevel, collapsed, setCollapsed, onSaveInsights, breathAnim }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      {/* Glass theme aurora blobs — breathe in sync */}
      {T.glass && (
        <View style={{ position:'absolute', top:0, left:0, right:0, bottom:0, overflow:'hidden' }} pointerEvents="none">
          <Animated.View style={{
            position:'absolute', width:340, height:340, borderRadius:170,
            backgroundColor:'rgba(103,80,164,0.22)',
            top:-100, left:-80,
        ...
        <Animated.View style={[{ marginHorizontal: 16, marginBottom: 12 }, aS(cardAnim)]}>
          ...
          <View style={T.glass ? {
            ...
            <View style={{ position:'relative', zIndex:1, padding: T.glass ? 20 : 0 }}>
              <Text style={[styles.cardLabel, T.glass && { color:'rgba(255,255,255,0.55)', fontSize:10, letterSpacing:0.5 }]}>
              ...
              <View style={styles.cardRow}>
                {[['Budget', state.budget], ['Remaining', Math.max(state.budget - spent, 0)]].map(([l, v]) => (
                  <View key={l} style={[styles.cardPill, T.glass && { backgroundColor:'rgba(255,255,255,0.09)', borderWidth:1, borderColor:'rgba(255,255,255,0.08)' }]}>
