...
// Safe LottieView — wrapped in try/catch so a bad lottie install
...
// Safe LocalAuthentication fallback
const SafeLocalAuth = LocalAuthentication || {
  authenticateAsync: async () => ({ success: false }),
  hasHardwareAsync: async () => false,
  isEnrolledAsync: async () => false,
};

// Error boundary to catch and display crashes instead of silent close
    ...
    if (this.state.error) {
      return (
        <View style={{ flex:1, backgroundColor:'#000', alignItems:'center', justifyContent:'center', padding:24 }}>
          ...
          <Text style={{ color:'rgba(255,255,255,0.5)', fontSize:10, marginTop:12 }}>{this.state.error.stack?.slice(0,300)}</Text>
        </View>
      );
    }
...
function SettingsScreen({ state, dispatch, T, animLevel, onRecurring, onSubscriptions, onTaxSummary, onBackup }) {
  ...
  useEffect(() => {
    SafeLocalAuth.hasHardwareAsync().then(has =>
      SafeLocalAuth.isEnrolledAsync().then(en => setBioAvail(has && en))
    );
    const init = {};
...
function LockScreen({ pin, biometric, onUnlock, T }) {
  ...
  const tryBio = async () => {
    try { const r = await SafeLocalAuth.authenticateAsync({ promptMessage: 'Unlock Expenses', fallbackLabel: 'Use PIN' }); if (r.success) onUnlock(); } catch (e) {}
  };
  const shake = () => {
    Animated.sequence([
