...
// ─── CONSTELLATION STAR ───────────────────────────────────────────────────────
function ConstellationStar({ ins, index, onPress, animLevel }) {
  const scaleAnim = useRef(new Animated.Value(0)).current;
  const glowAnim  = useRef(new Animated.Value(0)).current;
  const starRef   = useRef(null);
  const isHero = ins.size === 'h', isMid = ins.size === 'm';
  const sz  = isHero ? 44 : isMid ? 34 : 28;
  const fs  = isHero ? 20 : isMid ? 16 : 13;
  // Glow: just 1.4x star size — subtle halo, not a massive blob
  const gsz = sz * 1.4;

  useEffect(() => {
    if (animLevel === 'none') { scaleAnim.setValue(1); return; }
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(glowAnim, { toValue:1, duration:1500+index*300, easing:Easing.inOut(Easing.sin), useNativeDriver:true }),
        Animated.timing(glowAnim, { toValue:0.3, duration:1500+index*300, easing:Easing.inOut(Easing.sin), useNativeDriver:true }),
      ])
    ).start();
  }, []);
  ...
  const handlePress = () => {
    ...
    starRef.current.measure((fx, fy, w, h, px, py) => {
      onPress(ins, px + w/2, py + h/2);
    });
  };

  return (
    <Animated.View ref={starRef} style={{ alignItems:'center', transform:[{ scale: scaleAnim }] }}>
      {/* Glow ring */}
      <Animated.View style={{
        position:'absolute', width:gsz, height:gsz, borderRadius:gsz/2,
        backgroundColor: ins.color + '25',
        top: -(gsz-sz)/2, left: -(gsz-sz)/2,
        opacity: glowAnim,
      }} pointerEvents="none" />
      <TouchableOpacity onPress={handlePress} activeOpacity={0.8}>
        <View style={{
          ...
          borderWidth:1.5, borderColor: ins.color + '70',
          alignItems:'center', justifyContent:'center',
          shadowColor: ins.color, shadowOpacity:0.5, shadowRadius:8, elevation:6,
        }}>
          <Text style={{ fontSize:fs }}>{ins.icon}</Text>
        </View>
      </TouchableOpacity>
      <Text style={{ fontSize:9, fontWeight:'700', color:'rgba(230,225,229,0.55)',
        marginTop:5, textAlign:'center', width:72 }} numberOfLines={2}>
        {ins.title}
      </Text>
    </Animated.View>
  );
...
function SavedInsightsView({ savedInsights, dispatch, T, animLevel }) {
  ...
  // Assign size based on badge
  const withSize = arr => arr.map(i => ({
    ...i,
    size: i.badge === 'DNA' || i.badge === 'SCORE' ? 'h' : i.badge ? 'm' : 's',
    co: i.color, ic: i.icon, tt: i.title, tx: i.text,
  }));

  const [curSec, setCurSec] = useState('month');
  const [activeIns, setActiveIns] = useState(null);
  const [starPos, setStarPos] = useState({ x:0, y:0 });

  // Card animation — scale from 0.04 to 1, offset to star position
  const cardScale = useRef(new Animated.Value(0.04)).current;
  const cardOpacity = useRef(new Animated.Value(0)).current;
  const [cardOpen, setCardOpen] = useState(false);

  // Pixel-based positions calculated from actual screen width
  // Star cell = 80px wide (44px star + padding), field = SW-32 wide, height=380
  // Positions are centre of each star
  const FW = SW - 32; // field width
  const FH = 380;
  const CELL = 82; // min distance between star centres
  // Pre-computed grid slots — 3 columns, rows as needed
  // Spread evenly with guaranteed no-overlap
  const getPositions = (n) => {
    const cols = n <= 2 ? n : n <= 6 ? 3 : 3;
    const rows = Math.ceil(n / cols);
    const colW = FW / cols;
    const rowH = (FH - 40) / rows;
    const positions = [];
    for (let i = 0; i < n; i++) {
      const col = i % cols;
      const row = Math.floor(i / cols);
      // Centre of each cell with slight offset for natural look
      const offX = (i % 3 === 1) ? 0 : (i % 2 === 0 ? -8 : 8);
      const offY = (row % 2 === 1) ? 6 : 0;
      positions.push({
        x: colW * col + colW / 2 + offX,
        y: rowH * row + rowH / 2 + 20 + offY,
      });
    }
    return positions;
  };

  const openCard = (ins, sx, sy) => {
    setActiveIns(ins);
    setStarPos({ x:sx, y:sy });
    setCardOpen(true);
    cardScale.setValue(0.04);
    cardOpacity.setValue(0);
    if (animLevel === 'none') { cardScale.setValue(1); cardOpacity.setValue(1); return; }
    Animated.parallel([
  ...
  const closeCard = () => {
    if (animLevel === 'none') { setCardOpen(false); setActiveIns(null); return; }
    Animated.parallel([
      Animated.timing(cardScale,   { toValue:0.04, duration:260, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
      Animated.timing(cardOpacity, { toValue:0,    duration:200, easing:Easing.in(Easing.quad),  useNativeDriver:true }),
    ]).start(() => { setCardOpen(false); setActiveIns(null); });
  };
  ...
  const curItems = withSize(dedup(SECTIONS.find(s=>s.key===curSec)?.items||[]));
  const positions = getPositions(curItems.length);
    ...
    <View style={{ flex:1 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {SECTIONS.map(sec => {
          ...
          return (
            <TouchableOpacity key={sec.key} onPress={() => { closeCard(); setCurSec(sec.key); }}
      ...
      <View style={{ height:380, position:'relative' }}>
        ...
        {[...Array(40)].map((_,i) => (
          <View key={i} pointerEvents="none" style={{
            ...
            borderRadius:1,
            backgroundColor:'rgba(255,255,255,'+(0.08+i%5*0.04)+')',
          }} />
        ))}

        {/* Stars positioned at pixel coordinates — guaranteed no overlap */}
        {curItems.map((ins,i) => {
          const pos = positions[i] || { x: FW/2, y: FH/2 };
          const starSz = ins.size==='h' ? 44 : ins.size==='m' ? 34 : 28;
          return (
            <View key={ins.title+i} style={{
              position:'absolute',
              left: pos.x - starSz/2,
              top:  pos.y - starSz/2,
            }}>
      ...
      {/* Expanded card overlay — scale from star position */}
      {cardOpen && activeIns && (
        <>
          {/* Backdrop */}
          <TouchableWithoutFeedback onPress={closeCard}>
            <Animated.View style={{
              position:'absolute', top:-200, left:-20, right:-20, bottom:-200,
              backgroundColor:'rgba(0,0,0,0.55)',
              opacity: cardOpacity,
            }} />
          </TouchableWithoutFeedback>

          {/* Card — uses translateX/Y to fake transform-origin at star position */}
          <Animated.View style={{
            position:'absolute',
            left:16, right:16,
            top: 40,
            ...
            opacity: cardOpacity,
            transform:[
              // Translate to star, scale from there, translate back
              { translateX: starPos.x - (SW/2) },
              { translateY: starPos.y - 200 },
              { scale: cardScale },
              { translateX: -(starPos.x - (SW/2)) },
              { translateY: -(starPos.y - 200) },
            ],
            ...
            <View style={{
              backgroundColor: T.bg, borderRadius:22, padding:22,
              borderWidth:1.5, borderColor: activeIns.color+'40',
              shadowColor: activeIns.color, shadowOpacity:0.4,
              shadowRadius:20, elevation:16,
            }}>
              {/* Glow top */}
              <View style={{ position:'absolute', top:0, left:0, right:0, height:80,
                borderRadius:22, overflow:'hidden', opacity:0.5 }} pointerEvents="none">
                <View style={{ position:'absolute', top:-20, left:'25%', right:'25%',
                  height:60, borderRadius:30,
                  backgroundColor: activeIns.color+'40' }} />
              </View>
              <View style={{ position:'absolute', top:0, left:0, right:0, height:1,
                borderRadius:22, backgroundColor:'rgba(255,255,255,0.15)' }} />
              <Text style={{ fontSize:36, marginBottom:8 }}>{activeIns.icon}</Text>
              {activeIns.badge && (
                <View style={{ backgroundColor: activeIns.color+'20', borderRadius:8,
                  paddingHorizontal:10, paddingVertical:4, alignSelf:'flex-start', marginBottom:10 }}>
                  <Text style={{ color:activeIns.color, fontSize:10, fontWeight:'800', letterSpacing:0.5 }}>
                    {activeIns.badge}
                  </Text>
                </View>
              )}
              <Text style={{ fontSize:17, fontWeight:'800', color:activeIns.color, marginBottom:8 }}>
                {activeIns.title}
              </Text>
              <Text style={{ fontSize:13, lineHeight:20, color:T.sub, marginBottom:16 }}>
                {activeIns.text}
              </Text>
              <TouchableOpacity onPress={closeCard}
                style={{ padding:12, borderRadius:13, backgroundColor:'rgba(255,255,255,0.07)', alignItems:'center' }}>
                <Text style={{ fontSize:13, fontWeight:'700', color:T.text }}>✕  Close</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        </>
...
export default function App() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {tab === 'stats'    && <StatsScreen    expenses={state.expenses} featEntries={state.featEntries} bills={state.bills||[]} savedInsights={state.savedInsights||[]} dispatch={dispatch} T={T} animLevel={aL} selCat={statsCat} setSelCat={setStatsCat} onEditExpense={e => { setFormEx(e); setShowForm(true); }} budget={state.budget} />}
