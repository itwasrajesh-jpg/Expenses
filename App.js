...
// ─── PIE CHART ────────────────────────────────────────────────────────────────
function PieChart({ data, total, size }) {
  const R = size / 2;
  const innerR = R * 0.55;
  ...
  // Build segments as SVG-style using View with border trick
  // Each segment: a View rotated to its start angle, with a colored arc
  let cumPct = 0;
  const segments = data.map(c => {
    const pct = c.amt / total;
    const seg = { ...c, pct, startPct: cumPct };
    cumPct += pct;
    return seg;
  });
  ...
  return (
    <View style={{ width: size, height: size, position: 'relative', alignItems: 'center', justifyContent: 'center' }}>
      {segments.map((seg, i) => {
        const startAngle = seg.startPct * 360 - 90;
        const sweepAngle = seg.pct * 360;
        // Draw each segment as two halves if > 180 deg
        return (
          <ArcSegment
            key={seg.name}
            color={seg.color}
...
function ArcSegment({ color, startAngle, sweepAngle, R, strokeW, anim, index, total }) {
  const cr = R - strokeW / 2;
  const delay = index * (600 / total);

  // We approximate arc segments using thin rotated rectangles stacked
  // For a proper arc we draw multiple thin slices
  const slices = Math.max(Math.ceil(sweepAngle / 6), 1);
  const sliceAngle = sweepAngle / slices;

  return (
    <>
      {Array.from({ length: slices }, (_, s) => {
        const angle = (startAngle + s * sliceAngle + sliceAngle / 2) * (Math.PI / 180);
        const x = R + Math.cos(angle) * cr - strokeW / 2;
        const y = R + Math.sin(angle) * cr - strokeW / 2;
        return (
          <View key={s} style={{
            position: 'absolute',
            width: strokeW,
            ...
            <View style={[styles.section, { backgroundColor: T.surface }]}>
              ...
              {catD.length > 0 && (
                <View style={{ flexDirection: 'row', gap: 16, alignItems: 'center', marginBottom: 16 }}>
                  ...
                  <PieChart data={catD} total={catD.reduce((a,c)=>a+c.amt,0)} size={130} />
                  {/* Legend */}
                  <View style={{ flex: 1 }}>
