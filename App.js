...
// ── NLP PARSER ────────────────────────────────────────────────────────────────
const VOICE_CATS = [
  { keys:['food','eat','zomato','swiggy','restaurant','lunch','dinner','breakfast','coffee','cafe','pizza','burger','biryani','meal','snack'], name:'Food', icon:'🍔' },
  { keys:['transport','auto','uber','ola','cab','bus','metro','train','fuel','petrol','taxi','travel','fare','rickshaw'], name:'Transport', icon:'🚗' },
  { keys:['shopping','clothes','amazon','flipkart','myntra','purchase','buy','bought'], name:'Shopping', icon:'🛍️' },
  { keys:['entertainment','movie','game','beer','alcohol','bar','pub','party','show','film'], name:'Entertainment', icon:'🎬' },
  { keys:['subscriptions','subscription','netflix','spotify','hotstar','youtube','prime','disney','streaming'], name:'Subscriptions', icon:'📱' },
  { keys:['utilities','electricity','water','gas','bill','recharge','mobile'], name:'Utilities', icon:'💡' },
  { keys:['health','medicine','doctor','pharmacy','hospital','gym','fitness','medical'], name:'Health', icon:'💊' },
  { keys:['education','book','course','tuition','school','college','fees'], name:'Education', icon:'📚' },
  { keys:['grocery','groceries','vegetables','fruits','milk','bigbasket','blinkit','zepto','supermarket'], name:'Groceries', icon:'🛒' },
  { keys:['dining','restaurant','dine','lunch','dinner'], name:'Dining', icon:'🍽️' },
];
...
function parseVoice(text, userKeywords) {
  ...
  const amount = nums ? parseInt(nums[0]) : 0;
  if (amount <= 0) return null;

  // Check user-taught keywords FIRST — they always win
  if (userKeywords && userKeywords.length > 0) {
    for (const uk of userKeywords) {
      if (t.includes(uk.word.toLowerCase())) {
  ...
  // Find category from built-in list
  let cat = { name:'Other', icon:'🎯' };
  for (const vc of VOICE_CATS) {
    if (vc.keys.some(k => t.includes(k))) { cat = vc; break; }
  }

  // Build title — remove ALL category keywords and category names, use matched key as fallback
  const stop = ['add','rupees','rs','inr','spent','paid','for','on','under','to','and','the','a','of','worth'];
  const genericCatWords = ['food','transport','shopping','entertainment','utilities','health','education','groceries','grocery','dining','subscriptions','subscription','travelling','travel'];
  const allCatWords = VOICE_CATS.flatMap(vc => [...vc.keys, vc.name.toLowerCase()]);
  const catNameLower = cat.name.toLowerCase();
  // Prefer specific brand/item names (netflix, zomato, beer) over generic words (subscription, food)
  const matchedKey = cat.keys.find(k => t.includes(k) && k !== catNameLower && !genericCatWords.includes(k))
    || cat.keys.find(k => t.includes(k) && k !== catNameLower)
    || cat.keys.find(k => t.includes(k)) || '';
  const words = t.split(/\s+/).filter(w =>
    w && !w.match(/^\d+$/) && !stop.includes(w) && !allCatWords.includes(w)
  );
  // If no non-category words remain, use the matched trigger as title (e.g. "beer", "auto", "netflix")
    ...
    <View style={{ position:'absolute', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.88)', zIndex:999, alignItems:'center', justifyContent:'center' }}>
      ...
      {phase === 'result' && parsed && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24 }}>
          ...
          <View style={{ flexDirection:'row', gap:10 }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setParsed(null); ExpoSpeechRecognitionModule?.start({ lang:'en-IN', interimResults:true, continuous:false }); }}
      ...
      {phase === 'error' && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24, alignItems:'center' }}>
          ...
          <View style={{ flexDirection:'row', gap:10, width:'100%' }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setErrMsg(''); ExpoSpeechRecognitionModule?.start({ lang:'en-IN', interimResults:true, continuous:false }); }}
