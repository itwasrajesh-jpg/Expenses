...
function SlideScreen({ children, onClose, zIndex = 50, T, animLevel, originX, originY }) {
  ...
  const genieClose = useCallback(() => {
    ...
    if (!hasOrigin) {
      // Standard genie close
      Animated.parallel([
        Animated.timing(ty,   { toValue:SH*0.6, duration:Math.round(380*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        Animated.sequence([
          Animated.timing(scaleY, { toValue:0.6,  duration:180, easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleY, { toValue:0.04, duration:200, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        Animated.sequence([
          Animated.timing(scaleX, { toValue:0.5,  duration:200, easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.04, duration:180, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        Animated.timing(opac, { toValue:0, duration:340, delay:40, useNativeDriver:true }),
      ]).start(onClose);
    } else {
      // ── SPAGHETTIFICATION back to bubble ───────────────────────────────────
      // Sheet narrows to a stream and flows back toward bubble origin
      Animated.parallel([
        Animated.timing(opac, { toValue:0, duration:Math.round(320*DM), delay:Math.round(60*DM), useNativeDriver:true }),
        Animated.timing(tx, { toValue:offX, duration:Math.round(420*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        Animated.timing(ty, {
          toValue: Math.abs(offY) > 20 ? offY * 0.4 : SH * 0.5,
          duration:Math.round(420*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true,
        }),
        Animated.sequence([
          Animated.timing(scaleY, { toValue:1.3,  duration:Math.round(160*DM), easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.timing(scaleY, { toValue:0.04, duration:Math.round(260*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        Animated.sequence([
          Animated.timing(scaleX, { toValue:0.5,  duration:Math.round(180*DM), easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.04, duration:Math.round(240*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
      ]).start(onClose);
    }
