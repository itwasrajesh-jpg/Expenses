...
// neuSurface: use tileStyle for white/dark, plain surface for others
// Drop-in replacement for { backgroundColor: T.surface } on card Views
function neuSurface(T, extra = {}) {
  if (T.white || T.dark) return tileStyle(T, extra);
  return { backgroundColor: T.surface, borderRadius: 18, ...extra };
}

function tileStyle(T, extra = {}) {
  if (T.white) return {
    // Neumorphic: tile bg = page bg, directional borders fake the light source
    backgroundColor: '#ECEEF3',
    borderRadius: 18,
    // Android elevation gives the base lift
    elevation: 4,
    // Top-left: near-white border = light catching the raised edge
    borderTopWidth: 1.5,
    borderLeftWidth: 1.5,
    borderTopColor: 'rgba(255,255,255,0.95)',
    borderLeftColor: 'rgba(255,255,255,0.85)',
    // Bottom-right: grey border = shadow side of the raised tile
    borderBottomWidth: 1.5,
    borderRightWidth: 1.5,
    borderBottomColor: 'rgba(166,172,189,0.55)',
    borderRightColor: 'rgba(166,172,189,0.45)',
    // iOS shadow (one direction only on iOS)
    shadowColor: '#A6ACBD',
    shadowOpacity: 0.5,
    shadowRadius: 8,
