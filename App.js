...
function FeatEditModal({ entry, fid, dispatch, onClose, T }) {
  ...
  const close = (cb) => {
    Animated.parallel([
      Animated.spring(scaleAnim, { toValue: 0, tension: 80, friction: 10, useNativeDriver: true }),
  ...
  return (
    <Modal transparent visible animationType="none">
      <Animated.View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', alignItems: 'center', justifyContent: 'center', padding: 24, opacity: opAnim }}>
        <TouchableOpacity style={{ position:'absolute', inset:0 } } onPress={() => close()} activeOpacity={1} />
        <Animated.View style={{
          ...
          <View style={{ position:'absolute', inset:0, backgroundColor: '#1C1B1F', opacity: 0.88, borderRadius: 24 }} />
...
// ─── STATS SCREEN ─────────────────────────────────────────────────────────────
function StatsScreen({ expenses, featEntries, dispatch, T, animLevel }) {
  const allM = [...new Set(expenses.map(e => monthOf(e.date)))].sort().reverse();
  const [selCat,      setSelCat]      = useState('expenses');
  ...
  const buildData = () => {
    ...
    const COLORS = ['#B985FA','#2ECC8A','#FF9800','#2196F3','#E91E63','#00BCD4','#FF6B6B','#8BC34A','#FF5722','#9C27B0'];
...
'), title:'Expenses.csv' }); } catch(e) {}
  ...
  return (
    <View style={{ flex:1 }}>
      <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingHorizontal:16, paddingTop:16, paddingBottom:8 }}>
          ...
          <View style={{ flexDirection:'row', gap:8, alignItems:'center' }}>
            ...
            <TouchableOpacity onPress={() => setShowCatMenu(p=>!p)}
              style={{ flexDirection:'row', alignItems:'center', gap:6, backgroundColor:T.surface, borderRadius:12, paddingHorizontal:12, paddingVertical:7, borderWidth:1, borderColor:activeCat.color+'44' }}>
              <View style={{ width:8, height:8, borderRadius:4, backgroundColor:activeCat.color }} />
              <Text style={{ color:T.text, fontSize:12, fontWeight:'600' }}>{activeCat.label}</Text>
              <Text style={{ color:T.sub, fontSize:10 }}>▾</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setShowDateRange(true)} style={[styles.filterPill, { backgroundColor: rangeStart ? T.grad1 : T.surface }]}>
              <Text style={{ color: rangeStart?'#fff':T.accent, fontSize:11 }}>{rangeStart ? '📅' : '📅'}</Text>
            </TouchableOpacity>
            {rangeStart && (
        ...
        {showCatMenu && (
          <View style={{ marginHorizontal:16, marginBottom:10, backgroundColor:T.surface2, borderRadius:16, padding:8, borderWidth:1, borderColor:T.border }}>
            {CAT_OPTIONS.map(opt => (
              <TouchableOpacity key={opt.key} onPress={() => { setSelCat(opt.key); setShowCatMenu(false); }}
                style={{ flexDirection:'row', alignItems:'center', gap:10, padding:10, borderRadius:11, backgroundColor: selCat===opt.key ? opt.color+'18' : 'transparent' }}>
                <View style={{ width:10, height:10, borderRadius:5, backgroundColor:opt.color }} />
                <Text style={{ fontSize:14 }}>{opt.icon}</Text>
                <Text style={{ color: selCat===opt.key ? opt.color : T.text, fontSize:13, fontWeight: selCat===opt.key?'700':'400', flex:1 }}>{opt.label}</Text>
                {selCat===opt.key && <Text style={{ color:opt.color }}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        )}
        ...
        {!rangeStart && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingHorizontal:16, marginBottom:12 }}>
            {(selCat==='expenses' ? allM : [...new Set((featEntries[selCat]||[]).map(e=>monthOf(e.date)))].sort().reverse()).slice(0,6).map(m => (
              ...
              <TouchableOpacity onPress={() => setPieExpanded(true)} activeOpacity={0.85}>
                <StatsPie
                  ...
                  onDoubleTap={selCat!=='expenses' ? (seg) => {
                    ...
                    if (entries.length > 0) setEditEntry(entries[entries.length-1]);
                  } : null}
                />
              </TouchableOpacity>
              <View style={{ flex:1 }}>
      ...
      {editEntry && (
        <FeatEditModal entry={editEntry} fid={selCat} dispatch={dispatch} T={T} onClose={() => setEditEntry(null)} />
      )}
    </View>
  );
}
          ...
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={[styles.formField, { backgroundColor: T.surface }]}>
                ...
                <TextInput value={title} onChangeText={setTitle}
                  placeholder="e.g. Monthly Salary" placeholderTextColor={T.sub}
...
export default function App() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {tab === 'stats'    && <StatsScreen    expenses={state.expenses} featEntries={state.featEntries} dispatch={dispatch} T={T} animLevel={aL} />}
