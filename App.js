...
const INIT = {
  ...
  accentKey: 'purple', voiceEnabled: false,
...
const VOICE_CATS = [
  ...
  { keys:['entertainment','movie','game','beer','alcohol','bar','pub','party','netflix','show','film'], name:'Entertainment', icon:'🎬' },
  ...
  { keys:['subscriptions','subscription','spotify','hotstar','youtube','prime','disney','streaming'], name:'Subscriptions', icon:'📱' },
];
...
function parseVoice(text) {
  ...
  // Build title — remove numbers and stop words
  const stop = ['add','rupees','rs','inr','spent','paid','for','on','under','to','and','the','a','of','worth'];
  const catWords = VOICE_CATS.flatMap(vc => vc.keys);
  const words = t.split(/\s+/).filter(w =>
    w && !w.match(/^\d+$/) && !stop.includes(w) && !catWords.includes(w)
  );
...
function VoiceListenOverlay({ T, animLevel, onClose, onAdd }) {
...
function VoiceScreen({ state, dispatch, T, animLevel, onClose }) {
...
function AppInner() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {showVoice  && <VoiceScreen  state={state} dispatch={dispatch} T={T} animLevel={aL} onClose={() => setShowVoice(false)}  />}
      {showVoiceListen && <VoiceListenOverlay T={T} animLevel={aL} onClose={() => setShowVoiceListen(false)} onAdd={payload => { dispatch({ type:'ADD', payload }); setShowVoiceListen(false); }} />}
      {showRecurring && (
