...
const THEMES = {
  ...
  glass: {
    bg: '#0E0C14', surface: 'rgba(255,255,255,0.07)', surface2: 'rgba(255,255,255,0.04)',
    text: '#E6E1E5', sub: 'rgba(230,225,229,0.45)',
    border: 'rgba(255,255,255,0.12)', accent: '#B985FA',
    grad1: '#6750A4', grad2: '#9C68E8', nav: 'rgba(14,12,20,0.85)', card: 'rgba(103,80,164,0.25)',
    glass: true,
  },
};
...
// ─── HOME SCREEN ─────────────────────────────────────────────────────────────
function HomeScreen({ state, dispatch, onRow, onEdit, onQuickAdd, T, animLevel, collapsed, setCollapsed, onSaveInsights, breathAnim }) {
    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[{ marginHorizontal: 16, marginBottom: 12 }, aS(cardAnim)]}>
          {/* Glass theme: animated breathing glow around card */}
          <Animated.View style={T.glass ? {
            borderRadius: 28, marginBottom: 0,
            shadowColor: '#B985FA',
            shadowOpacity: breathAnim ? breathAnim.interpolate({ inputRange:[0,1], outputRange:[0.15,0.55] }) : 0.3,
            shadowRadius:  breathAnim ? breathAnim.interpolate({ inputRange:[0,1], outputRange:[12,32]    }) : 16,
            elevation:     12,
            transform: [{ scale: breathAnim ? breathAnim.interpolate({ inputRange:[0,1], outputRange:[1,1.008] }) : 1 }],
          } : {}}>
          <View style={[styles.card, { backgroundColor: T.card, borderRadius: T.glass ? 28 : undefined,
            borderWidth: T.glass ? 1 : 0, borderColor: T.glass ? 'rgba(255,255,255,0.13)' : 'transparent' }]}>
            {T.glass && <View style={{ position:'absolute', top:0, left:0, right:0, height:1, borderRadius:28, backgroundColor:'rgba(255,255,255,0)', overflow:'hidden' }}><View style={{ position:'absolute', top:0, left:0, right:0, height:1, backgroundColor:'transparent', borderTopWidth:1, borderTopColor:'rgba(255,255,255,0.25)' }} /></View>}
          ...
          <View key={sec.key} style={{ marginBottom: 8 }}>
            ...
            {!isCollapsed && items.map((ins, i) => (
              <View key={i} style={{ borderRadius:16, padding:14, marginBottom:8,
                backgroundColor:ins.color+'15', borderWidth:1.5, borderColor:ins.color+'40' }}>
      ...
      <View style={styles.settingSection}>
        ...
        <View style={[styles.settingCard, { backgroundColor: T.surface }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
            {[['default', '🌑 Default'], ['amoled', '⬛ AMOLED'], ['glass', '🪟 Glass']].map(([key, label]) => (
...
export default function App() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      <StatusBar barStyle="light-content" backgroundColor={T.bg} />
      {tab === 'home' && <HomeScreen state={state} dispatch={dispatch} onRow={handleRow} onEdit={handleEdit} onQuickAdd={() => { setFormEx(null); setShowForm(true); }} T={T} animLevel={aL} collapsed={homeCollapsed} setCollapsed={setHomeCollapsed} onSaveInsights={handleSaveInsights} breathAnim={breathAnim} />}
