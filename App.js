...
import {
  ...
  Alert, KeyboardAvoidingView, BackHandler, PanResponder, AppState, Share,
  Easing, Modal
} from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import LottieView from 'lottie-react-native';
import * as LocalAuthentication from 'expo-local-authentication';
// expo-speech-recognition handles voice recognition natively
...
try {
  ...
  console.log('[VOICE] expo-speech-recognition not available:', e.message);
}
...
console.log('[APP] App.js loaded, DEBUG_MODE:', DEBUG_MODE);
const SPRING      = { tension: 68, friction: 12, useNativeDriver: true };
const SPRING_LITE = { toValue: 1, tension: 90, friction: 18, useNativeDriver: true }; // snappier, fewer frames on low-end
...
const THEMES = {
  ...
  matte: {
    // bg = surface = same colour — neumorphic tiles emerge from the surface
    // Not pure black — #1A1A1A gives enough contrast for directional shadows
    bg: '#1A1A1A', surface: '#1A1A1A', surface2: '#161616',
    text: '#E8E8E8', sub: 'rgba(232,232,232,0.38)',
    border: 'rgba(255,255,255,0.05)', accent: '#A855F7',
    grad1: '#7C3AED', grad2: '#A855F7', nav: '#161616', card: '#1E0545',
    matte: true,
  },
};

// ─── ACCENT PRESETS — colour palettes applied on top of any theme ─────────────
// default/amoled themes keep their bg/surface — only accent colours change
// white/dark/glass themes also adapt their card gradient to the chosen accent
const ACCENT_PRESETS = {
  // Each preset includes tonal fields for White Magic theme:
  // bg_white/nav_white: the coloured canvas (like reference images)
  // surface_white: frosted white card floating on canvas
  // text_white/sub_white: dark tonal text readable on light card
  purple: { label:'Purple', swatch:'#6750A4', accent:'#B985FA', grad1:'#6750A4', grad2:'#9C68E8', card:'#6750A4',
    bg_white:'#ECEEF3', nav_white:'#ECEEF3', surface_white:'#ECEEF3',
    text_white:'#1A1D2E', sub_white:'rgba(26,29,46,0.45)' },
  blue:   { label:'Blue',   swatch:'#1D6AE5', accent:'#60A5FA', grad1:'#1D4ED8', grad2:'#3B82F6', card:'#1E3A8A',
    bg_white:'#DBEAFE', nav_white:'#D0E8FC', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#1E3A8A', sub_white:'rgba(30,58,138,0.55)' },
  teal:   { label:'Teal',   swatch:'#0D9488', accent:'#2DD4BF', grad1:'#0D9488', grad2:'#14B8A6', card:'#134E4A',
    bg_white:'#CCFBF1', nav_white:'#B2F5EA', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#134E4A', sub_white:'rgba(19,78,74,0.55)' },
  rose:   { label:'Rose',   swatch:'#BE185D', accent:'#FB7185', grad1:'#BE185D', grad2:'#F43F5E', card:'#881337',
    bg_white:'#FCE7F3', nav_white:'#FBD0E8', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#831843', sub_white:'rgba(131,24,67,0.55)' },
  orange: { label:'Orange', swatch:'#C2410C', accent:'#FB923C', grad1:'#C2410C', grad2:'#EA580C', card:'#7C2D12',
    bg_white:'#FED7AA', nav_white:'#FDBA74', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#7C2D12', sub_white:'rgba(124,45,18,0.55)' },
  green:  { label:'Green',  swatch:'#15803D', accent:'#4ADE80', grad1:'#15803D', grad2:'#16A34A', card:'#14532D',
    bg_white:'#DCFCE7', nav_white:'#BBF7D0', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#14532D', sub_white:'rgba(20,83,45,0.55)' },
  gold:   { label:'Gold',   swatch:'#B45309', accent:'#FBBF24', grad1:'#B45309', grad2:'#D97706', card:'#78350F',
    bg_white:'#FEF3C7', nav_white:'#FDE68A', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#78350F', sub_white:'rgba(120,53,15,0.55)' },
  pink:   { label:'Pink',   swatch:'#A21CAF', accent:'#E879F9', grad1:'#A21CAF', grad2:'#C026D3', card:'#701A75',
    bg_white:'#FAE8FF', nav_white:'#F0ABFC', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#701A75', sub_white:'rgba(112,26,117,0.55)' },
  indigo: { label:'Indigo', swatch:'#4338CA', accent:'#818CF8', grad1:'#4338CA', grad2:'#6366F1', card:'#312E81',
    bg_white:'#E0E7FF', nav_white:'#C7D2FE', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#312E81', sub_white:'rgba(49,46,129,0.55)' },
  red:    { label:'Red',    swatch:'#B91C1C', accent:'#F87171', grad1:'#B91C1C', grad2:'#DC2626', card:'#7F1D1D',
    bg_white:'#FEE2E2', nav_white:'#FECACA', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#7F1D1D', sub_white:'rgba(127,29,29,0.55)' },
};

// ─── TILE STYLE HELPER — adds depth shadow for all themes ────────────────────
// neuSurface: drop-in replacement for { backgroundColor: T.surface } on cards
function neuSurface(T, extra = {}) {
  if (T.glass) return { backgroundColor: T.surface, borderRadius: 18, ...extra };
  return tileStyle(T, extra); // matte, white, dark, amoled all route through tileStyle
}

function tileStyle(T, extra = {}) {
  // ── WHITE MAGIC — tonal neumorphism, colour adapts to accent preset ─────────
  if (T.white) return {
    // surface is now the tinted colour from accent preset
    // cards float as frosted rgba panels — not solid surface colour
    ...
    // Shadow uses sub colour which is now tinted — depth feels tonal
  ...
  // ── MATTE BLACK — dark neumorphism, raised tiles from matte surface ─────────
  if (T.matte) return {
    backgroundColor: '#1A1A1A', // matches bg exactly — key to neumorphic illusion
    borderRadius: 18,
    elevation: 6,
    // Top-left: subtle white edge — simulates light source from top-left
    borderTopWidth: 1.5,
    borderLeftWidth: 1.5,
    borderTopColor: 'rgba(255,255,255,0.08)',
    borderLeftColor: 'rgba(255,255,255,0.05)',
    // Bottom-right: deep black — shadow side of raised tile
    borderBottomWidth: 1.5,
    borderRightWidth: 1.5,
  ...
  // ── AMOLED MAGIC — dramatic dark depth ─────────────────────────────────────
  if (T.dark) return {
    backgroundColor: '#141414',
    borderRadius: 18,
    elevation: 8,
    // Top-left: barely visible light edge
    borderTopWidth: 1,
    borderLeftWidth: 1,
    borderTopColor: 'rgba(255,255,255,0.07)',
    borderLeftColor: 'rgba(255,255,255,0.05)',
    // Bottom-right: deep black — tiles sink into the dark
    borderBottomWidth: 1,
    borderRightWidth: 1,
  ...
  // ── DEFAULT DARK — subtle material depth ───────────────────────────────────
  if (!T.white && !T.dark && !T.glass && !T.amoled) return {
    backgroundColor: T.surface,
    borderRadius: 18,
    elevation: 3,
    // Very subtle directional borders — depth without being heavy
    borderTopWidth: 1,
    borderLeftWidth: 1,
  ...
  // ── AMOLED (pure black) — floating tiles in darkness ──────────────────────
  if (T.amoled) return {
    backgroundColor: T.surface,
    borderRadius: 18,
    elevation: 6,
  ...
  // ── GLASS — leave untouched, glass has its own border system ──────────────
...
const INIT = {
  ...
  accentKey: 'purple', voiceEnabled: false, voiceKeywords: [], voiceCorrections: [],
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_VOICE':      return { ...state, voiceEnabled: action.v };
    case 'ADD_VOICE_KW': {
      const existing = (state.voiceKeywords||[]).findIndex(k => k.englishTitle?.toLowerCase() === action.kw.englishTitle?.toLowerCase());
      if (existing >= 0) {
        const updated = [...(state.voiceKeywords||[])];
        const newVars = [...new Set([...(updated[existing].variations||[]), ...action.kw.variations])];
        updated[existing] = { ...updated[existing], variations: newVars };
        return { ...state, voiceKeywords: updated };
      }
      return { ...state, voiceKeywords: [...(state.voiceKeywords||[]), action.kw] };
    }
    case 'ADD_VOICE_CORR':  return { ...state, voiceCorrections: [...(state.voiceCorrections||[]), action.correction] };
    case 'DEL_VOICE_CORR':  return { ...state, voiceCorrections: (state.voiceCorrections||[]).filter((_,i) => i !== action.i) };
    case 'DEL_VOICE_KW':   return { ...state, voiceKeywords: (state.voiceKeywords||[]).filter((_,i) => i !== action.i) };
    ...
    case 'CLEAR_TODAY_INSIGHTS': return { ...state, savedInsights: (state.savedInsights||[]).filter(s => s.savedAt !== todayStr()) };
...
// ─── SLIDE SCREEN (Spaghettification from bubble origin) ─────────────────────
// originX, originY: screen coords of the element that opened this sheet.
// When provided, the sheet emerges from/collapses back to that point.
// Falls back gracefully to center-bottom if coords are 0 or missing.
function SlideScreen({ children, onClose, zIndex = 50, T, animLevel, originX, originY }) {
  // Duration multiplier: full=1.0, reduced=0.55 (snappier on low-end), none=0
  const DM = animLevel === 'reduced' ? 0.55 : 1.0;
  const screenCX = SW / 2;
  const screenCY = SH;

  // If valid origin provided, compute offset from screen centre-bottom
  const hasOrigin = originX > 0 && originY > 0;
  const offX = hasOrigin ? (originX - screenCX) : 0; // horizontal shift from centre
  const offY = hasOrigin ? (originY - screenCY) : 0; // vertical shift from bottom
  ...
  // translateX shifts start point to bubble's horizontal position
  const tx     = useRef(new Animated.Value(offX)).current;

  useEffect(() => {
    if (animLevel === 'none') {
      ty.setValue(0); tx.setValue(0); scaleX.setValue(1); scaleY.setValue(1); opac.setValue(1);
      return;
    }

    if (!hasOrigin) {
      // Standard genie from center bottom (existing behaviour for non-bubble sheets)
      Animated.parallel([
        ...
        Animated.sequence([
          ...
          Animated.spring(scaleX, animLevel==='reduced' ? SPRING_LITE : { toValue:1, tension:55, friction:9, useNativeDriver:true }),
        ]),
        Animated.sequence([
          Animated.timing(scaleY, { toValue:1.06, duration:Math.round(260*DM), easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.spring(scaleY, animLevel==='reduced' ? SPRING_LITE : { toValue:1, tension:65, friction:9, useNativeDriver:true }),
        ]),
      ]).start();
    } else {
      // ── SPAGHETTIFICATION from bubble ──────────────────────────────────────
      // Phase 1: bubble stretches — scaleX narrows (spaghetti), scaleY elongates
      //          tx homes in from bubble's X position to centre
      //          ty rises from bubble's Y to top
      Animated.parallel([
        ...
        Animated.sequence([
          ...
          Animated.spring(scaleX, animLevel==='reduced' ? SPRING_LITE : { toValue:1, tension:60, friction:9, useNativeDriver:true }),
        ]),
        Animated.sequence([
          Animated.timing(scaleY, { toValue:1.18, duration:Math.round(220*DM), easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.spring(scaleY, animLevel==='reduced' ? SPRING_LITE : { toValue:1, tension:70, friction:9, useNativeDriver:true }),
        ]),
      ]).start();
    }
  }, []);
  ...
  const genieClose = useCallback(() => {
    ...
    if (!hasOrigin) {
      // Standard genie close — exact reverse of open
      // Open: ty 0←SH, scaleX 0.04→1 (3 stages), scaleY 0.04→1.06→1
      // Close: ty 0→SH, scaleX 1→0.7→0.3→0.04, scaleY 1→1.06→0.04
      Animated.parallel([
        ...
        Animated.sequence([
          // scaleX narrows in reverse — wide → mid → thin
        ...
        Animated.sequence([
          // scaleY elongates first then collapses — reverse of open spring
      ...
      // ── SPAGHETTIFICATION back to bubble — exact reverse of open ─────────────
      // Open: tx offX→0, ty SH→0, scaleX 0.04→0.08→0.35→0.75→1, scaleY 0.04→1.18→1
      // Close: tx 0→offX, ty 0→SH, scaleX 1→0.75→0.35→0.04, scaleY 1→1.18→0.04
      Animated.parallel([
        Animated.timing(opac, { toValue:0, duration:Math.round(360*DM), delay:Math.round(40*DM), useNativeDriver:true }),
        // tx shifts back to bubble's horizontal position
        Animated.timing(tx, { toValue:offX, duration:Math.round(460*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        // ty drops back to where the bubble was
        Animated.timing(ty, { toValue:SH, duration:Math.round(460*DM), easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        Animated.sequence([
          // scaleX reverses its 3-stage widen — narrows back to a stream
        ...
        Animated.sequence([
          // scaleY elongates first (spaghetti stretch) then collapses
  ...
  }, [onClose, animLevel, hasOrigin, offX, offY]);
...
// ─── SPAGHETTIFY ROW — smooth row-level deletion animation ───────────────────
function SpaghettifyRow({ children, onDelete, animLevel, T }) {
  const progress  = useRef(new Animated.Value(0)).current; // 0=normal, 1=gone
  const rowHeight = useRef(new Animated.Value(1)).current; // 1=full, 0=collapsed
  const [deleting, setDeleting] = useState(false);

  const handleDel = () => {
    if (deleting) return;
    setDeleting(true);
    if (animLevel === 'none') { onDelete(); return; }

    const dur = animLevel === 'reduced' ? 280 : 500;

    // Single smooth animation on progress 0→1
    // All transforms derived from this one value — no jank
    Animated.timing(progress, {
      ...
      // After row disappears, collapse its height so gap closes smoothly
  ...
  // Derive all visual properties from single progress value
  // scaleX: 1 → 0.01 (spaghetti stream)
  const scaleX = progress.interpolate({ inputRange:[0,1], outputRange:[1, 0.01] });
  // scaleY: 1 → 2.2 (elongation — spaghettification)
  const scaleY = progress.interpolate({ inputRange:[0,0.3,1], outputRange:[1, 1.15, 2.2] });
  // translateY: 0 → -40 (flows upward toward singularity)
  const translateY = progress.interpolate({ inputRange:[0,1], outputRange:[0, -40] });
  // opacity: stays full until 60% through then fades
  const opacity = progress.interpolate({ inputRange:[0,0.6,1], outputRange:[1, 0.9, 0] });

  return (
    <Animated.View style={{
      // Single Animated.View — all transforms in one place, no nesting issues
      transform:[{ scaleX }, { scaleY }, { translateY }],
      opacity,
    }}>
      {children(handleDel)}
      {/* Height spacer — collapses after animation to close gap */}
      <Animated.View style={{
        height: rowHeight.interpolate({ inputRange:[0,1], outputRange:[0,71], extrapolate:'clamp' }),
      }} />
    </Animated.View>
  );
}
...
function SingularityDelete({ onDone, animLevel }) {
  ...
  useEffect(() => {
    if (animLevel === 'reduced') {
      // Reduced: simple fast implode — no glow phase
      Animated.parallel([
        Animated.timing(implode,  { toValue: 0, duration: 280, easing: Easing.in(Easing.cubic), useNativeDriver: true }),
        Animated.timing(scale,    { toValue: 0, duration: 280, easing: Easing.in(Easing.cubic), useNativeDriver: true }),
...
function Row({ expense, onPress, T, delay = 0, animLevel }) {
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      Animated.timing(tx, { toValue: 0, duration: animLevel === 'reduced' ? 200 : 380, delay: animLevel==='reduced'?delay*0.6:delay, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.timing(op, { toValue: 1, duration: animLevel === 'reduced' ? 160 : 320, delay: animLevel==='reduced'?delay*0.6:delay, useNativeDriver: true }),
    ]).start();
  }, []);
  return (
    <TouchableOpacity onPress={() => onPress(expense)}
      ...
      <Animated.View style={[styles.row, { backgroundColor: T.surface, transform: [{ scale }, { translateX: tx }], opacity: op }]}>
        <View style={[styles.rowIcon, T.white ? { backgroundColor:T.surface, borderTopWidth:1, borderLeftWidth:1, borderTopColor:'rgba(255,255,255,0.92)', borderLeftColor:'rgba(255,255,255,0.82)', borderBottomWidth:1, borderRightWidth:1, borderBottomColor:'rgba(0,0,0,0.10)', borderRightColor:'rgba(0,0,0,0.07)', elevation:3 } : T.matte ? { backgroundColor:'#1A1A1A', borderTopWidth:1, borderLeftWidth:1, borderTopColor:'rgba(255,255,255,0.08)', borderLeftColor:'rgba(255,255,255,0.05)', borderBottomWidth:1, borderRightWidth:1, borderBottomColor:'rgba(0,0,0,0.85)', borderRightColor:'rgba(0,0,0,0.7)', elevation:4 } : { backgroundColor: cat.color + '28' }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection:'row', alignItems:'center', gap:6, flexWrap:'wrap' }}>
            ...
            {expense.source === 'voice' && <Text style={{ fontSize:9, fontWeight:'800', color:'#B985FA', backgroundColor:'rgba(185,133,250,0.12)', borderRadius:4, paddingHorizontal:4, paddingVertical:1 }}>🎙️ VOICE</Text>}
            {expense.source === 'auto'  && <Text style={{ fontSize:9, fontWeight:'800', color:'#34D399', backgroundColor:'rgba(52,211,153,0.12)', borderRadius:4, paddingHorizontal:4, paddingVertical:1 }}>✓ AUTO</Text>}
          </View>
        </View>
    ...
    <View style={{ width: SW, height: fieldH, position: 'relative' }}>
      {cats.map((cat, i) => (
        <Animated.View key={cat.name} ref={refs[i].wrap}
          ...
          <TouchableOpacity onPress={() => onSelect(i, refs[i].wrap)} activeOpacity={0.9}
...
// ─── LIQUID BUDGET BAR ────────────────────────────────────────────────────────
function LiquidBudgetBar({ pct, animLevel }) { // animLevel already a prop
  ...
  <View style={{ height: 18, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 99, overflow: 'hidden', marginTop: 12, marginBottom: 4 }}>
      ...
      {/* Lottie wave overlay — only on full animations */}
      {animLevel !== 'reduced' && (
...
// ─── HOME SCREEN ─────────────────────────────────────────────────────────────
function HomeScreen({ state, dispatch, onRow, onEdit, onQuickAdd, onDelete, T, animLevel, collapsed, setCollapsed, onSaveInsights, breathAnim, liveInsights, onVoiceTap }) {
  ...
  const totalIncome      = (state.featEntries?.income||[]).filter(e => monthOf(e.date) === curMonth()).reduce((a,e)=>a+(e.amount||0),0);
  const totalMonthlyEMI  = (state.featEntries?.emi||[]).reduce((a,e)=>a+(e.amount||0),0);
  const disposable       = totalIncome > 0 ? totalIncome - totalMonthlyEMI : state.budget - totalMonthlyEMI;
  const monthlySavings   = totalIncome > 0 ? Math.max(totalIncome - spent - totalMonthlyEMI, 0) : Math.max(state.budget - spent, 0);
  const allGoals         = (state.featEntries?.goals||[]);
  const activeGoals      = allGoals.filter(g => { const saved=(g.contributions||[]).reduce((a,c)=>a+c.amount,0); return saved < (g.targetAmount||g.amount||0); });
  const totalGoalSaved   = allGoals.reduce((a,g)=>a+(g.contributions||[]).reduce((s,c)=>s+c.amount,0),0);
  const totalInvest      = (state.featEntries?.investments||[]).reduce((a,e)=>a+(e.amount||0),0);
  const invest80C        = (state.featEntries?.investments||[]).filter(e=>e.subType==='80C Investment').reduce((a,e)=>a+(e.amount||0),0);
  const homeLoanInterest = (state.featEntries?.emi||[]).filter(e=>e.subType==='Home Loan').reduce((a,e)=>a+(e.amount||0),0);
  const taxEntries       = state.featEntries?.tax||[];
  const tax80C           = taxEntries.filter(e=>e.subType==='80C Investment').reduce((a,e)=>a+(e.amount||0),0) + invest80C;
  const taxHRA           = taxEntries.filter(e=>e.subType==='HRA').reduce((a,e)=>a+(e.amount||0),0);
  const tax80D           = taxEntries.filter(e=>e.subType==='Medical Insurance'||e.subType==='80D').reduce((a,e)=>a+(e.amount||0),0);
  const totalDeductions  = tax80C + taxHRA + tax80D + homeLoanInterest;
  const monthlyRecSubs   = (state.recurring||[]).filter(r=>r.category==='Subscriptions').reduce((a,r)=>a+(r.amount||0),0);
  const expenseSubs      = mEx.filter(e=>e.category==='Subscriptions').reduce((a,e)=>a+(e.amount||0),0);
  const totalSubs        = monthlyRecSubs || expenseSubs;
  // Tile visibility — only show tile if data exists
  const showIncomeTile   = totalIncome > 0;
  const showGoalsTile    = activeGoals.length > 0;
  const showTaxTile      = totalDeductions > 0;
  const showEMITile      = totalMonthlyEMI > 0;
  const showSubsTile     = totalSubs > 0;
  ...
  // ── SMART INSIGHT ENGINE — receives live insights from App ────────────────
  // insights prop passed from App level so they stay live on all tabs
  const insights = liveInsights || [];
  const _unused_insights_memo = React.useMemo(() => {
    const result = [];
    const now    = new Date();
    ...
    // ── 21. FINANCIAL DNA PROFILE ─────────────────────────────────────────────
    // Build a personality label from actual behaviour patterns
    if (state.expenses.length >= 15) {
      const traits = [];
      // Weekend warrior
      if (wkendAvg > wkdayAvg * 1.5) traits.push('Weekend Warrior 🎭');
      // Subscription hoarder — if subscriptions > 20% of total
      const subAmt = mEx.filter(e=>e.category==='Subscriptions').reduce((a,e)=>a+e.amount,0);
      if (mTotal > 0 && subAmt/mTotal > 0.2) traits.push('Subscription Hoarder 📱');
      // Disciplined saver
      if (savingsRate !== null && savingsRate >= 30) traits.push('Disciplined Saver 💎');
      // Impulse buyer — many small transactions
      const avgTxn = mTotal / Math.max(mEx.length, 1);
      if (mEx.length >= 20 && avgTxn < 400) traits.push('Impulse Buyer ⚡');
      // Food lover
      const foodAmt = mEx.filter(e=>e.category==='Food'||e.category==='Dining').reduce((a,e)=>a+e.amount,0);
      if (mTotal > 0 && foodAmt/mTotal > 0.3) traits.push('Foodie 🍔');
      // Travel enthusiast
      const travelAmt = mEx.filter(e=>e.category==='Travel').reduce((a,e)=>a+e.amount,0);
      if (travelAmt > 5000) traits.push('Explorer ✈️');
      // Consistent logger
      if (streak >= 10) traits.push('Habit Builder 🔥');
      // Goal achiever
      if (achievedGoals.length > 0) traits.push('Goal Crusher 🎯');
      // Default
      if (traits.length === 0) traits.push('Balanced Spender ⚖️');
      const primary = traits[0];
      const extra = traits.length > 1 ? ' · ' + traits.slice(1,3).join(' · ') : '';
      result.push({ icon:'🧬', title:'Your Financial DNA', text:'You are a ' + primary + extra + '. Based on ' + state.expenses.length + ' transactions across your history.', color:'#7C3AED', priority:1, badge:'DNA' });
    }

    // ── 22. SAVINGS SCORE (0-100) ─────────────────────────────────────────────
    if (state.expenses.length >= 10) {
      let score = 50; // base
      // Budget adherence (+/-20)
      if (state.budget > 0) {
        const adherence = mTotal / state.budget;
        if (adherence <= 0.7)  score += 20;
        else if (adherence <= 0.9) score += 12;
        else if (adherence <= 1.0) score += 5;
        else if (adherence <= 1.2) score -= 10;
        else score -= 20;
      }
      // Savings rate (+/-15)
      if (savingsRate !== null) {
      ...
      // Goal progress (+10)
      if (nearGoals.length > 0 || achievedGoals.length > 0) score += 10;
      // MoM improvement (+5)
      if (lmTotal > 0 && mTotal < lmTotal) score += 5;
      score = Math.max(10, Math.min(100, score));
      const grade = score >= 85 ? 'Excellent 🏆' : score >= 70 ? 'Good 👍' : score >= 55 ? 'Average 📊' : score >= 40 ? 'Needs work ⚠️' : 'Critical 🚨';
      const trend = lmTotal > 0 ? (mTotal < lmTotal ? ' ↑ improving' : ' ↓ declining') : '';
      result.push({ icon:'⭐', title:'Savings Score: ' + score + '/100', text:grade + trend + '. Score factors in budget discipline, savings rate, logging consistency and goal progress.', color: score >= 70 ? '#2ECC8A' : score >= 50 ? '#FF9800' : '#FF3B30', priority:1, badge:'SCORE' });
    }

    // ── 23. ₹1 LAKH CHALLENGE ────────────────────────────────────────────────
    if (state.expenses.length >= 10 && lmTotal > 0) {
      const cuts = [];
      // Find categories where this month > last month by significant amount
      Object.entries(catTotals).forEach(([cat, amt]) => {
        const lm = catLMTotals[cat] || 0;
        if (amt > lm + 300 && lm > 0) cuts.push({ cat, save: Math.round((amt-lm)*0.6) });
      });
      cuts.sort((a,b) => b.save-a.save);
      const totalAnnual = cuts.slice(0,3).reduce((a,c)=>a+c.save*12,0);
      if (totalAnnual >= 12000 && cuts.length >= 2) {
        const cutList = cuts.slice(0,2).map(c => 'cut ' + c.cat + ' by ' + fmt(c.save) + '/mo').join(' + ');
        result.push({ icon:'💰', title:'Save ₹' + Math.round(totalAnnual/1000) + 'k this year', text:'If you ' + cutList + ', you will save ' + fmt(totalAnnual) + ' over 12 months. Small consistent cuts compound dramatically.', color:'#2ECC8A', priority:2, badge:'CHALLENGE' });
      }
    }

    // ── 24. LIFESTYLE INFLATION DETECTOR ─────────────────────────────────────
    const m5Key = (() => { const d=new Date(now); d.setMonth(d.getMonth()-5); return d.toISOString().slice(0,7); })();
    const m6Key = (() => { const d=new Date(now); d.setMonth(d.getMonth()-6); return d.toISOString().slice(0,7); })();
    const oldEx = state.expenses.filter(e => [m5Key,m6Key].includes(monthOf(e.date)));
    if (oldEx.length >= 5 && lmTotal > 0) {
      const oldAvg = oldEx.reduce((a,e)=>a+e.amount,0) / 2;
      const newAvg = (mTotal + lmTotal) / 2;
      const inflation = Math.round(((newAvg-oldAvg)/Math.max(oldAvg,1))*100);
      if (inflation >= 20) {
        const monthly = Math.round(newAvg - oldAvg);
        // Find biggest driver
        const oldCats = {}; oldEx.forEach(e=>{oldCats[e.category]=(oldCats[e.category]||0)+e.amount/2;});
        const drivers = Object.entries(catTotals).map(([cat,amt])=>({ cat, rise: amt-(oldCats[cat]||0) })).filter(x=>x.rise>200).sort((a,b)=>b.rise-a.rise);
        const driver = drivers[0] ? ' Biggest driver: ' + drivers[0].cat + ' (+' + fmt(Math.round(drivers[0].rise)) + '/mo).' : '';
        result.push({ icon:'📈', title:'Lifestyle inflation +' + inflation + '%', text:'Your monthly spend is ' + fmt(monthly) + ' more than 6 months ago.' + driver + ' This compounds to ' + fmt(monthly*12) + ' extra per year.', color:'#FF6B6B', priority:2 });
      }
    }

    // ── 25. SPENDING TRIGGERS ─────────────────────────────────────────────────
    // Find which day of week consistently has highest spending
    if (state.expenses.length >= 20) {
      ...
      const overallAvg = dayAvgs.filter(a=>a>0).reduce((a,b)=>a+b,0) / dayAvgs.filter(a=>a>0).length;
      const peakDay = dayAvgs.indexOf(Math.max(...dayAvgs));
      const peakAvg = dayAvgs[peakDay];
      const DNAMES = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
      if (peakAvg > overallAvg * 1.5 && dayCounts[peakDay] >= 4) {
        result.push({ icon:'🎯', title:DNAMES[peakDay] + ' spending trigger', text:'You spend ' + fmt(Math.round(peakAvg)) + ' on average every ' + DNAMES[peakDay] + ' — ' + Math.round((peakAvg/overallAvg-1)*100) + '% above your daily average. Awareness is the first step to change.', color:'#9C27B0', priority:3 });
      }
    }

    // ── 26. TRUE COST CALCULATOR ─────────────────────────────────────────────
    // Find the most expensive recurring pattern and show 5-year cost
    const recurringPatterns = {};
    state.expenses.forEach(e => {
      const key = e.title.toLowerCase().trim();
      if (!recurringPatterns[key]) recurringPatterns[key] = { total:0, count:0, amt:0 };
      recurringPatterns[key].total += e.amount;
      recurringPatterns[key].count++;
      recurringPatterns[key].amt = e.amount;
    });
    const trueCostCandidates = Object.entries(recurringPatterns)
      .filter(([,v]) => v.count >= 3 && v.amt >= 100)
      .sort((a,b) => b[1].total - a[1].total);
    if (trueCostCandidates.length > 0) {
      const [name, data] = trueCostCandidates[0];
      const monthly = Math.round(data.total / Math.max(data.count, 1));
      const fiveYear = monthly * 12 * 5;
      const displayName = name.charAt(0).toUpperCase() + name.slice(1);
      result.push({ icon:'🔍', title:'True cost: ' + displayName, text:'You spend ~' + fmt(monthly) + '/month on ' + displayName + '. Over 5 years that is ' + fmt(fiveYear) + '. Is it worth it to you?', color:'#FF9800', priority:4 });
    }

    // ── 27. PEER BENCHMARKING ─────────────────────────────────────────────────
    // Compare against realistic Indian spending averages by category
    const BENCHMARKS = { Food:0.15, Transport:0.10, Shopping:0.08, Entertainment:0.06, Subscriptions:0.08, Utilities:0.07, Health:0.05, Groceries:0.12 };
    if (mTotal > 5000) {
      const benchInsights = [];
      Object.entries(BENCHMARKS).forEach(([cat, benchmark]) => {
        const catAmt = mEx.filter(e=>e.category===cat).reduce((a,e)=>a+e.amount,0);
        const catPct = catAmt / mTotal;
        if (catAmt > 500 && catPct > benchmark * 1.5) {
          benchInsights.push({ cat, catPct: Math.round(catPct*100), avg: Math.round(benchmark*100), excess: fmt(Math.round(catAmt - mTotal*benchmark)) });
        }
      });
      if (benchInsights.length > 0) {
        const b = benchInsights.sort((a,b)=>(b.catPct-b.avg)-(a.catPct-a.avg))[0];
        result.push({ icon:'👥', title:b.cat + ' vs average', text:'You spend ' + b.catPct + '% of your budget on ' + b.cat + '. Most people at your level spend ' + b.avg + '%. That is ' + b.excess + ' above average per month.', color:'#3F51B5', priority:3 });
      }
    }

    // ── 28. MONEY LEAK REPORT ─────────────────────────────────────────────────
    // Find small recurring expenses that add up unnoticed
    if (state.expenses.length >= 15) {
      const smallRecurring = Object.entries(recurringPatterns)
        .filter(([,v]) => v.count >= 2 && v.amt >= 50 && v.amt <= 500)
        .sort((a,b) => b[1].total - a[1].total)
        .slice(0, 5);
      if (smallRecurring.length >= 3) {
        const leakTotal = smallRecurring.reduce((a,[,v])=>a+v.amt,0);
        const leakAnnual = leakTotal * 12;
        const leakNames = smallRecurring.slice(0,3).map(([n])=>n.charAt(0).toUpperCase()+n.slice(1)).join(', ');
        result.push({ icon:'🚰', title:'Money leak: ' + fmt(leakTotal) + '/mo', text:leakNames + ' and similar small spends add up to ' + fmt(leakAnnual) + '/year. You probably never think about them.', color:'#FF6B6B', priority:3 });
      }
    }

    // ── 29. MOOD-AWARE MESSAGING ──────────────────────────────────────────────
    // Tone adapts to financial health — same data, different emotional register
    if (state.budget > 0 && dayOfMonth >= 10) {
      const healthPct = mTotal / state.budget;
      if (healthPct < 0.5 && savingsRate !== null && savingsRate >= 20) {
        result.push({ icon:'🌟', title:'You are crushing it', text:'Halfway through the month and only ' + Math.round(healthPct*100) + '% of budget used with a ' + savingsRate + '% savings rate. This is genuinely impressive financial discipline.', color:'#2ECC8A', priority:2 });
      } else if (healthPct > 0.95 && dayOfMonth < 20) {
        result.push({ icon:'🆘', title:'Budget emergency', text:'You have used ' + Math.round(healthPct*100) + '% of your budget but the month is only ' + Math.round(dayOfMonth/daysInMonth*100) + '% done. Freeze non-essential spending now.', color:'#FF3B30', priority:1 });
      } else if (healthPct > 0.80 && dayOfMonth < 25) {
        result.push({ icon:'😬', title:'Watch your spending', text:Math.round(healthPct*100) + '% of budget used with ' + (daysInMonth-dayOfMonth) + ' days left. You can still finish the month well — just be mindful of every purchase.', color:'#FF9800', priority:2 });
      }
    }

    // ── 30. GOAL INTELLIGENCE ─────────────────────────────────────────────────
    // Smart projection based on actual contribution rate
    const allGoals = state.featEntries?.goals || [];
    allGoals.forEach(g => {
      const saved = (g.contributions||[]).reduce((a,c)=>a+c.amount,0);
      const tgt = g.targetAmount||g.amount||0;
      if (tgt <= 0 || saved >= tgt) return;
      const remaining = tgt - saved;
      // Monthly contribution rate
      const contribByMonth = {};
      (g.contributions||[]).forEach(c => { const mk=monthOf(c.date); contribByMonth[mk]=(contribByMonth[mk]||0)+c.amount; });
      const monthlyRates = Object.values(contribByMonth);
      if (monthlyRates.length >= 2) {
        const avgMonthly = monthlyRates.reduce((a,b)=>a+b,0)/monthlyRates.length;
        if (avgMonthly > 0) {
          const monthsLeft = Math.ceil(remaining/avgMonthly);
          const dueDate = new Date(now); dueDate.setMonth(dueDate.getMonth()+monthsLeft);
          const dueStr = dueDate.toLocaleString('en-IN',{month:'long',year:'numeric'});
          result.push({ icon:'🎯', title:'"' + g.title + '" projection', text:'At your current rate of ' + fmt(Math.round(avgMonthly)) + '/month you will reach this goal by ' + dueStr + '. ' + (monthsLeft <= 3 ? 'Almost there!' : 'Stay consistent.'), color:'#E91E63', priority:2 });
        }
      }
    });

    // Sort — prioritise mind-blowing insights first, cap at 10
    const sorted = result.sort((a,b) => a.priority - b.priority).slice(0,10);
    return sorted;
  }, []); // disabled — engine runs at App level now

  useEffect(() => {
    if (insights.length > 0 && onSaveInsights) onSaveInsights(insights);
  }, [insights.length]);

  const handleBbl = (i, bubbleRef) => {
    if (collapsed) { setCollapsed(false); return; }
    if (selBbl === i) { setSelBbl(-1); return; }
    // Capture bubble's screen position for spaghetti origin
    if (bubbleRef && bubbleRef.current && bubbleRef.current.measure) {
      bubbleRef.current.measure((fx, fy, w, h, px, py) => {
        // px, py are screen coords of bubble top-left
        const bCX = px + w / 2;
        const bCY = py + h / 2;
        setSheetOrigin({ x: bCX > 0 ? bCX : 0, y: bCY > 0 ? bCY : 0 });
        setSelBbl(i);
        setSheetCat(CATS[i]);
      });
    } else {
      setSheetOrigin({ x: 0, y: 0 });
      setSelBbl(i);
      setSheetCat(CATS[i]);
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={T.glass ? {
            ...
            <View style={{ position:'relative', zIndex:1, padding: (T.glass || T.white || T.dark) ? 20 : 0 }}>
              <Text style={[styles.cardLabel,
                ...
                T.white && { color: T.sub, fontSize:10, letterSpacing:0.5 },
                T.dark  && { color:'rgba(255,255,255,0.65)', fontSize:10, letterSpacing:0.5 },
              ]}>
        ...
        {/* ── DASHBOARD TILES — slidable, only show if data exists ── */}
        {(showIncomeTile || showGoalsTile || showTaxTile || showEMITile || showSubsTile) && (
          <Animated.View style={[{ marginBottom: 4 }, aS(cardAnim)]}>
            <ScrollView
              horizontal
            ...
            >
              {/* OVERVIEW TILE — always shown in carousel */}
              <View style={{ width: SW - 52, borderRadius:20, padding:16,
                backgroundColor: T.surface, borderWidth:1, borderColor: T.accent+'22',
                overflow:'hidden' }}>
                <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                  borderRadius:50, backgroundColor: T.accent+'10' }} pointerEvents="none" />
                <Text style={{ fontSize:9, fontWeight:'700', color:T.accent+'80', letterSpacing:0.8, marginBottom:6 }}>OVERVIEW</Text>
                <Text style={{ fontSize:26, fontWeight:'800', color:T.text }}>{fmt(spent)}</Text>
                <Text style={{ fontSize:10, color:T.sub, marginBottom:10 }}>spent this month</Text>
                {totalIncome > 0 && (
                  <View style={{ flexDirection:'row', gap:6, marginBottom:10 }}>
                    <View style={{ flex:1, backgroundColor:T.surface2, borderRadius:10, padding:8 }}>
                      <Text style={{ fontSize:11, fontWeight:'800', color:'#34D399' }}>{fmt(totalIncome)}</Text>
                      <Text style={{ fontSize:8, color:T.sub, marginTop:1 }}>INCOME</Text>
                    </View>
                    {totalMonthlyEMI > 0 && (
                      <View style={{ flex:1, backgroundColor:T.surface2, borderRadius:10, padding:8 }}>
                        <Text style={{ fontSize:11, fontWeight:'800', color:'#FF6B6B' }}>{fmt(totalMonthlyEMI)}</Text>
                        <Text style={{ fontSize:8, color:T.sub, marginTop:1 }}>EMI</Text>
                      </View>
                    )}
                    <View style={{ flex:1, backgroundColor:T.surface2, borderRadius:10, padding:8 }}>
                      <Text style={{ fontSize:11, fontWeight:'800', color:T.accent }}>{fmt(Math.max(disposable - spent, 0))}</Text>
                      <Text style={{ fontSize:8, color:T.sub, marginTop:1 }}>FREE</Text>
                    </View>
                  </View>
                )}
                <View style={{ height:4, borderRadius:2, backgroundColor:T.surface2, overflow:'hidden' }}>
                  <View style={{ height:4, borderRadius:2, width:pct+'%',
                    backgroundColor: pct >= 100 ? '#FF3B30' : pct >= 80 ? '#FF9800' : T.accent }} />
                </View>
                <Text style={{ fontSize:9, color:T.sub, marginTop:4 }}>{pct}% of ₹{state.budget.toLocaleString('en-IN')} budget</Text>
              </View>

              {/* INCOME TILE */}
              {showIncomeTile && (
                <View style={{ width: SW - 72, borderRadius:20, padding:16,
                  backgroundColor:'rgba(13,40,24,0.95)', borderWidth:1, borderColor:'rgba(52,211,153,0.2)',
                  overflow:'hidden' }}>
                  <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                    borderRadius:50, backgroundColor:'rgba(52,211,153,0.08)' }} pointerEvents="none" />
                  <Text style={{ fontSize:9, fontWeight:'700', color:'rgba(52,211,153,0.6)', letterSpacing:0.8, marginBottom:4 }}>INCOME</Text>
                  <Text style={{ fontSize:22, fontWeight:'800', color:'#34D399' }}>{fmt(totalIncome)}</Text>
                  <Text style={{ fontSize:10, color:'rgba(52,211,153,0.5)', marginBottom:8 }}>this month</Text>
                  {monthlySavings > 0 && (
                    <View style={{ backgroundColor:'rgba(52,211,153,0.1)', borderRadius:10, padding:8 }}>
                      <Text style={{ fontSize:11, color:'#34D399', fontWeight:'700' }}>
                        {'Saving ' + fmt(monthlySavings) + '/mo'}
                      </Text>
                      <Text style={{ fontSize:9, color:'rgba(52,211,153,0.5)', marginTop:2 }}>
                        {totalIncome > 0 ? Math.round((monthlySavings/totalIncome)*100) + '% of income' : 'under budget'}
                      </Text>
                    </View>
                  )}
                </View>
              ...
              {showGoalsTile && (
                <View style={{ width: SW - 52, borderRadius:20, padding:16,
                  backgroundColor:'rgba(26,13,46,0.95)', borderWidth:1, borderColor: T.accent+'28',
                  overflow:'hidden' }}>
                  <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                    borderRadius:50, backgroundColor: T.accent+'08' }} pointerEvents="none" />
                  <Text style={{ fontSize:9, fontWeight:'700', color:T.accent+'70', letterSpacing:0.8, marginBottom:4 }}>
                    {activeGoals.length} ACTIVE {activeGoals.length === 1 ? 'GOAL' : 'GOALS'}
                  </Text>
                  <Text style={{ fontSize:22, fontWeight:'800', color:T.accent }}>{fmt(totalGoalSaved)}</Text>
                  <Text style={{ fontSize:10, color:T.sub, marginBottom:10 }}>saved across all goals</Text>
                  {activeGoals.slice(0,2).map((g,i) => {
                    const saved = (g.contributions||[]).reduce((a,cc)=>a+cc.amount,0);
                    const tgt   = g.targetAmount||g.amount||0;
                    const pctG  = tgt > 0 ? Math.min(Math.round((saved/tgt)*100),100) : 0;
                    const monthsLeft = monthlySavings > 0 && tgt > saved ? Math.ceil((tgt-saved)/monthlySavings) : null;
                    return (
                      <View key={g.id||i} style={{ marginBottom:8 }}>
                        <View style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:3 }}>
                          <Text style={{ fontSize:11, fontWeight:'700', color:T.text }}>{g.title||g.name||'Goal'}</Text>
                          <Text style={{ fontSize:10, fontWeight:'800', color:T.accent }}>{pctG}%</Text>
                        </View>
                        <View style={{ height:3, borderRadius:2, backgroundColor:T.surface2 }}>
                          <View style={{ height:3, borderRadius:2, width:pctG+'%', backgroundColor:T.accent }} />
                        </View>
                        {monthsLeft && <Text style={{ fontSize:9, color:T.sub, marginTop:2 }}>{monthsLeft} months at current savings</Text>}
                      </View>
                    );
                  })}
                </View>
              ...
              {showEMITile && (
                <View style={{ width: SW - 72, borderRadius:20, padding:16,
                  backgroundColor:'rgba(31,13,13,0.95)', borderWidth:1, borderColor:'rgba(255,152,0,0.2)',
                  overflow:'hidden' }}>
                  <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                    borderRadius:50, backgroundColor:'rgba(255,152,0,0.06)' }} pointerEvents="none" />
                  <Text style={{ fontSize:9, fontWeight:'700', color:'rgba(255,152,0,0.6)', letterSpacing:0.8, marginBottom:4 }}>EMI</Text>
                  <Text style={{ fontSize:22, fontWeight:'800', color:'#FF9800' }}>{fmt(totalMonthlyEMI)}</Text>
                  <Text style={{ fontSize:10, color:'rgba(255,152,0,0.5)', marginBottom:8 }}>monthly burden</Text>
                  {totalIncome > 0 && (
                    <Text style={{ fontSize:10, color:'rgba(255,152,0,0.6)', marginBottom:8 }}>
                      {Math.round((totalMonthlyEMI/totalIncome)*100)}% of income · real disposable: {fmt(disposable)}
                    </Text>
                  )}
                  {(state.featEntries?.emi||[]).slice(0,2).map((e,i) => (
                    <View key={i} style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:4 }}>
                      <Text style={{ fontSize:11, color:'rgba(230,225,229,0.6)' }}>{e.subType||e.title}</Text>
                      <Text style={{ fontSize:11, fontWeight:'700', color:'#FF9800' }}>{fmt(e.amount)}</Text>
                    </View>
                  ))}
                </View>
              )}
              ...
              {showTaxTile && (
                <View style={{ width: SW - 72, borderRadius:20, padding:16,
                  backgroundColor:'rgba(13,31,13,0.95)', borderWidth:1, borderColor:'rgba(139,195,74,0.2)',
                  overflow:'hidden' }}>
                  <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                    borderRadius:50, backgroundColor:'rgba(139,195,74,0.06)' }} pointerEvents="none" />
                  <Text style={{ fontSize:9, fontWeight:'700', color:'rgba(139,195,74,0.6)', letterSpacing:0.8, marginBottom:4 }}>TAX</Text>
                  <Text style={{ fontSize:22, fontWeight:'800', color:'#8BC34A' }}>{fmt(totalDeductions)}</Text>
                  <Text style={{ fontSize:10, color:'rgba(139,195,74,0.5)', marginBottom:8 }}>
                    {'total deductions · ~' + fmt(Math.round(totalDeductions*0.3)) + ' saved'}
                  </Text>
                  {tax80C > 0 && (
                    <View style={{ marginBottom:6 }}>
                      <View style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:3 }}>
                        <Text style={{ fontSize:10, color:'rgba(230,225,229,0.6)' }}>80C</Text>
                        <Text style={{ fontSize:10, fontWeight:'700', color:'#8BC34A' }}>{fmt(tax80C)} / ₹1.5L</Text>
                      </View>
                      <View style={{ height:3, borderRadius:2, backgroundColor:'rgba(255,255,255,0.08)' }}>
                        <View style={{ height:3, borderRadius:2, backgroundColor:'#8BC34A',
                          width:Math.min(Math.round((tax80C/150000)*100),100)+'%' }} />
                      </View>
                    </View>
                  )}
                  {invest80C > 0 && <Text style={{ fontSize:9, color:'rgba(139,195,74,0.5)' }}>✓ Investments auto-included in 80C</Text>}
                  {homeLoanInterest > 0 && <Text style={{ fontSize:9, color:'rgba(139,195,74,0.5)' }}>✓ Home loan interest included (Sec 24)</Text>}
                </View>
              )}
              ...
              {showSubsTile && (
                <View style={{ width: SW - 72, borderRadius:20, padding:16,
                  backgroundColor:'rgba(13,26,46,0.95)', borderWidth:1, borderColor:'rgba(33,150,243,0.2)',
                  overflow:'hidden' }}>
                  <View style={{ position:'absolute', top:-30, right:-30, width:100, height:100,
                    borderRadius:50, backgroundColor:'rgba(33,150,243,0.06)' }} pointerEvents="none" />
                  <Text style={{ fontSize:9, fontWeight:'700', color:'rgba(33,150,243,0.6)', letterSpacing:0.8, marginBottom:4 }}>SUBSCRIPTIONS</Text>
                  <Text style={{ fontSize:22, fontWeight:'800', color:'#42A5F5' }}>{fmt(totalSubs)}</Text>
                  <Text style={{ fontSize:10, color:'rgba(33,150,243,0.5)', marginBottom:8 }}>
                    {'per month · ' + fmt(totalSubs*12) + '/year'}
                  </Text>
                  {totalIncome > 0 && (
                    <Text style={{ fontSize:10, color:'rgba(33,150,243,0.6)' }}>
                      {Math.round((totalSubs/totalIncome)*100)}% of monthly income on subscriptions
                    </Text>
                  )}
                </View>
              )}
            ...
            <View style={{ flexDirection:'row', justifyContent:'center', gap:5 }}>
              {[true, showIncomeTile, showGoalsTile, showEMITile, showTaxTile, showSubsTile]
                .filter(Boolean).map((_, i) => (
                <View key={i} style={{ width: i===0 ? 16 : 5, height:5, borderRadius:3,
                  backgroundColor: i===0 ? T.accent : T.surface2 }} />
              ))}
            </View>
          </Animated.View>
        )}
        ...
        {top.length > 0 && (
          <Animated.View style={[{ flexDirection: 'row', gap: 8, marginHorizontal: 16, marginBottom: 8 }, aS(catsAnim)]}>
            {top.map(([name, amt]) => {
              ...
              return (
                <View key={name} style={[{ flex: 1, borderRadius: 14, padding: 10 }, neuSurface(T, { borderRadius: 14 })]}>
                ...
                {insights.map((ins, i) => (
                  <View key={i} style={{
                    ...
                    <View style={{ flexDirection:'row', alignItems:'center', gap:10, marginBottom:8 }}>
                      ...
                      {ins.badge && (
                        <View style={{ backgroundColor: ins.color+'30', borderRadius:8, paddingHorizontal:7, paddingVertical:3, marginRight:4 }}>
                          <Text style={{ color: ins.color, fontSize:9, fontWeight:'800', letterSpacing:0.5 }}>{ins.badge}</Text>
                        </View>
                      )}
      ...
      {/* ── Voice mic bubble — true floating overlay, above everything ── */}
      {state.voiceEnabled && (
        <Animated.View style={{
          position:'absolute',
          bottom: 90,
          ...
          transform:[{ scale: breathAnim
            ? breathAnim.interpolate({ inputRange:[0,1], outputRange:[1,1.08] })
            : 1 }],
        }}>
          {/* Outer glow ring */}
          <Animated.View style={{
            ...
            transform:[{ scale: breathAnim
              ? breathAnim.interpolate({ inputRange:[0,1], outputRange:[1,1.3] })
              : 1 }],
          }} pointerEvents="none" />
          <TouchableOpacity
            onPress={onVoiceTap}
            ...
            <Text style={{ fontSize:7, fontWeight:'800', color:T.accent, letterSpacing:0.5 }}>VOICE</Text>
          </TouchableOpacity>
        </Animated.View>
      )}
      ...
      {/* Category sheet */}
      {sheetCat && selBbl >= 0 && (
        <SlideScreen onClose={() => { setSelBbl(-1); setSheetCat(null); setSheetOrigin({ x:0, y:0 }); }} zIndex={50} T={T} animLevel={animLevel} originX={sheetOrigin.x} originY={sheetOrigin.y}>
          {close => {
            const cat = sheetCat;
      ...
      <Text style={{ color: T.sub, fontSize: 12, marginTop: 4 }}>Tap ➕ to add your first</Text>
                    ...
                    : catEx.slice(0, 15).map((e, idx) => (
                      <SpaghettifyRow key={e.id} animLevel={animLevel} T={T}
                        onDelete={() => {
                          if (onDelete) onDelete(e.id);
                          else dispatch({ type: 'DELETE', id: e.id });
                        }}>
                        {(handleDel) => (
...
function StatsPie({ data, total, T, onDoubleTap, animLevel }) {
  ...
  useEffect(() => {
    ...
    const stagger = animLevel === 'reduced' ? 40 : 90;
    const pieSpring = animLevel === 'reduced' ? { toValue:1, tension:80, friction:20, useNativeDriver:true } : { toValue:1, tension:28, friction:12, useNativeDriver:true };
    Animated.stagger(stagger, scaleAnims.map(a => Animated.spring(a, pieSpring))).start();
    setSelIdx(-1);
  }, [key]);
...
function GoalAchievedBadge({ isEmi, T, animLevel }) {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(pulseAnim, { toValue: 1.04, duration: animLevel==='reduced'?500:900, useNativeDriver: true }),
        Animated.timing(pulseAnim, { toValue: 1,    duration: animLevel==='reduced'?500:900, useNativeDriver: true }),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(glowAnim, { toValue: 1, duration: animLevel==='reduced'?600:1200, useNativeDriver: true }),
        Animated.timing(glowAnim, { toValue: 0.3, duration: animLevel==='reduced'?600:1200, useNativeDriver: true }),
      ])
    ).start();

    // Spark particles — skip on reduced animLevel
    if (animLevel === 'reduced') return;
...
function GoalCard({ feat, entry, dispatch, T, onContribute, animLevel }) {
  ...
  return (
    <View style={[{ borderLeftWidth: 4, borderLeftColor: color, padding: 16, marginBottom: 12 }, neuSurface(T, { borderRadius: 18 })]}>
      {achieved && <GoalAchievedBadge isEmi={feat.id === 'emi'} T={T} animLevel={animLevel} />}
...
function GoalContributeSheet({ entry, feat, dispatch, T, onClose, animLevel }) {
  const color    = FEAT_COLORS[feat.id] || feat.color;
  ...
  return (
    <SlideScreen onClose={onClose} zIndex={70} T={T} animLevel={animLevel || 'full'}>
      {close => (
        <>
...
// ─── POLAROID CARD ───────────────────────────────────────────────────────────
function PolaroidCard({ ins, stackIdx, onSwipedAway, animLevel }) {
  const tx      = useRef(new Animated.Value(0)).current;
  const ty      = useRef(new Animated.Value(0)).current;
  const rot     = useRef(new Animated.Value(stackIdx === 0 ? 0 : stackIdx%2===0 ? 1.5 : -1.5)).current;
  const opacity = useRef(new Animated.Value(stackIdx >= 3 ? 0.4 : 1)).current;
  const scale   = useRef(new Animated.Value(1 - stackIdx * 0.045)).current;
  const offsetY = useRef(new Animated.Value(stackIdx * 10)).current;

  // When this card becomes the new top, animate it forward
  useEffect(() => {
    if (stackIdx === 0) {
      Animated.parallel([
        Animated.spring(scale,   { toValue:1,   tension:65, friction:10, useNativeDriver:true }),
        Animated.spring(offsetY, { toValue:0,   tension:65, friction:10, useNativeDriver:true }),
        Animated.spring(rot,     { toValue:0,   tension:65, friction:10, useNativeDriver:true }),
        Animated.timing(opacity, { toValue:1, duration:200, useNativeDriver:true }),
      ]).start();
    } else {
      Animated.parallel([
        Animated.spring(scale,   { toValue:1 - stackIdx*0.045, tension:65, friction:10, useNativeDriver:true }),
        Animated.spring(offsetY, { toValue:stackIdx*10,        tension:65, friction:10, useNativeDriver:true }),
        Animated.spring(rot,     { toValue:stackIdx%2===0?1.5:-1.5, tension:65, friction:10, useNativeDriver:true }),
        Animated.timing(opacity, { toValue:stackIdx>=3?0.4:1, duration:200, useNativeDriver:true }),
      ]).start();
    }
  }, [stackIdx]);
  ...
  const pan = useRef(PanResponder.create({
    onMoveShouldSetPanResponder: (_, g) =>
      stackIdx === 0 && Math.abs(g.dx) > 6 && Math.abs(g.dx) > Math.abs(g.dy),
    onPanResponderMove: (_, g) => {
      tx.setValue(g.dx);
      ty.setValue(g.dy * 0.25);
      rot.setValue(g.dx * 0.06);
      opacity.setValue(Math.max(0, 1 - Math.abs(g.dx) / 220));
    },
    onPanResponderRelease: (_, g) => {
      if (Math.abs(g.dx) >= 90 || Math.abs(g.vx) >= 0.8) {
        const dir = g.dx > 0 ? 1 : -1;
        if (animLevel === 'none') { onSwipedAway(); return; }
        Animated.parallel([
          Animated.timing(tx,      { toValue: dir*450, duration:320, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
          Animated.timing(rot,     { toValue: dir*20,  duration:320, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
          Animated.timing(opacity, { toValue: 0,       duration:260, useNativeDriver:true }),
        ]).start(onSwipedAway);
      } else {
        Animated.parallel([
          Animated.spring(tx,      { toValue:0, tension:65, friction:10, useNativeDriver:true }),
          Animated.spring(ty,      { toValue:0, tension:65, friction:10, useNativeDriver:true }),
          Animated.spring(rot,     { toValue:0, tension:65, friction:10, useNativeDriver:true }),
  ...
  const rotDeg = rot.interpolate({ inputRange:[-30,30], outputRange:['-30deg','30deg'] });

  return (
    <Animated.View
      {...(stackIdx === 0 ? pan.panHandlers : {})}
      style={{
      ...
      <View style={{
        ...
        <View style={{ position:'absolute', top:0, left:0, right:0, height:3,
          backgroundColor: ins.color, borderRadius:14 }} />
        <View style={{ width:48, height:48, borderRadius:14,
          backgroundColor: ins.color+'18', alignItems:'center',
          justifyContent:'center', marginBottom:14 }}>
          <Text style={{ fontSize:26 }}>{ins.icon}</Text>
        </View>
        {ins.badge && (
          <View style={{ backgroundColor: ins.color+'20', borderRadius:8,
            paddingHorizontal:10, paddingVertical:4, alignSelf:'flex-start', marginBottom:10 }}>
            <Text style={{ color:ins.color, fontSize:10, fontWeight:'800', letterSpacing:0.5 }}>
              {ins.badge}
            </Text>
          </View>
        )}
        <Text style={{ fontSize:18, fontWeight:'800', color:ins.color, marginBottom:10, lineHeight:24 }}>
          {ins.title}
        </Text>
        <Text style={{ fontSize:13, lineHeight:20, color:'rgba(230,225,229,0.65)' }}>
          {ins.text}
        </Text>
        {/* Polaroid strip at bottom */}
        <View style={{ position:'absolute', bottom:0, left:0, right:0, height:44,
          backgroundColor:'rgba(255,255,255,0.04)',
          borderTopWidth:1, borderTopColor:'rgba(255,255,255,0.06)',
          alignItems:'center', justifyContent:'center' }}>
          <Text style={{ fontSize:11, color:'rgba(230,225,229,0.35)', fontStyle:'italic' }}>
            {ins.title}
          </Text>
        </View>
      </View>
...
// ─── SAVED INSIGHTS — POLAROID STACK ─────────────────────────────────────────
function SavedInsightsView({ savedInsights, dispatch, T, animLevel }) {
  // savedInsights is actually liveInsights — live data, no date keys needed
  ...
  // Build stack whenever live insights change
  useEffect(() => {
    const items = dedup(saved);
    setAllItems(items);
    setStack(items.slice());
    setDone(false);
  }, [savedInsights]);
  ...
  if (saved.length === 0) {
    return (
      <View style={styles.emptyBox}>
        ...
        <Text style={{ color:T.sub, fontSize:12, marginTop:6, textAlign:'center', lineHeight:18 }}>
          Insights save automatically each day you open the app
        </Text>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex:1 }}>
      ...
      <View style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between',
        ...
        <Text style={{ color:T.text, fontSize:14, fontWeight:'800' }}>✦ Live Insights</Text>
        {!done && stack.length > 0 && (
          <View style={{ paddingHorizontal:12, paddingVertical:5, borderRadius:99,
            backgroundColor: T.accent+'18' }}>
            <Text style={{ fontSize:11, fontWeight:'700', color:T.accent }}>
              {allItems.length - stack.length + 1} / {allItems.length}
            </Text>
          </View>
        )}
      </View>
      ...
      {/* Stack area */}
      <View style={{ height:340, position:'relative', marginHorizontal:8 }}>
        {/* Colour glow behind stack */}
        {stack.length > 0 && (
          <View style={{ position:'absolute', top:40, left:'20%', right:'20%', height:200,
            borderRadius:100, backgroundColor: stack[0].color+'30' }}
            pointerEvents="none" />
        )}
        ...
        {done ? (
          ...
          <View style={{ flex:1, alignItems:'center', justifyContent:'center', gap:12 }}>
            ...
            <Text style={{ fontSize:16, fontWeight:'700', color:T.text }}>All caught up!</Text>
            <Text style={{ fontSize:12, color:T.sub, textAlign:'center', lineHeight:18 }}>
              You have seen all insights for this period
            </Text>
            <TouchableOpacity onPress={handleReplay}
              style={{ marginTop:4, paddingHorizontal:24, paddingVertical:12,
                borderRadius:14, backgroundColor: T.accent+'18',
                borderWidth:1, borderColor: T.accent+'40' }}>
              <Text style={{ color:T.accent, fontSize:13, fontWeight:'700' }}>↺ Review again</Text>
            </TouchableOpacity>
          </View>
        ) : (
          // Render top 4 cards, back to front
          [...stack].slice(0,4).reverse().map((ins, revIdx) => {
            const stackIdx = Math.min(stack.length, 4) - 1 - revIdx;
            return (
              <PolaroidCard
                key={ins.title + (ins.savedAt||'') + stackIdx}
                ins={ins}
                stackIdx={stackIdx}
        ...
        {!done && stack.length === allItems.length && (
          <View style={{ position:'absolute', bottom:-24, left:0, right:0,
            alignItems:'center' }} pointerEvents="none">
            <Text style={{ fontSize:11, color:T.sub, fontStyle:'italic' }}>
              ← Swipe to dismiss →
            </Text>
          </View>
        )}
      ...
      {/* No clear button needed — insights are live and always current */}
    </View>
  );
}
...
// ─── TREND BAR — budget-relative bar with liquid fill ────────────────────────
function TrendBar({ value, budget, maxH, color, isCur, animLevel }) {
  const fillAnim  = useRef(new Animated.Value(0)).current;
  const waveAnim  = useRef(new Animated.Value(0)).current;
  const pct       = budget > 0 ? Math.min(value / budget, 1) : 0;
  const targetH   = Math.max(Math.round(pct * maxH), value > 0 ? 4 : 2);
  const fillColor = isCur ? color : color + '99';

  useEffect(() => {
    if (animLevel === 'none') { fillAnim.setValue(targetH); return; }
    // Rise animation
    Animated.timing(fillAnim, {
      toValue: targetH,
      duration: animLevel === 'reduced' ? 500 : 1000,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: false,
    }).start();
    // Wave loop — only on full
    if (animLevel !== 'reduced') {
      Animated.loop(
  ...
  // Wave translateX: moves -50% of wave width = seamless loop
  // Wave is 200% wide so -50% = exactly 1 cycle
  const waveX = waveAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -40], // moves left continuously
  });

  return (
    <View style={{ height: maxH, width: '100%', justifyContent: 'flex-end',
      backgroundColor: color + '18', borderRadius: 6, overflow: 'hidden' }}>
      <Animated.View style={{ height: fillAnim, width: '100%', overflow: 'hidden',
        backgroundColor: fillColor, borderRadius: 6 }}>
        {/* Wave surface — two overlapping oval shapes that scroll left */}
        {animLevel !== 'none' && animLevel !== 'reduced' && (
          <Animated.View style={{
            position: 'absolute', top: -6,
            flexDirection: 'row',
            transform: [{ translateX: waveX }],
          }}>
            {/* 6 wave humps side by side — scroll left seamlessly */}
            {[0,1,2,3,4,5].map(k => (
              <View key={k} style={{
                width: 14, height: 12,
                marginRight: 2,
...
function StatsScreen({ expenses, featEntries, bills, savedInsights, dispatch, T, animLevel, selCat, setSelCat, onEditExpense, budget }) {
  ...
  const buildData = () => {
    ...
    if (selCat === 'expenses') {
      ...
      // Add MoM delta to each category
      const lmF = expenses.filter(e => monthOf(e.date) === (() => { const d=new Date(selM+'-01'); d.setMonth(d.getMonth()-1); return d.toISOString().slice(0,7); })());
      const lmCatTot = {};
      lmF.forEach(e => { lmCatTot[e.name] = (lmCatTot[e.name]||0)+e.amount; });
      // Attach delta to each cat
      const bdWithDelta = bd.map(cat => ({
        ...cat,
        lmAmt: expenses.filter(e => monthOf(e.date) === (() => { const d=new Date(selM+'-01'); d.setMonth(d.getMonth()-1); return d.toISOString().slice(0,7); })() && e.category===cat.name).reduce((a,e)=>a+e.amount,0),
      }));
      return { breakdown: bdWithDelta, total: filtered.reduce((a,e)=>a+e.amount,0) };
    }
    if (selCat === 'split') {
  ...
  const buildTrend = () => {
    const getVal = m => {
      ...
      if (selCat === 'income') {
        // Show available (income minus all outflows) per month — not gross income
        const income = (featEntries['income']||[]).filter(e=>monthOf(e.date)===m).reduce((a,e)=>a+(e.amount||0),0);
        const outflow = expenses.filter(e=>monthOf(e.date)===m).reduce((a,e)=>a+e.amount,0);
        return Math.max(income - outflow, 0); // 0 floor — can't show negative bar
      }
  ...
  // Budget-relative scaling — bar height = % of budget used, not % of max
  const trendBudget = view === 'Quarterly' ? budget * 3 : view === 'Yearly' ? budget * 12 : budget;
  // tmx kept as fallback for non-expense categories (income, investments etc)
  // where budget doesn't apply — those use relative max scaling
  const tmx = selCat === 'expenses' ? Math.max(trendBudget, 1) : Math.max(...trend.map(t=>t.value), 1);
  ...
  return (
    <View style={{ flex:1 }}>
      <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
        ...
        {selCat !== 'insights' && (
        <View style={[styles.section, neuSurface(T)]}>
            ...
            {trend.map((t,i) => {
              ...
              return (
                <TouchableOpacity key={i} onPress={() => { if(t.key){ setSelM(t.key); if(view==='Monthly'){setTrendMonth(t.key);} }}}
                  ...
                  <TrendBar value={t.value} budget={tmx} maxH={80} color={activeCat.color} isCur={isCur} animLevel={animLevel} />
        ...
        <View style={[styles.section, tileStyle(T)]}>
          ...
          {/* Saved Insights view */}
          {selCat === 'insights' && (
            <SavedInsightsView savedInsights={savedInsights} dispatch={dispatch} T={T} animLevel={animLevel} />
          )}
            ...
            <>
              ...
              <View style={{ flexDirection:'row', gap:16, alignItems:'center', marginBottom:16 }}>
                <TouchableOpacity onPress={() => setPieExpanded(true)} activeOpacity={0.85}>
                  <StatsPie
                    data={catD} total={catTotal} T={T} animLevel={animLevel}
                ...
                <View style={{ flex:1 }}>
                  {catD.slice(0,5).map((cat,i) => {
                    const delta = cat.lmAmt !== undefined ? cat.amt - cat.lmAmt : null;
                    const deltaStr = delta !== null && Math.abs(delta) > 50 ? (delta>0?'+':'-')+'₹'+Math.round(Math.abs(delta)/1000*10)/10+'k' : null;
                    return (
                    <View key={cat.name} style={{ flexDirection:'row', alignItems:'center', gap:7, marginBottom:7 }}>
                      ...
                      {deltaStr && <Text style={{ color:delta>0?'#FF6B6B':'#2ECC8A', fontSize:9, fontWeight:'700' }}>{deltaStr}</Text>}
              ...
              {(featEntries[selCat]||[]).length === 0
                ...
                : (featEntries[selCat]||[]).map(entry => (
                    <GoalCard key={entry.id}
                      ...
                      entry={entry} dispatch={dispatch} T={T} animLevel={animLevel}
...
// ─── FORM SCREEN ──────────────────────────────────────────────────────────────
function FormScreen({ existing, onSave, onClose, T, animLevel, voiceKeywords }) {
  ...
  // Auto-suggest category from title — checks user-trained keywords first
  const handleTitleChange = (t) => {
    setTitle(t);
    if (!existing) {
      const tl = t.toLowerCase();
      // Check user-taught keywords first
      const userMatch = (voiceKeywords||[]).find(k =>
        (k.variations||[k.word||'']).some(v => v && tl.includes(v.toLowerCase()))
      );
      if (userMatch) { setCat(userMatch.cat); return; }
...
function TaxSummaryScreen({ featEntries, T }) {
  ...
  // Auto-pull from investments and EMI
  const invest80C   = (featEntries?.investments||[]).filter(e=>e.subType==='80C Investment').reduce((a,e)=>a+(e.amount||0),0);
  const homeLoanSec24 = (featEntries?.emi||[]).filter(e=>e.subType==='Home Loan').reduce((a,e)=>a+(e.amount||0),0);
  const totalInc    = (featEntries?.income||[]).reduce((a,e)=>a+(e.amount||0),0);
  const SECTIONS = [
  ...
  // Auto-add from investments and EMI — no double counting
  if (invest80C > 0) totals['80C Investment'] = (totals['80C Investment']||0) + invest80C;
  if (homeLoanSec24 > 0) totals['Home Loan'] = (totals['Home Loan']||0) + homeLoanSec24;
  const grandTotal = Object.values(totals).reduce((a,v)=>a+v,0);
  const estSaving  = Math.round(grandTotal * 0.3);

  return (
    <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
      <Text style={[styles.screenTitle, { color:T.text }]}>Tax Summary 📊</Text>
      {/* Auto-connection notice */}
      {(invest80C > 0 || homeLoanSec24 > 0) && (
        <View style={{ marginHorizontal:16, marginBottom:10, backgroundColor:T.accent+'12',
          borderRadius:12, padding:10, borderWidth:1, borderColor:T.accent+'25', flexDirection:'row', gap:8 }}>
          <Text style={{ fontSize:14 }}>✦</Text>
          <Text style={{ flex:1, fontSize:11, color:T.sub, lineHeight:16 }}>
            {invest80C > 0 ? fmt(invest80C)+' from your Investments auto-added to 80C. ' : ''}
            {homeLoanSec24 > 0 ? fmt(homeLoanSec24)+' home loan interest auto-added under Section 24.' : ''}
          </Text>
        </View>
      )}
      {/* Summary */}
      <View style={{ marginHorizontal:16, marginBottom:16, backgroundColor:T.grad1, borderRadius:18, padding:18 }}>
        <Text style={{ color:'rgba(255,255,255,0.7)', fontSize:11 }}>TOTAL DEDUCTIONS</Text>
        ...
        {totalInc > 0 && (
          <Text style={{ color:'rgba(255,255,255,0.5)', fontSize:11, marginTop:6 }}>
            {'Annual income logged: '+fmt(totalInc)}
          </Text>
        )}
      </View>
      {(taxEntries.length === 0 && invest80C === 0 && homeLoanSec24 === 0)
        ? <View style={styles.emptyBox}>
            <Text style={{ fontSize:40, marginBottom:12 }}>📊</Text>
...
// ─── VOICE QUICK ADD SCREEN ───────────────────────────────────────────────────

// ── NLP PARSER — HINGLISH AWARE ───────────────────────────────────────────────
...
const HINDI_NUMBERS = {
  ...
  'bees':20,'ikkees':21,'baaees':22,'teis':23,'chaubees':24,'pachees':25,
  'chabbees':26,'sattaees':27,'atthaees':28,'untees':29,'tees':30,
  'ikattees':31,'battees':32,'taintees':33,'chautees':34,'paintees':35,
  'chattees':36,'saintees':37,'artees':38,'untaalees':39,'chalis':40,
  'iktaalees':41,'bytaalees':42,'taintaalees':43,'chavaalees':44,'paintaalees':45,
  'chhiyaalees':46,'saintaalees':47,'artaalees':48,'unchaas':49,'pachaas':50,
  'ikkaavan':51,'baavan':52,'tirpan':53,'chauvan':54,'pachpan':55,
  'chhappan':56,'sattavan':57,'atthaavan':58,'unsaath':59,'saath':60,
  'iksath':61,'basath':62,'tirsath':63,'chausath':64,'painsath':65,
  'chhiyasath':66,'sadsath':67,'arsath':68,'unhattar':69,'sattar':70,
  'ikhattar':71,'bahattar':72,'tihattar':73,'chauhattar':74,'pachhattar':75,
  'chhihattar':76,'satahattar':77,'athahattar':78,'unnaasi':79,'assi':80,
  'ikyaasi':81,'bayaasi':82,'tiraasi':83,'chaurasi':84,'pachhaasi':85,
  'chhiyaasi':86,'sataasi':87,'atthaasi':88,'navaasi':89,'nabbe':90,
  'ikyaanave':91,'baanave':92,'tiraanave':93,'chauraanave':94,'panchaanave':95,
  'chhiyaanave':96,'sataanave':97,'atthaanave':98,'ninyaanave':99,
  'sau':100,'do sau':200,'teen sau':300,'char sau':400,'paanch sau':500,
  'chhe sau':600,'saat sau':700,'aath sau':800,'nau sau':900,
  'ek hazaar':1000,'do hazaar':2000,'paanch hazaar':5000,'das hazaar':10000,
};

// Devanagari → Roman for common words
const DEVA_MAP = {
  'पान':'paan','चाय':'chai','सब्जी':'sabzi','दूध':'milk','पानी':'water',
  'खाना':'food','खाना खाया':'food','ऑटो':'auto','बस':'bus','मेट्रो':'metro',
  'दवाई':'medicine','दवा':'medicine','डॉक्टर':'doctor','किताब':'book',
  'किराया':'rent','बिजली':'electricity','गैस':'gas','मोबाइल':'mobile',
  'बाजार':'grocery','सब्ज़ी':'sabzi','फल':'fruits','अख़बार':'newspaper',
  'चाय':'chai','कॉफी':'coffee','बिस्कुट':'biscuit','चिप्स':'chips',
  'सिगरेट':'cigarette','बीड़ी':'beedi','पान':'paan','गुटखा':'gutka',
  'में':'','मे':'','का':'','के':'','की':'','लिए':'','से':'','और':'','पर':'',
  'रुपये':'','रुपए':'','रुपया':'','का':'','को':'','ने':'',
};

// Hindi stop words to remove from title
const HINDI_STOPS = ['mein','meh','ka','ke','ki','liye','se','aur','par','pe',
  'rupaye','rupee','rupaya','karo','add','kiya','hua','hai','tha','ko'];

// Transliterate Devanagari to Roman
function devaToRoman(text) {
  let t = text;
  Object.entries(DEVA_MAP).forEach(([deva, roman]) => {
    t = t.replace(new RegExp(deva, 'g'), roman);
  });
  return t;
}

// Extract amount — handles digits AND Hindi number words
function extractAmount(t) {
  // Try multi-word first (do sau = 200)
  const multiWord = Object.entries(HINDI_NUMBERS)
    .filter(([k]) => k.includes(' '))
    .sort((a,b) => b[0].length - a[0].length);
  for (const [word, val] of multiWord) {
  ...
  for (const w of words) {
    if (HINDI_NUMBERS[w]) return HINDI_NUMBERS[w];
  }
  return 0;
}

const VOICE_CATS = [
  { keys:['food','eat','zomato','swiggy','restaurant','lunch','dinner','breakfast','coffee','cafe','pizza','burger','biryani','meal','snack','khana'], name:'Food', icon:'🍔' },
  { keys:['transport','auto','uber','ola','cab','bus','metro','train','fuel','petrol','taxi','travel','fare','rickshaw','sawari'], name:'Transport', icon:'🚗' },
  { keys:['shopping','clothes','amazon','flipkart','myntra','purchase','buy','bought','kharida'], name:'Shopping', icon:'🛍️' },
  { keys:['subscriptions','subscription','netflix','spotify','hotstar','youtube','prime','disney','streaming'], name:'Subscriptions', icon:'📱' },
  { keys:['entertainment','movie','game','beer','alcohol','bar','pub','party','show','film','timepass'], name:'Entertainment', icon:'🎬' },
  { keys:['utilities','electricity','water','gas','bill','recharge','mobile','bijli','paani','kiraya'], name:'Utilities', icon:'💡' },
  { keys:['health','medicine','doctor','pharmacy','hospital','gym','fitness','medical','dawai','dawa'], name:'Health', icon:'💊' },
  { keys:['education','book','course','tuition','school','college','fees','kitaab'], name:'Education', icon:'📚' },
  { keys:['grocery','groceries','vegetables','fruits','milk','bigbasket','blinkit','zepto','supermarket','sabzi','subzi','sabji','doodh'], name:'Groceries', icon:'🛒' },
  { keys:['dining','restaurant','dine'], name:'Dining', icon:'🍽️' },
];

function parseVoice(text, userKeywords, userCorrections) {
  if (!text) return null;

  // Step 1 — Transliterate Devanagari to Roman
  let t = devaToRoman(text).toLowerCase().trim();

  // Step 2 — Extract amount (digits + Hindi number words)
  const amount = extractAmount(t);
  if (amount <= 0) return null;

  // Step 3 — Remove amount words from text for cleaner title/category detection
  let cleaned = t.replace(/\d+/g, ' ');
  Object.keys(HINDI_NUMBERS).forEach(w => { cleaned = cleaned.replace(new RegExp('\\b'+w+'\\b','g'), ' '); });
  cleaned = cleaned.trim();

  // Step 4a — Check voice corrections FIRST (highest priority)
  if (userCorrections && userCorrections.length > 0) {
    for (const corr of userCorrections) {
      if (t.includes(corr.heard.toLowerCase()) || cleaned.includes(corr.heard.toLowerCase())) {
        return {
          title: corr.englishTitle,
          amount,
          category: corr.cat,
  ...
  // Step 4b — Check user-taught keywords SECOND
  if (userKeywords && userKeywords.length > 0) {
    for (const uk of userKeywords) {
      const vars = (uk.variations && uk.variations.length > 0) ? uk.variations : (uk.word ? [uk.word] : []);
      const matched = vars.some(v => v && (cleaned.includes(v.toLowerCase()) || t.includes(v.toLowerCase())));
      if (matched) {
        return {
          title: uk.englishTitle || (uk.word ? uk.word.charAt(0).toUpperCase()+uk.word.slice(1) : uk.cat),
          amount,
          category: uk.cat,
          date: new Date().toISOString().slice(0,10),
          note: 'Added by voice',
  ...
  // Step 5 — Find category from built-in list
  let cat = { name:'Other', icon:'🎯', keys:[] };
  for (const vc of VOICE_CATS) {
    if (vc.keys.some(k => cleaned.includes(k))) { cat = vc; break; }
  }

  // Step 6 — Build title
  const stop = ['add','rupees','rs','inr','spent','paid','for','on','under','to','and','the','a','of','worth',...HINDI_STOPS];
  const genericCatWords = ['food','transport','shopping','entertainment','utilities','health','education','groceries','grocery','dining','subscriptions','subscription'];
  const allCatWords = VOICE_CATS.flatMap(vc => [...vc.keys, vc.name.toLowerCase()]);
  const catNameLower = cat.name.toLowerCase();
  const matchedKey = cat.keys.find(k => cleaned.includes(k) && k !== catNameLower && !genericCatWords.includes(k))
    || cat.keys.find(k => cleaned.includes(k) && k !== catNameLower)
    || cat.keys.find(k => cleaned.includes(k)) || '';
  const words = cleaned.split(/\s+/).filter(w =>
    w && w.length > 1 && !stop.includes(w) && !allCatWords.includes(w)
  );
  let title = words.length > 0 ? words.join(' ').trim() : matchedKey;
  title = title.charAt(0).toUpperCase() + title.slice(1);
  if (!title) title = cat.name;

  return { title, amount, category: cat.name, date: new Date().toISOString().slice(0,10), note: 'Added by voice' };
}

// ── VOICE LISTENING OVERLAY ───────────────────────────────────────────────────
// SPEECH_HTML removed — using expo-speech-recognition instead


function VoiceListenOverlay({ T, animLevel, onClose, onAdd, onCorrect, userKeywords, userCorrections }) {
  const [phase, setPhase] = React.useState('listening');
  const [corrForm, setCorrForm] = React.useState({ heard:'', englishTitle:'', cat:'Other' });
  const [corrPickerOpen, setCorrPickerOpen] = React.useState(false);
  ...
  React.useEffect(() => {
    if (!ExpoSpeechRecognitionModule) {
      setErrMsg('Speech recognition not available on this device.');
      setPhase('error');
      return;
    }
    ...
    ExpoSpeechRecognitionModule.requestPermissionsAsync().then(result => {
      if (!result.granted) {
        setErrMsg('Microphone permission denied. Please allow in Settings.');
        setPhase('error');
        return;
      }
      ExpoSpeechRecognitionModule.start({
        ...
        androidIntentOptions: {
          ...
          EXTRA_LANGUAGE: 'hi-IN',
          EXTRA_SUPPORTED_LANGUAGES: 'hi-IN,en-IN',
        },
      });
    }).catch(err => {
      setErrMsg('Could not start speech recognition: ' + err.message);
      setPhase('error');
    });
    // Cleanup on unmount
    return () => {
  ...
  // Register speech recognition events using addListener (not a hook — safe inside useEffect)
  React.useEffect(() => {
    if (!ExpoSpeechRecognitionModule) return;
    const onResult = ExpoSpeechRecognitionModule.addListener('result', event => {
      const text = event.results[0]?.transcript || '';
      setTranscript(text);
      if (event.results[0]?.isFinal || event.isFinal) {
        const result = parseVoice(text, userKeywords, userCorrections);
        if (result) { setParsed(result); setPhase('result'); }
        else { setErrMsg('Could not understand. Try: "Auto 200 transport"'); setPhase('error'); }
      }
    });
    const onError = ExpoSpeechRecognitionModule.addListener('error', event => {
      if (event.error === 'aborted') return;
      setErrMsg(event.error === 'not-allowed'
    ...
    const onEnd = ExpoSpeechRecognitionModule.addListener('end', () => {
      // Wait 800ms before showing error — result event can arrive just after end on some devices
      setTimeout(() => {
        setPhase(prev => {
          if (prev === 'listening') {
    ...
    return () => {
      try { onResult?.remove(); onError?.remove(); onEnd?.remove(); } catch(e) {}
    };
  }, []);
  ...
  const ring2Scale = pulseAnim.interpolate({ inputRange:[0,1], outputRange:[1,1.35] });

  return (
    <View style={{ position:'absolute', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.88)', zIndex:999, alignItems:'center', justifyContent:'center' }}>
      {phase === 'listening' && (
        <>
          <Animated.View style={{ position:'absolute', width:180, height:180, borderRadius:90, borderWidth:2, borderColor:T.accent+'40', transform:[{scale:ring2Scale}] }} pointerEvents="none" />
          <Animated.View style={{ position:'absolute', width:140, height:140, borderRadius:70, borderWidth:2, borderColor:T.accent+'60', transform:[{scale:ringScale}] }} pointerEvents="none" />
          <Animated.View style={{ width:100, height:100, borderRadius:50, backgroundColor:T.accent+'30', borderWidth:2, borderColor:T.accent, alignItems:'center', justifyContent:'center', transform:[{scale:ringScale}] }}>
            <Text style={{ fontSize:40 }}>🎙️</Text>
          </Animated.View>
          <Text style={{ color:'#fff', fontSize:20, fontWeight:'800', marginTop:28 }}>Listening...</Text>
          <Text style={{ color:T.accent, fontSize:14, marginTop:8, fontStyle:'italic', textAlign:'center', paddingHorizontal:40, minHeight:22 }}>{transcript || ' '}</Text>
          <Text style={{ color:'rgba(255,255,255,0.4)', fontSize:12, marginTop:12, textAlign:'center', paddingHorizontal:40 }}>
            {'Say: "Auto 200 transport" or "Zomato food 450"'}
          </Text>
          <TouchableOpacity onPress={() => { try { ExpoSpeechRecognitionModule.abort(); } catch(e) {} onClose(); }} style={{ marginTop:36, paddingHorizontal:28, paddingVertical:12, borderRadius:99, borderWidth:1, borderColor:'rgba(255,255,255,0.2)' }}>
      ...
      {phase === 'result' && parsed && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24 }}>
          <Text style={{ color:T.sub, fontSize:12, marginBottom:16, textAlign:'center' }}>Got it! Does this look right?</Text>
          <View style={{ flexDirection:'row', alignItems:'center', gap:14, marginBottom:20 }}>
            <View style={{ width:56, height:56, borderRadius:16, backgroundColor:T.accent+'22', alignItems:'center', justifyContent:'center' }}>
              <Text style={{ fontSize:28 }}>{VOICE_CATS.find(v=>v.name===parsed.category)?.icon || '🎯'}</Text>
            </View>
            <View style={{ flex:1 }}>
              <Text style={{ color:T.text, fontSize:16, fontWeight:'800' }}>{parsed.title}</Text>
              <Text style={{ color:T.sub, fontSize:12, marginTop:3 }}>{parsed.category} · Today</Text>
            </View>
            <Text style={{ color:T.accent, fontSize:22, fontWeight:'800' }}>{'₹'}{parsed.amount}</Text>
          </View>
          <View style={{ flexDirection:'row', gap:8, marginBottom:8 }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setParsed(null); ExpoSpeechRecognitionModule?.start({ lang:'hi-IN', interimResults:true, continuous:false, androidIntentOptions:{ EXTRA_LANGUAGE_MODEL:'web_search', EXTRA_LANGUAGE:'hi-IN', EXTRA_SUPPORTED_LANGUAGES:'hi-IN,en-IN' } }); }}
              style={{ flex:1, padding:12, borderRadius:14, backgroundColor:T.surface2, alignItems:'center' }}>
              <Text style={{ color:T.sub, fontSize:13, fontWeight:'700' }}>Try Again</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => onAdd(parsed)}
              style={{ flex:2, padding:12, borderRadius:14, backgroundColor:T.accent, alignItems:'center' }}>
              <Text style={{ color:'#fff', fontSize:13, fontWeight:'800' }}>✓ Add Expense</Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity
            onPress={() => {
              setCorrForm({ heard: transcript, englishTitle: '', cat: parsed.category });
              setPhase('correct');
            }}
            style={{ padding:10, borderRadius:12, borderWidth:1, borderColor:T.accent+'40', alignItems:'center' }}>
            <Text style={{ color:T.accent, fontSize:12, fontWeight:'700' }}>✏️ Wrong? Correct & teach the app</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onClose} style={{ marginTop:10, alignItems:'center' }}>
            <Text style={{ color:T.sub, fontSize:12 }}>Cancel</Text>
          </TouchableOpacity>
        </View>
      )}

      {phase === 'correct' && (
        <View style={{ width:'92%', backgroundColor:T.surface, borderRadius:24, padding:20 }}>
          <Text style={{ color:T.text, fontSize:16, fontWeight:'800', marginBottom:4 }}>✏️ Teach the correction</Text>
          <Text style={{ color:T.sub, fontSize:12, marginBottom:16, lineHeight:18 }}>
            {'The app heard this wrong. Tell it what you said and what it means — it will remember forever.'}
          </Text>

          {/* What the app heard */}
          <Text style={{ color:T.sub, fontSize:10, fontWeight:'700', letterSpacing:0.8, marginBottom:6 }}>APP HEARD</Text>
          <View style={{ backgroundColor:T.surface2, borderRadius:12, padding:12, marginBottom:14 }}>
            <Text style={{ color:'#FF6B6B', fontSize:14, fontWeight:'600' }}>{transcript || 'Unknown'}</Text>
          </View>

          {/* What you actually said */}
          <Text style={{ color:T.sub, fontSize:10, fontWeight:'700', letterSpacing:0.8, marginBottom:6 }}>WHAT YOU SAID (as the app mishears it)</Text>
          <TextInput
            value={corrForm.heard}
            onChangeText={v => setCorrForm(p => ({ ...p, heard: v }))}
            placeholder="e.g. pan  (how app hears paan)"
            placeholderTextColor={T.sub}
            style={{ backgroundColor:T.surface2, borderRadius:12, padding:12,
              color:T.text, fontSize:14, marginBottom:14,
              borderWidth:1, borderColor:T.accent+'30' }}
          />

          {/* English title to log */}
          <Text style={{ color:T.sub, fontSize:10, fontWeight:'700', letterSpacing:0.8, marginBottom:6 }}>LOG THIS ENGLISH NAME</Text>
          <TextInput
            value={corrForm.englishTitle}
            onChangeText={v => setCorrForm(p => ({ ...p, englishTitle: v }))}
            placeholder="e.g. Paan"
            placeholderTextColor={T.sub}
            style={{ backgroundColor:T.surface2, borderRadius:12, padding:12,
              color:T.text, fontSize:14, marginBottom:14,
              borderWidth:1, borderColor:T.accent+'30' }}
          ...
          {/* Category picker */}
          <Text style={{ color:T.sub, fontSize:10, fontWeight:'700', letterSpacing:0.8, marginBottom:8 }}>CATEGORY</Text>
          <View style={{ flexDirection:'row', flexWrap:'wrap', gap:6, marginBottom:20 }}>
            {CATS.map(cat => (
              <TouchableOpacity key={cat.name}
                onPress={() => setCorrForm(p => ({ ...p, cat: cat.name }))}
                style={{ paddingHorizontal:10, paddingVertical:6, borderRadius:99,
                  backgroundColor: corrForm.cat === cat.name ? cat.color+'28' : T.surface2,
                  borderWidth:1, borderColor: corrForm.cat === cat.name ? cat.color : 'transparent' }}>
                <Text style={{ fontSize:11, color: corrForm.cat === cat.name ? cat.color : T.sub, fontWeight:'600' }}>
                  {cat.icon} {cat.name}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={{ flexDirection:'row', gap:8 }}>
            <TouchableOpacity onPress={() => setPhase('result')}
              style={{ flex:1, padding:13, borderRadius:13, backgroundColor:T.surface2, alignItems:'center' }}>
              <Text style={{ color:T.sub, fontSize:13, fontWeight:'700' }}>Back</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => {
                const heard = corrForm.heard.trim().toLowerCase();
                const englishTitle = corrForm.englishTitle.trim();
                if (!heard || !englishTitle) return;
                const correction = { heard, englishTitle, cat: corrForm.cat };
                onCorrect(correction);
                // Also add expense with corrected values
                onAdd({
                  title: englishTitle,
                  amount: parsed.amount,
                  category: corrForm.cat,
                  date: new Date().toISOString().slice(0,10),
                  note: 'Added by voice',
                });
              }}
              style={{ flex:2, padding:13, borderRadius:13, backgroundColor:T.accent, alignItems:'center' }}>
              <Text style={{ color:'#fff', fontSize:13, fontWeight:'800' }}>💾 Save & Add Expense</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
      ...
      {phase === 'error' && (
        <View style={{ width:'88%', backgroundColor:T.surface, borderRadius:24, padding:24, alignItems:'center' }}>
          <Text style={{ fontSize:40, marginBottom:12 }}>😕</Text>
          <Text style={{ color:T.text, fontSize:16, fontWeight:'800', marginBottom:8, textAlign:'center' }}>Oops!</Text>
          <Text style={{ color:T.sub, fontSize:13, textAlign:'center', lineHeight:20, marginBottom:20 }}>{errMsg}</Text>
          <View style={{ flexDirection:'row', gap:10, width:'100%' }}>
            <TouchableOpacity onPress={() => { setPhase('listening'); setTranscript(''); setErrMsg(''); ExpoSpeechRecognitionModule?.start({ lang:'hi-IN', interimResults:true, continuous:false, androidIntentOptions:{ EXTRA_LANGUAGE_MODEL:'web_search', EXTRA_LANGUAGE:'hi-IN', EXTRA_SUPPORTED_LANGUAGES:'hi-IN,en-IN' } }); }}
            ...
            <TouchableOpacity onPress={onClose}
              style={{ flex:1, padding:14, borderRadius:14, backgroundColor:T.surface2, alignItems:'center' }}>
              <Text style={{ color:T.sub, fontSize:14, fontWeight:'700' }}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
...
// ─── TRAIN SCREEN ─────────────────────────────────────────────────────────────
const TRAIN_QUICK = {
  'Common':  ['cigarette','chai','paan','beedi','supari','gutka'],
  'Food':    ['vadapav','samosa','lassi','mithai','bhajiya','dabeli'],
  'Daily':   ['newspaper','parking','toll','tip','donation','laundry'],
};
const TRAIN_SUGGESTIONS = {
  cigarette:'Entertainment', bidi:'Entertainment', beedi:'Entertainment',
  paan:'Entertainment', hookah:'Entertainment', gutka:'Entertainment', supari:'Entertainment',
  chai:'Food', tea:'Food', coffee:'Food', lassi:'Food', juice:'Food',
  sabzi:'Groceries', vegetables:'Groceries', fruits:'Groceries', milk:'Groceries',
  vadapav:'Food', samosa:'Food', mithai:'Food', bhajiya:'Food', dabeli:'Food',
  medicine:'Health', tablet:'Health', doctor:'Health', pharmacy:'Health',
  petrol:'Transport', diesel:'Transport', parking:'Transport', toll:'Transport',
  rent:'Utilities', electricity:'Utilities', water:'Utilities',
  newspaper:'Other', tip:'Other', donation:'Other', laundry:'Other',
};

function TrainScreen({ state, dispatch, T, animLevel, onClose }) {
  const [tab, setTab] = useState(0);
  const [teachInput, setTeachInput] = useState('');
  const [englishInput, setEnglishInput] = useState('');
  const [sugCard, setSugCard] = useState(null);
  const [showPicker, setShowPicker] = useState(false);
  const [pickerFor, setPickerFor] = useState(null);
  const [pickerWord, setPickerWord] = useState('');
  const [addedChips, setAddedChips] = useState({});
  const [addVarFor, setAddVarFor] = useState(null); // englishTitle of entry to add variation to
  const [varInput, setVarInput] = useState('');
  const confAnim = useRef(new Animated.Value(0)).current;
  const cardAnim = useRef(new Animated.Value(0)).current;
  const toastAnim = useRef(new Animated.Value(0)).current;
  const [toastMsg, setToastMsg] = useState('');
  const keywords = state.voiceKeywords || [];

  // ── Particle burst ──────────────────────────────────────────────────────────
  // (visual only — no actual particles in RN, we use a flash animation instead)
  ...
  // ── Save keyword — new structure with englishTitle + variations ─────────────
  const saveKeyword = (spokenWord, cat, englishTitle) => {
    const variation = spokenWord.toLowerCase().trim();
    const title = (englishTitle || spokenWord).trim();
    if (!variation || !title) return;
    dispatch({ type:'ADD_VOICE_KW', kw: {
      englishTitle: title,
    ...
    doFlash();
    showToast('🧠 Learned "' + spokenWord + '" = ' + title + ' → ' + cat.name);
  };

  // ── Add variation to existing entry ─────────────────────────────────────────
  const addVariation = (englishTitle, newVariation) => {
    const v = newVariation.toLowerCase().trim();
    if (!v || !englishTitle) return;
    const existing = keywords.find(k => k.englishTitle?.toLowerCase() === englishTitle.toLowerCase());
    if (!existing) return;
    dispatch({ type:'ADD_VOICE_KW', kw: { ...existing, variations: [v] } });
    doFlash();
    showToast('➕ Added "' + newVariation + '" as variation of ' + englishTitle);
    setVarInput('');
    setAddVarFor(null);
  };
  ...
  const getSuggestion = (word) => {
    ...
    const catName = TRAIN_SUGGESTIONS[w] || 'Other';
    return CATS.find(c => c.name === catName) || CATS[CATS.length-1];
  };

  // ── Submit teach ────────────────────────────────────────────────────────────
  const submitTeach = () => {
    const w = teachInput.trim();
    if (!w) return;
    const cat = getSuggestion(w);
    const conf = TRAIN_SUGGESTIONS[w.toLowerCase()] ? (70 + Math.floor(Math.random()*25)) : (35 + Math.floor(Math.random()*30));
    // Auto-suggest English title — capitalize first letter
    const suggestedEnglish = englishInput.trim() || w.charAt(0).toUpperCase() + w.slice(1);
    setSugCard({ word: w, englishTitle: suggestedEnglish, cat, conf });
    setTeachInput('');
    setEnglishInput('');
    confAnim.setValue(0);
    cardAnim.setValue(0);
    Animated.parallel([
      ...
      Animated.timing(confAnim, { toValue: conf/100, duration:900, useNativeDriver:false }),
    ]).start();
  };

  const confColor = sugCard ? (sugCard.conf > 70 ? '#34D399' : sugCard.conf > 50 ? '#FBBF24' : '#FB923C') : '#B985FA';

  // ── Render ──────────────────────────────────────────────────────────────────
  return (
    <SlideScreen onClose={onClose} zIndex={95} T={T} animLevel={animLevel}>
      {close => (
        <>
          {/* Flash overlay */}
          <Animated.View pointerEvents="none" style={{ position:'absolute', top:0, left:0, right:0, bottom:0,
            backgroundColor:T.accent, opacity:flashAnim.interpolate({inputRange:[0,1],outputRange:[0,0.18]}),
            zIndex:200 }} />

          {/* Toast */}
          <Animated.View pointerEvents="none" style={{ position:'absolute', top:60, alignSelf:'center',
            backgroundColor:T.accent+'22', borderWidth:1.5, borderColor:T.accent+'60',
            borderRadius:99, paddingHorizontal:20, paddingVertical:10, zIndex:201,
            opacity:toastAnim, transform:[{scale:toastAnim.interpolate({inputRange:[0,1],outputRange:[0.8,1]})}] }}>
            <Text style={{ color:T.accent, fontSize:13, fontWeight:'700' }}>{toastMsg}</Text>
          </Animated.View>

          {/* Header */}
          <View style={[styles.detailHero, { backgroundColor:T.accent+'18' }]}>
            <TouchableOpacity onPress={close} style={styles.backBtn}>
              <Text style={{ color:T.text, fontSize:18 }}>←</Text>
            </TouchableOpacity>
            <Text style={{ fontSize:36, marginBottom:6 }}>🧠</Text>
            <Text style={[styles.detailTitle, { color:T.text }]}>Train Your App</Text>
            <Text style={{ color:T.sub, fontSize:12, marginTop:4, textAlign:'center' }}>
              {'Teach custom words for smarter voice recognition'}
            </Text>
          </View>

          {/* Tabs */}
          <View style={{ flexDirection:'row', gap:6, paddingHorizontal:16, paddingVertical:10 }}>
            {['⚡ Quick Add','🎓 Teach','📚 My Words'].map((label, i) => (
              <TouchableOpacity key={i} onPress={() => setTab(i)}
                style={{ flex:1, paddingVertical:8, borderRadius:12, alignItems:'center',
                  backgroundColor: tab===i ? T.accent+'22' : T.surface2,
                  borderWidth:1, borderColor: tab===i ? T.accent+'50' : 'transparent' }}>
                <Text style={{ fontSize:10, fontWeight:'700',
                  color: tab===i ? T.accent : T.sub }}>
                  {label}{i===2 && keywords.length>0 ? ' ('+keywords.length+')' : ''}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <ScrollView style={{ flex:1 }} contentContainerStyle={{ padding:16, paddingBottom:60 }}
            ...
            {tab === 0 && (
              <>
                {Object.entries(TRAIN_QUICK).map(([section, words]) => (
                  <View key={section}>
                    <Text style={{ fontSize:10, fontWeight:'700', color:T.accent+'99',
                      letterSpacing:0.8, marginBottom:8, marginTop:4 }}>{section.toUpperCase()}</Text>
                    <View style={{ flexDirection:'row', flexWrap:'wrap', gap:8, marginBottom:14 }}>
                      {words.map(word => {
                        const added = addedChips[word] || keywords.some(k=>k.word===word);
                        return (
                          <TouchableOpacity key={word}
                            onPress={() => {
                              if (added) return;
                              setPickerFor('chip'); setPickerWord(word); setShowPicker(true);
                            }}
                            activeOpacity={0.7}
                            style={{ paddingHorizontal:14, paddingVertical:8, borderRadius:99,
                              backgroundColor: added ? '#34D39922' : T.accent+'14',
                              borderWidth:1.5, borderColor: added ? '#34D399' : T.accent+'40' }}>
                            <Text style={{ fontSize:12, fontWeight:'700',
                              color: added ? '#34D399' : T.accent }}>
                              {added ? '✓ ' : ''}{word}
                            </Text>
                          </TouchableOpacity>
                        );
                ...
                <Text style={{ color:T.sub, fontSize:11, textAlign:'center', marginTop:8, lineHeight:18 }}>
                  {'Tap a word → pick its category → saved!\nThe app will recognise it next time you use voice.'}
                </Text>
              </>
            )}
            ...
            {tab === 1 && (
              <>
                <View style={{ ...neuSurface(T), padding:14, marginBottom:10 }}>
                  <Text style={{ fontSize:10, fontWeight:'700', color:T.accent+'80',
                    letterSpacing:0.8, marginBottom:8 }}>WHAT WILL YOU SAY? (spoken word)</Text>
                  <View style={{ flexDirection:'row', gap:10, alignItems:'center', marginBottom:10 }}>
                    <TextInput
                      value={teachInput}
                      onChangeText={setTeachInput}
                      placeholder={'e.g. kaam waali, kam wali, maid...'}
                      placeholderTextColor={T.sub+'60'}
                      style={{ flex:1, color:T.text, fontSize:15, fontWeight:'600', paddingVertical:4 }}
                      returnKeyType="next"
                    />
                  </View>
                  <Text style={{ fontSize:10, fontWeight:'700', color:T.accent+'80',
                    letterSpacing:0.8, marginBottom:8 }}>LOG AS (English name)</Text>
                  <View style={{ flexDirection:'row', gap:10, alignItems:'center' }}>
                    <TextInput
                      value={englishInput}
                      onChangeText={setEnglishInput}
                      placeholder={'e.g. Maid (what gets logged in app)'}
                      placeholderTextColor={T.sub+'60'}
                      style={{ flex:1, color:T.text, fontSize:15, fontWeight:'600', paddingVertical:4 }}
                      onSubmitEditing={submitTeach}
                      returnKeyType="done"
                    />
                    <TouchableOpacity onPress={submitTeach}
                      style={{ width:38, height:38, borderRadius:12,
                        backgroundColor: teachInput.trim() ? T.accent : T.surface2,
                        alignItems:'center', justifyContent:'center' }}>
                      <Text style={{ fontSize:16, color: teachInput.trim() ? '#fff' : T.sub }}>→</Text>
                    </TouchableOpacity>
                  </View>
                </View>

                {sugCard && (
                  <Animated.View style={{ ...neuSurface(T), padding:16, marginBottom:12,
                    opacity:cardAnim, transform:[{scale:cardAnim.interpolate({inputRange:[0,1],outputRange:[0.88,1]})}] }}>
                    <View style={{ flexDirection:'row', alignItems:'center', gap:12, marginBottom:8 }}>
                      <View style={{ width:52, height:52, borderRadius:15,
                        backgroundColor:sugCard.cat.color+'22', alignItems:'center', justifyContent:'center' }}>
                        <Text style={{ fontSize:26 }}>{sugCard.cat.icon}</Text>
                      </View>
                      <View style={{ flex:1 }}>
                        <Text style={{ color:T.text, fontSize:17, fontWeight:'800' }}>
                          {sugCard.englishTitle}
                        </Text>
                        <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>
                          {'spoken as "'+sugCard.word+'" → '+sugCard.cat.name}
                        </Text>
                      </View>
                    </View>
                    {/* Confidence bar */}
                    <View style={{ flexDirection:'row', alignItems:'center', gap:8, marginBottom:14 }}>
                      <View style={{ flex:1, height:4, borderRadius:2, backgroundColor:T.surface2 }}>
                    ...
                    <View style={{ flexDirection:'row', gap:8 }}>
                      <TouchableOpacity onPress={() => { setPickerFor('teach'); setPickerWord(sugCard.word); setShowPicker(true); }}
                        style={{ flex:1, padding:12, borderRadius:12, backgroundColor:T.surface2, alignItems:'center' }}>
                        <Text style={{ color:T.sub, fontSize:13, fontWeight:'700' }}>Pick different ›</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => { saveKeyword(sugCard.word, sugCard.cat, sugCard.englishTitle); setSugCard(null); }}
                        style={{ flex:2, padding:12, borderRadius:12, backgroundColor:T.accent, alignItems:'center' }}>
                        <Text style={{ color:'#fff', fontSize:13, fontWeight:'800' }}>✓ Yes, save it!</Text>
                      </TouchableOpacity>
                    </View>
                  </Animated.View>
                )}

                <Text style={{ color:T.sub, fontSize:11, textAlign:'center', lineHeight:18 }}>
                  {'Type the spoken word and what it means in English.\nThe app will log it in English always.'}
                </Text>
              </>
            )}
            ...
            {tab === 2 && (
              <>
                {keywords.length === 0 && (state.voiceCorrections||[]).length === 0 ? (
                  <View style={{ alignItems:'center', paddingVertical:48 }}>
                    <Text style={{ fontSize:48, marginBottom:12 }}>🧠</Text>
                    <Text style={{ color:T.text, fontSize:15, fontWeight:'700', marginBottom:6 }}>Nothing taught yet</Text>
                    <Text style={{ color:T.sub, fontSize:12, textAlign:'center', lineHeight:18 }}>
                      {'Use Quick Add or Teach tab to add your personal words'}
                    </Text>
                  </View>
                ) : (
                  <>
                    {keywords.length > 0 && (
                      <>
                        <Text style={{ color:T.accent, fontSize:10, fontWeight:'700',
                          letterSpacing:0.8, marginBottom:8, marginTop:4 }}>TAUGHT WORDS</Text>
                        {keywords.map((kw, i) => (
                          <View key={i} style={{ ...neuSurface(T), padding:14, marginBottom:10 }}>
                            <View style={{ flexDirection:'row', alignItems:'center', gap:12, marginBottom:10 }}>
                              <View style={{ width:42, height:42, borderRadius:12,
                                backgroundColor:kw.color+'22', alignItems:'center', justifyContent:'center' }}>
                                <Text style={{ fontSize:20 }}>{kw.icon}</Text>
                              </View>
                              <View style={{ flex:1 }}>
                                <Text style={{ color:T.text, fontSize:14, fontWeight:'800' }}>
                                  {kw.englishTitle || (kw.word ? kw.word.charAt(0).toUpperCase()+kw.word.slice(1) : '')}
                                </Text>
                                <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>{kw.cat}</Text>
                              </View>
                              <TouchableOpacity onPress={() => dispatch({ type:'DEL_VOICE_KW', i })}
                            ...
                            <View style={{ flexDirection:'row', flexWrap:'wrap', gap:6, marginBottom:8 }}>
                              {(kw.variations||[kw.word||'']).filter(Boolean).map((v,vi) => (
                                <View key={vi} style={{ paddingHorizontal:10, paddingVertical:4,
                                  borderRadius:99, backgroundColor:kw.color+'15',
                                  borderWidth:1, borderColor:kw.color+'30' }}>
                                  <Text style={{ fontSize:11, color:kw.color, fontWeight:'600' }}>{v}</Text>
                                </View>
                              ))}
                            </View>
                            {addVarFor === kw.englishTitle ? (
                              <View style={{ flexDirection:'row', gap:8, alignItems:'center' }}>
                                <TextInput
                                  value={varInput}
                                  onChangeText={setVarInput}
                                  placeholder={'Add another way to say it...'}
                                  placeholderTextColor={T.sub+'60'}
                                  style={{ flex:1, backgroundColor:T.surface2, borderRadius:10,
                                    padding:8, color:T.text, fontSize:13 }}
                                  autoFocus
                                  returnKeyType="done"
                                  onSubmitEditing={() => addVariation(kw.englishTitle, varInput)}
                                />
                                <TouchableOpacity onPress={() => addVariation(kw.englishTitle, varInput)}
                                  style={{ paddingHorizontal:12, paddingVertical:8, borderRadius:10, backgroundColor:T.accent }}>
                                  <Text style={{ color:'#fff', fontSize:12, fontWeight:'700' }}>Add</Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => { setAddVarFor(null); setVarInput(''); }}>
                                  <Text style={{ color:T.sub, fontSize:12 }}>Cancel</Text>
                                </TouchableOpacity>
                              </View>
                            ) : (
                              <TouchableOpacity
                                onPress={() => { setAddVarFor(kw.englishTitle); setVarInput(''); }}
                                style={{ flexDirection:'row', alignItems:'center', gap:6,
                                  paddingVertical:6, paddingHorizontal:10, borderRadius:99,
                                  borderWidth:1, borderColor:T.accent+'30',
                                  alignSelf:'flex-start', backgroundColor:T.accent+'10' }}>
                                <Text style={{ color:T.accent, fontSize:11, fontWeight:'700' }}>+ Add variation</Text>
                              </TouchableOpacity>
                            )}
                          </View>
                        ))}
                    ...
                    {(state.voiceCorrections||[]).length > 0 && (
                      <>
                        <Text style={{ color:'#34D399', fontSize:10, fontWeight:'700',
                          letterSpacing:0.8, marginBottom:8, marginTop:12 }}>VOICE CORRECTIONS</Text>
                        {(state.voiceCorrections||[]).map((corr, i) => (
                          <View key={i} style={{ ...neuSurface(T), flexDirection:'row', alignItems:'center',
                            gap:12, padding:14, marginBottom:8 }}>
                            <View style={{ width:42, height:42, borderRadius:12,
                            ...
                            <View style={{ flex:1 }}>
                              <View style={{ flexDirection:'row', alignItems:'center', gap:6 }}>
                                <Text style={{ color:'#FF6B6B', fontSize:12, fontWeight:'600' }}>{corr.heard}</Text>
                                <Text style={{ color:T.sub, fontSize:11 }}>→</Text>
                                <Text style={{ color:'#34D399', fontSize:12, fontWeight:'700' }}>{corr.englishTitle}</Text>
                              </View>
                              <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>{corr.cat}</Text>
                            </View>
                            <TouchableOpacity onPress={() => dispatch({ type:'DEL_VOICE_CORR', i })}
                              style={{ width:32, height:32, borderRadius:10,
                                backgroundColor:'rgba(255,107,107,0.1)', alignItems:'center', justifyContent:'center' }}>
                              <Text style={{ fontSize:14, color:'#FF6B6B' }}>✕</Text>
                            </TouchableOpacity>
          ...
          {showPicker && (
            <TouchableOpacity activeOpacity={1} onPress={() => setShowPicker(false)}
              style={{ position:'absolute', top:0, left:0, right:0, bottom:0,
                backgroundColor:'rgba(0,0,0,0.6)', justifyContent:'flex-end', zIndex:300 }}>
              <TouchableOpacity activeOpacity={1}>
                <View style={{ backgroundColor:T.surface, borderTopLeftRadius:24, borderTopRightRadius:24,
                  ...
                  <Text style={{ color:T.text, fontSize:16, fontWeight:'800',
                    textAlign:'center', marginBottom:4 }}>Which category?</Text>
                  <Text style={{ color:T.sub, fontSize:12, textAlign:'center', marginBottom:16 }}>
                    {'"'+pickerWord+'" belongs in...'}
                  </Text>
                  <View style={{ flexDirection:'row', flexWrap:'wrap', gap:8 }}>
                    {CATS.map(cat => (
                      <TouchableOpacity key={cat.name}
                        onPress={() => {
                          setShowPicker(false);
                          if (pickerFor === 'chip') {
                            saveKeyword(pickerWord, cat);
                            setAddedChips(prev => ({ ...prev, [pickerWord]: true }));
                          } else {
                            setSugCard(prev => prev ? { ...prev, cat } : null);
                            saveKeyword(pickerWord, cat);
                            setSugCard(null);
                          }
                        }}
                        style={{ paddingHorizontal:14, paddingVertical:10, borderRadius:12,
                          backgroundColor:cat.color+'18', borderWidth:1, borderColor:cat.color+'40',
                          flexDirection:'row', alignItems:'center', gap:6 }}>
                        <Text style={{ fontSize:16 }}>{cat.icon}</Text>
                        <Text style={{ color:T.text, fontSize:12, fontWeight:'700' }}>{cat.name}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </View>
...
function VoiceScreen({ state, dispatch, T, animLevel, onClose, onTrain }) {
  const [expanded, setExpanded] = useState({ how:false, offline:false, commands:false, trouble:false });
  const toggle = key => setExpanded(prev => ({ ...prev, [key]: !prev[key] }));
  const isOn = state.voiceEnabled || false;

  const Section = ({ id, title, icon, children }) => (
    <TouchableOpacity
      onPress={() => toggle(id)}
      activeOpacity={0.8}
      style={{ ...neuSurface(T), padding:16, marginBottom:10 }}>
      <View style={{ flexDirection:'row', alignItems:'center', gap:10 }}>
        <Text style={{ fontSize:20 }}>{icon}</Text>
        <Text style={{ flex:1, color:T.text, fontSize:14, fontWeight:'700' }}>{title}</Text>
        <Text style={{ color:T.sub, fontSize:14 }}>{expanded[id] ? '▲' : '▼'}</Text>
      </View>
      {expanded[id] && (
  ...
  return (
    <SlideScreen onClose={onClose} zIndex={90} T={T} animLevel={animLevel}>
      {close => (
        <>
          <View style={[styles.detailHero, { backgroundColor: T.accent+'22' }]}>
            <TouchableOpacity onPress={close} style={styles.backBtn}>
              <Text style={{ color: T.text, fontSize: 18 }}>←</Text>
            </TouchableOpacity>
            <Text style={{ fontSize: 40, marginBottom: 8 }}>🎙️</Text>
            <Text style={[styles.detailTitle, { color: T.text }]}>Voice Quick Add</Text>
            <Text style={{ color: T.sub, fontSize: 12, marginTop: 4, textAlign: 'center' }}>
              Add expenses hands-free with your voice
            </Text>
          </View>
          ...
          <ScrollView style={{ flex:1 }} contentContainerStyle={{ padding:16, paddingBottom:60 }}
            ...
            <View style={{ ...neuSurface(T), padding:16, marginBottom:10 }}>
              <View style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
                <View style={{ width:44, height:44, borderRadius:14,
                  backgroundColor: isOn ? T.accent+'22' : T.surface2,
                  alignItems:'center', justifyContent:'center' }}>
                  <Text style={{ fontSize:22 }}>🎙️</Text>
                </View>
                <View style={{ flex:1 }}>
                  <Text style={{ color:T.text, fontSize:14, fontWeight:'700' }}>Voice Quick Add</Text>
                  <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>
                    {isOn ? 'Enabled — mic bubble visible on home screen' : 'Disabled — enable to use voice commands'}
                  </Text>
                </View>
                <TouchableOpacity
                  onPress={() => dispatch({ type:'SET_VOICE', v: !isOn })}
                  style={{ width:50, height:28, borderRadius:14,
                    backgroundColor: isOn ? T.accent : T.surface2,
                    alignItems:'center', justifyContent:'center',
                    padding:2 }}>
            ...
            {/* Train Your App button */}
            <TouchableOpacity onPress={onTrain}
              style={{ ...neuSurface(T), padding:16, marginBottom:10, flexDirection:'row', alignItems:'center', gap:12 }}>
              <View style={{ width:44, height:44, borderRadius:14, backgroundColor:T.accent+'22', alignItems:'center', justifyContent:'center' }}>
                <Text style={{ fontSize:22 }}>🧠</Text>
              </View>
              <View style={{ flex:1 }}>
                <Text style={{ color:T.text, fontSize:14, fontWeight:'700' }}>Train Your App</Text>
                <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>
                  {(state.voiceKeywords||[]).length > 0
                    ? (state.voiceKeywords||[]).length + ' words taught · Teach more'
                    : 'Teach custom words for better recognition'}
                </Text>
              </View>
            ...
            <Section id="how" title="How it works" icon="💡">
              <Text style={{ color:T.sub, fontSize:13, lineHeight:20 }}>
                {'When enabled, a glowing mic bubble appears on the home screen above the Settings icon.\n\nTap it — speak your expense — the app automatically detects the amount, category and name — tap Add to save it.\n\nVoice recognition uses your device\'s built-in speech engine — no internet required if offline pack is installed.'}
              </Text>
            </Section>

            <Section id="offline" title="Enable offline voice (recommended)" icon="📶">
              <Text style={{ color:T.sub, fontSize:13, lineHeight:22 }}>
                {'To use voice without internet:\n\n1. Open your phone Settings\n2. Go to General Management → Language\n3. Tap Speech recognition\n4. Tap Offline speech recognition\n5. Download the English (India) pack\n\nOnce downloaded, voice works completely offline with zero latency.'}
              </Text>
            </Section>

            <Section id="commands" title="Example voice commands" icon="🗣️">
              {[
                ['"Auto 200 transport"', 'Auto fare · Transport · ₹200'],
                ['"Zomato food 450 rupees"', 'Zomato order · Food · ₹450'],
                ['"Beer entertainment 500"', 'Beer · Entertainment · ₹500'],
                ['"Netflix subscriptions 649"', 'Netflix · Subscriptions · ₹649'],
                ['"Grocery 800 bigbasket"', 'Groceries · Groceries · ₹800'],
              ].map(([cmd, result], i) => (
                <View key={i} style={{ backgroundColor:T.surface2, borderRadius:10, padding:10, marginBottom:8 }}>
                  <Text style={{ color:T.accent, fontSize:12, fontWeight:'700', marginBottom:4 }}>Say: {cmd}</Text>
                  <Text style={{ color:T.sub, fontSize:11 }}>→ {result}</Text>
                </View>
              ))}
              <Text style={{ color:T.sub, fontSize:11, marginTop:4, lineHeight:18 }}>
                Tip: Say amount, category and name in any order — the app figures it out.
              </Text>
            </Section>

            <Section id="trouble" title="Troubleshooting" icon="🔧">
              <Text style={{ color:T.sub, fontSize:13, lineHeight:22 }}>
                {'🔴 Voice not working?\nMake sure microphone permission is granted to this app in your phone settings.\n\n🔴 Takes too long?\nInstall the offline speech pack (see above) for instant recognition.\n\n🔴 Wrong category detected?\nBe more specific — say "transport" instead of just the merchant name.\n\n🔴 Wrong amount?\nSpeak the number clearly — "two hundred" or "200".'}
              </Text>
            </Section>
...
function BackupScreen({ state, dispatch, T, animLevel, onClose }) {
    ...
    try {
      ...
      const backup = {
        ...
        data: {
          ...
          accentKey:     state.accentKey || 'purple',
        }
      };
      ...
      try {
        ...
        await Share.share({ title: 'Celestial Backup', message: json, url: fileUri });
        setStatus('success:Backup shared! Save the file to Google Drive, WhatsApp, or Files app.');
      } catch (fileErr) {
        await Share.share({ title: 'Celestial Backup', message: json });
        setStatus('success:Backup shared as text. Copy and save it somewhere safe.');
      }
    } catch (e) {
  ...
  const handlePickFile = async () => {
    try {
      ...
      setStatus('error:Could not read the file. Try using the Paste JSON option below.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <>
          ...
          <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}
            ...
            <View style={{ backgroundColor: T.surface, borderRadius: 18, padding: 16, marginBottom: 14 }}>
              ...
              <Text style={{ color:T.sub, fontSize:12, lineHeight:18, marginBottom:12 }}>
                Opens your file manager so you can select your backup file directly.
              </Text>
            ...
            {/* IMPORT via paste — always works, no file needed */}
            <View style={{ backgroundColor: T.surface, borderRadius: 18, padding: 16, marginBottom: 14 }}>
              <View style={{ flexDirection:'row', alignItems:'center', gap:12, marginBottom:10 }}>
                <View style={{ width:44, height:44, borderRadius:14, backgroundColor:'#FB923C22', alignItems:'center', justifyContent:'center' }}>
                  <Text style={{ fontSize:22 }}>📋</Text>
                </View>
                <View style={{ flex:1 }}>
                  <Text style={{ color:T.text, fontSize:14, fontWeight:'700' }}>Paste Backup JSON</Text>
                  <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>Paste your backup text directly</Text>
                </View>
              </View>
              <Text style={{ color:T.sub, fontSize:12, lineHeight:18, marginBottom:10 }}>
                If the file picker doesn't work, paste your backup JSON text here directly.
              </Text>
              <TextInput
                multiline
                numberOfLines={4}
                ...
                placeholderTextColor={T.sub}
                style={{ backgroundColor: T.bg, borderRadius:12, padding:12,
                  color:T.text, fontSize:11, fontFamily:'monospace',
                  minHeight:80, borderWidth:1, borderColor:T.border, marginBottom:10 }}
              />
              <TouchableOpacity
                onPress={() => { if(pasteText.trim()) processRestore(pasteText.trim()); else setStatus('error:Please paste your backup JSON first.'); }}
                disabled={loading || saved || !pasteText.trim()}
                style={{ backgroundColor: pasteText.trim() ? '#FB923C' : T.surface2,
                  borderRadius:13, padding:14, alignItems:'center',
                  opacity: (!pasteText.trim() || loading) ? 0.5 : 1 }}>
                <Text style={{ color:'#fff', fontSize:14, fontWeight:'700' }}>
                  {saved ? '✓ Restored!' : '📋 Restore from Pasted Text'}
                </Text>
              </TouchableOpacity>
            </View>
...
// ─── SETTINGS SCREEN ──────────────────────────────────────────────────────────
function SettingsScreen({ state, dispatch, T, animLevel, onRecurring, onSubscriptions, onTaxSummary, onBackup, onVoice }) {
  const [bi, setBi] = useState(String(state.budget));
  const [budgetSaved, setBudgetSaved] = useState(false);
  const [pinInput, setPinInput] = useState('');
  const [bioAvail, setBioAvail] = useState(false);
  const [bioType,  setBioType]  = useState('fingerprint'); // 'face' or 'fingerprint'
  ...
  useEffect(() => {
    if (!DEBUG_MODE) {
      // Use supportedAuthenticationTypesAsync to detect both face and fingerprint
      LocalAuthentication.hasHardwareAsync().then(has => {
        if (!has) { setBioAvail(false); return; }
        LocalAuthentication.supportedAuthenticationTypesAsync().then(types => {
          // types: 1 = FINGERPRINT, 2 = FACIAL_RECOGNITION, 3 = IRIS
          const supported = types && types.length > 0;
          setBioAvail(supported);
          // Store the best available type for icon display
      ...
      console.log('[APP] Biometric disabled in DEBUG_MODE');
      setBioAvail(false);
    }
    const init = {};
  ...
  return (
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity onPress={onVoice}
        ...
        <View style={{ flex:1 }}>
          <Text style={{ color:T.text, fontSize:14, fontWeight:'700' }}>Voice Quick Add</Text>
          <Text style={{ color:T.sub, fontSize:11, marginTop:2 }}>{state.voiceEnabled ? 'Enabled — mic bubble on home screen' : 'Disabled — tap to set up'}</Text>
        </View>
      ...
      <View style={styles.settingSection}>
        ...
        <View style={[styles.settingCard, { backgroundColor: T.surface }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
            {[['default', '🌑 Default'], ['amoled', '⬛ AMOLED'], ['glass', '🪟 Glass'], ['white', '🤍 White Magic'], ['dark', '🖤 AMOLED Magic'], ['matte', '🩶 Matte Black']].map(([key, label]) => (
          ...
          {/* Accent colour picker — not shown for default/amoled which stay fixed */}
          {state.theme !== 'default' && state.theme !== 'amoled' && (
            <>
              <Text style={{ color: T.sub, fontSize: 10, marginBottom: 10, marginTop: 4 }}>ACCENT COLOUR</Text>
              <View style={{ flexDirection:'row', flexWrap:'wrap', gap:10, marginBottom:14 }}>
                {/* Default/reset option */}
                {(() => { const isDefault = (state.accentKey || 'purple') === 'purple'; return (
                  <TouchableOpacity onPress={() => dispatch({ type:'SET_ACCENT', v:'purple' })}
                    style={{ alignItems:'center', gap:4 }}>
                    <View style={{
                      width:32, height:32, borderRadius:16,
                      backgroundColor: state.theme === 'white' ? '#ECEEF3' : '#1A1A1A',
                      borderWidth: isDefault ? 3 : 1.5,
                      borderColor: isDefault ? T.accent : 'rgba(128,128,128,0.4)',
                      alignItems:'center', justifyContent:'center',
                    }}>
                      {isDefault && <Text style={{ fontSize:14, fontWeight:'900', color: state.theme === 'white' ? '#6750A4' : '#fff' }}>✓</Text>}
                    </View>
                    <Text style={{ color: isDefault ? T.accent : T.sub, fontSize:8, fontWeight: isDefault ? '700' : '400' }}>Default</Text>
                  </TouchableOpacity>
                ); })()}
                {Object.entries(ACCENT_PRESETS).map(([key, preset]) => {
                  const isOn = (state.accentKey || 'purple') === key;
                  return (
                    <TouchableOpacity key={key} onPress={() => dispatch({ type:'SET_ACCENT', v:key })}
                      style={{ alignItems:'center', gap:4 }}>
                      <View style={{
                        width:32, height:32, borderRadius:16,
                        backgroundColor: preset.swatch,
                        borderWidth: isOn ? 3 : 1.5,
                        borderColor: isOn ? '#fff' : preset.swatch+'80',
                        shadowColor: preset.swatch, shadowOpacity:0.6,
                        shadowRadius: isOn ? 8 : 3, elevation: isOn ? 6 : 2,
                      }}>
                        {isOn && (
                      ...
                      <Text style={{ color: isOn ? preset.swatch : T.sub, fontSize:8, fontWeight:isOn?'700':'400' }}>
                        {preset.label}
                      </Text>
                    </TouchableOpacity>
                  );
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {[['full', '🚀 Full'], ['reduced', '⚡ Lite'], ['none', '🚫 Off']].map(([key, label]) => (
          ...
          <Text style={{ color: T.sub, fontSize: 11, marginTop: 8, lineHeight: 16 }}>
            {state.animLevel === 'full'    && '120fps on capable devices. All effects enabled.'}
            {state.animLevel === 'reduced' && '⚡ Optimised for low-end devices. Faster, lighter animations — no wave overlay or spark effects.'}
            {state.animLevel === 'none'    && 'All animations disabled. Best performance on very old devices.'}
          </Text>
        </View>
      </View>
...
// ─── LOCK SCREEN ──────────────────────────────────────────────────────────────
function LockScreen({ pin, biometric, bioType, onUnlock, T }) {
  ...
  const tryBio = async () => {
    try { const r = await LocalAuthentication.authenticateAsync({ promptMessage: 'Unlock Expenses', fallbackLabel: 'Use PIN', disableDeviceFallback: false, requireConfirmation: false }); if (r.success) onUnlock(); } catch (e) {}
  };
  const shake = () => {
    Animated.sequence([
  ...
  return (
    <Animated.View style={[styles.lockScreen, { backgroundColor: T.bg, opacity: fadeAnim }]}>
      {/* Lock animation */}
      <View style={{ backgroundColor:'transparent', width:200, height:150, marginBottom:8 }}>
        <LottieView
      ...
      <View style={{ width: 240 }}>
        {[[1,2,3],[4,5,6],[7,8,9],['bio',0,'del']].map((row, ri) => (
          <View key={ri} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
            {row.map(k => {
              if (k === 'bio') return <TouchableOpacity key={k} onPress={biometric ? tryBio : undefined} style={[styles.numKey, { backgroundColor: biometric ? T.surface : 'transparent' }]}><Text style={{ fontSize: 22 }}>{biometric ? (bioType === 'face' ? '😊' : '👆') : ''}</Text></TouchableOpacity>;
...
function ParticleBurst({ onDone, animLevel }) {
  return (
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(e, info) {
    console.error('[APP] ErrorBoundary caught:', e, info);
    Alert.alert('Startup Error', e.toString() + '\n\n' + (e.stack||'').slice(0,200));
  }
  render() {
    if (this.state.error) {
      return (
        <View style={{ flex:1, backgroundColor:'#000', alignItems:'center', justifyContent:'center', padding:24 }}>
          <Text style={{ color:'#FF6B6B', fontSize:16, fontWeight:'800', marginBottom:12 }}>Startup Error</Text>
          <Text style={{ color:'#fff', fontSize:11, lineHeight:18 }}>{this.state.error.toString()}</Text>
        </View>
      );
    }
...
function AppInner() {
  ...
  useEffect(() => { console.log('[APP] HomeScreen/App rendered successfully'); }, []);
  const [state,      dispatch    ] = useReducer(reducer, INIT);
  const [tab,        setTab      ] = useState('home');
  const [loaded,     setLoaded   ] = useState(false);
  const [locked,     setLocked   ] = useState(false);
  const [bioType,    setBioType  ] = useState('fingerprint'); // 'face' or 'fingerprint' — used by LockScreen
  const [splashTheme, setSplashTheme] = useState({ bg:'#0E0C14', grad1:'#6750A4', grad2:'#9C68E8', accent:'#B985FA' });

  // Read saved theme from AsyncStorage to colour splash correctly
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(raw => {
      if (!raw) return;
      try {
        const saved = JSON.parse(raw);
        const themeKey = saved?.theme || 'default';
        const accentKey = saved?.accentKey || 'purple';
        const baseTheme = THEMES[themeKey] || THEMES.default;
        const ap = ACCENT_PRESETS[accentKey] || ACCENT_PRESETS.purple;
        setSplashTheme({
          bg:     baseTheme.bg || '#0E0C14',
          grad1:  ap.grad1,
          grad2:  ap.grad2,
          accent: ap.accent,
        });
  ...
  const [showVoiceListen,   setShowVoiceListen]   = useState(false);
  const [showTrain,         setShowTrain]         = useState(false);

  // Detect face/fingerprint at App level so LockScreen gets correct bioType
  useEffect(() => {
    if (DEBUG_MODE) return;
    LocalAuthentication.hasHardwareAsync().then(has => {
      if (!has) return;
      LocalAuthentication.supportedAuthenticationTypesAsync().then(types => {
        if (types && types.includes(2)) setBioType('face');
        else setBioType('fingerprint');
      }).catch(() => {});
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    console.log('[APP] Loading AsyncStorage...');
      ...
      if (s) {
        try {
          ...
          if (p && typeof p === 'object') {
            ...
            console.log('[APP] AsyncStorage loaded OK');
          }
        } catch (e) {
          console.error('[APP] JSON parse failed, using defaults:', e);
          Alert.alert('Debug Info', 'State parse failed: ' + e.message);
        }
      } else {
        console.log('[APP] No saved state found, using defaults');
      }
      setLoaded(true);
      console.log('[APP] State loaded, app ready');
    }).catch(e => {
      console.error('[APP] AsyncStorage read failed:', e);
      Alert.alert('Debug Info', 'Storage read failed: ' + e.message);
      setLoaded(true);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(state)).catch(e => {
      console.error('[APP] AsyncStorage save failed:', e);
    });
  }, [state, loaded]);

  // ── LIVE INSIGHT ENGINE — App level, always current ──────────────────────
  // Runs whenever expenses, budget, or any financial data changes
  const _appMEx = state.expenses.filter(e => monthOf(e.date) === curMonth());
  const liveInsights = React.useMemo(() => {
    const result = [];
    const now    = new Date();
    const mEx    = state.expenses.filter(e => monthOf(e.date) === curMonth());
    const mTotal = mEx.reduce((a, e) => a + e.amount, 0);
    ...
    const wkendAvg = weekend.reduce((a,e)=>a+e.amount,0)/wkendDays;
    const wkdayAvg = weekday.reduce((a,e)=>a+e.amount,0)/wkdayDays;
    const catTotals = {}; mEx.forEach(e=>{catTotals[e.category]=(catTotals[e.category]||0)+e.amount;});
    const catLMTotals = {}; lmEx.forEach(e=>{catLMTotals[e.category]=(catLMTotals[e.category]||0)+e.amount;});
    const topCat = Object.entries(catTotals).sort((a,b)=>b[1]-a[1])[0];
    const streak = (() => { let s=0,d=new Date(); while(true){ const ds=d.toISOString().slice(0,10); if(!state.expenses.some(e=>e.date===ds))break; s++;d.setDate(d.getDate()-1); } return s; })();
    const monthlyEMITotal = (state.featEntries?.emi||[]).reduce((a,e)=>a+(e.amount||0),0);
    const savingsRate = (() => { const inc=(state.featEntries?.income||[]).filter(e=>monthOf(e.date)===curMonth()).reduce((a,e)=>a+(e.amount||0),0); if(!inc)return null; return Math.round(((inc-mTotal-monthlyEMITotal)/inc)*100); })();
    const disposableIncome = (() => { const inc=(state.featEntries?.income||[]).filter(e=>monthOf(e.date)===curMonth()).reduce((a,e)=>a+(e.amount||0),0); return inc > 0 ? inc - monthlyEMITotal : state.budget - monthlyEMITotal; })();
    const nearGoals=[]; const achievedGoals=[];
    (state.features?.goals ? (state.featEntries?.goals||[]) : []).forEach(g=>{
      const saved=(g.contributions||[]).reduce((a,c)=>a+c.amount,0);
      const tgt=g.targetAmount||g.amount||0;
      if(tgt>0&&saved>=tgt) achievedGoals.push(g);
      else if(tgt>0&&saved/tgt>=0.8) nearGoals.push(g);
    });
    const fmt2 = n => n>=100000?'₹'+(n/100000).toFixed(1)+'L':n>=1000?'₹'+Math.round(n/1000)+'k':'₹'+n;
    if(mEx.length<3){ result.push({icon:'💡',title:'Getting started',text:'Add a few more expenses to unlock personalised financial insights.',color:'#B985FA',priority:5}); }
    if(monthlyEMITotal>0&&disposableIncome>0){ result.push({icon:'🏦',title:'Real disposable: '+fmt2(disposableIncome),text:'After your '+fmt2(monthlyEMITotal)+' monthly EMI, you have '+fmt2(disposableIncome)+' truly available this month. You have used '+Math.round((mTotal/disposableIncome)*100)+'% of that.',color:'#FF9800',priority:2}); }
    if(state.budget>0&&mTotal>state.budget){ result.push({icon:'🚨',title:'Over budget',text:'You have spent '+fmt2(mTotal-state.budget)+' over your '+fmt2(state.budget)+' budget this month.',color:'#FF3B30',priority:1}); }
    else if(state.budget>0&&dayOfMonth<=20&&projected>state.budget){ result.push({icon:'⚠️',title:'On track to overspend',text:'At this rate you will spend '+fmt2(projected)+' by month end — '+fmt2(projected-state.budget)+' over budget.',color:'#FF9800',priority:1}); }
    if(lmTotal>0&&mTotal<lmTotal*0.85){ result.push({icon:'📉',title:'Spending down '+(Math.round((1-mTotal/lmTotal)*100))+'%',text:'Great discipline! You have spent '+fmt2(lmTotal-mTotal)+' less than last month so far.',color:'#2ECC8A',priority:2}); }
    else if(lmTotal>0&&mTotal>lmTotal*1.2){ result.push({icon:'📈',title:'Spending up '+(Math.round((mTotal/lmTotal-1)*100))+'%',text:'You have spent '+fmt2(mTotal-lmTotal)+' more than last month. Biggest driver: '+(topCat?topCat[0]:'unknown')+'.',color:'#FF6B6B',priority:2}); }
    if(savingsRate!==null&&savingsRate>=20){ result.push({icon:'💪',title:'Saving '+savingsRate+'% of income',text:'You are saving '+fmt2(Math.round((state.featEntries?.income||[]).filter(e=>monthOf(e.date)===curMonth()).reduce((a,e)=>a+(e.amount||0),0)*savingsRate/100))+' this month. Excellent!',color:'#2ECC8A',priority:2}); }
    else if(savingsRate!==null&&savingsRate<0){ result.push({icon:'🔴',title:'Spending exceeds income',text:'You have spent '+fmt2(Math.abs(savingsRate))+'% more than your income this month.',color:'#FF3B30',priority:1}); }
    if(streak>=7){ result.push({icon:'🔥',title:streak+'-day logging streak',text:'Consistent tracking is the foundation of financial awareness. Keep it going!',color:'#FF9800',priority:3}); }
    nearGoals.forEach(g=>{ const saved=(g.contributions||[]).reduce((a,c)=>a+c.amount,0); const tgt=g.targetAmount||g.amount||0; result.push({icon:'🎯',title:g.title+' is '+(Math.round(saved/tgt*100))+'% there',text:fmt2(tgt-saved)+' remaining. You are almost at your goal!',color:'#E91E63',priority:2}); });
    // Goals savings projection — show months to reach each active goal
    const inc2=(state.featEntries?.income||[]).filter(e=>monthOf(e.date)===curMonth()).reduce((a,e)=>a+(e.amount||0),0);
    const savings2=inc2>0?Math.max(inc2-mTotal-monthlyEMITotal,0):Math.max(state.budget-mTotal,0);
    if(savings2>0){
      (state.featEntries?.goals||[]).forEach(g=>{
        const saved=(g.contributions||[]).reduce((a,c)=>a+c.amount,0);
        const tgt=g.targetAmount||g.amount||0;
        const pctG=tgt>0?Math.round((saved/tgt)*100):0;
        if(tgt>saved&&pctG<80){
          const months=Math.ceil((tgt-saved)/savings2);
          if(months>=1&&months<=36) result.push({icon:'🎯',title:(g.title||'Goal')+': '+months+(months===1?' month':' months')+' away',text:'At your savings rate of '+fmt2(savings2)+'/month you will reach this in '+months+(months===1?' month':' months')+'. '+fmt2(tgt-saved)+' remaining.',color:'#B985FA',priority:3});
        }
      });
    }
    achievedGoals.forEach(g=>result.push({icon:'🏆',title:g.title+' completed!',text:'Congratulations! You reached your goal of '+fmt2(g.targetAmount||g.amount||0)+'.',color:'#FFD700',priority:1}));
    if(topCat&&mTotal>0){ result.push({icon:'🏷️',title:'Top category: '+topCat[0],text:topCat[0]+' accounts for '+Math.round(topCat[1]/mTotal*100)+'% of your spend ('+fmt2(topCat[1])+') this month.',color:'#9C27B0',priority:3}); }
    if(wkendAvg>wkdayAvg*1.3&&weekend.length>=3){ result.push({icon:'🎉',title:'Weekend spender',text:'You spend '+fmt2(Math.round(wkendAvg))+'/day on weekends vs '+fmt2(Math.round(wkdayAvg))+'/day on weekdays.',color:'#FF9800',priority:3}); }
    const freq={}; state.expenses.forEach(e=>{const k=e.title.toLowerCase().trim();freq[k]=(freq[k]||0)+1;}); const topM=Object.entries(freq).sort((a,b)=>b[1]-a[1])[0];
    if(topM&&topM[1]>=4){ result.push({icon:'🏪',title:'Frequent merchant',text:'You have logged "'+topM[0]+'" '+topM[1]+' times. Consider if this recurring spend is intentional.',color:'#607D8B',priority:4}); }
    if(state.budget>0&&dayOfMonth>0){ const dailyBudget=state.budget/daysInMonth; const actualDaily=mTotal/dayOfMonth; if(actualDaily>dailyBudget*1.2){ result.push({icon:'📅',title:'Daily spend too high',text:'You are spending '+fmt2(Math.round(actualDaily))+'/day vs a budget of '+fmt2(Math.round(dailyBudget))+'/day.',color:'#FF6B6B',priority:2}); } }
    const subAmt2=mEx.filter(e=>e.category==='Subscriptions').reduce((a,e)=>a+e.amount,0);
    if(mTotal>0&&subAmt2/mTotal>0.25){ result.push({icon:'📱',title:'High subscription spend',text:'Subscriptions are '+Math.round(subAmt2/mTotal*100)+'% of your total spend ('+fmt2(subAmt2)+'). Review and cancel unused ones.',color:'#673AB7',priority:3}); }
    const noSpendDays=[...new Set(Array.from({length:dayOfMonth-1},(_,i)=>{const d=new Date();d.setDate(i+1);return d.toISOString().slice(0,10);}))].filter(d=>!state.expenses.some(e=>e.date===d)).length;
    if(noSpendDays>=5){ result.push({icon:'✨',title:noSpendDays+' no-spend days',text:'You had '+noSpendDays+' days with zero spending this month. Mindful spending!',color:'#2ECC8A',priority:3}); }
    const largestEx=mEx.reduce((a,e)=>e.amount>a.amount?e:a,{amount:0});
    if(largestEx.amount>state.budget*0.15&&state.budget>0){ result.push({icon:'💸',title:'Largest expense: '+largestEx.title,text:fmt2(largestEx.amount)+' was your biggest single spend this month — '+Math.round(largestEx.amount/state.budget*100)+'% of your budget.',color:'#FF6B6B',priority:3}); }
    const mthsSorted=[...new Set(state.expenses.map(e=>monthOf(e.date)))].sort();
    if(mthsSorted.length>=2){ const prevMth=mthsSorted[mthsSorted.length-2]; const prevEx=state.expenses.filter(e=>monthOf(e.date)===prevMth); const prevCats={}; prevEx.forEach(e=>{prevCats[e.category]=(prevCats[e.category]||0)+e.amount;}); const surges=Object.entries(catTotals).filter(([cat,amt])=>prevCats[cat]&&amt>prevCats[cat]*1.5&&amt>500).sort((a,b)=>b[1]-a[1]); if(surges.length>0){ result.push({icon:'🆘',title:surges[0][0]+' spending surged',text:surges[0][0]+' is up '+Math.round(surges[0][1]/prevCats[surges[0][0]]*100-100)+'% vs last month ('+fmt2(prevCats[surges[0][0]])+' → '+fmt2(surges[0][1])+').',color:'#FF3B30',priority:2}); } }
    const pmtCounts={}; mEx.forEach(e=>{if(e.paymentMethod){pmtCounts[e.paymentMethod]=(pmtCounts[e.paymentMethod]||0)+1;}}); const topPmt=Object.entries(pmtCounts).sort((a,b)=>b[1]-a[1])[0];
    if(topPmt){ result.push({icon:'💳',title:'Payment habit: '+topPmt[0],text:'You used '+topPmt[0]+' for '+topPmt[1]+' out of '+mEx.length+' transactions this month.',color:'#3F51B5',priority:4}); }
    const thisWeek=mEx.filter(e=>{const d=new Date(e.date);const now2=new Date();const diff=(now2-d)/86400000;return diff<7;});
    const wkSpend=thisWeek.reduce((a,e)=>a+e.amount,0);
    if(wkSpend>0){ result.push({icon:'📅',title:'This week',text:'You spent '+fmt2(wkSpend)+' over '+[...new Set(thisWeek.map(e=>e.date))].length+' days this week.',color:'#2196F3',priority:4}); }
    if(mTotal>0&&savingsRate!==null&&savingsRate>=40){ result.push({icon:'🌟',title:'Excellent savings rate',text:'Saving '+savingsRate+'% of your income. You are in the top tier of savers.',color:'#2ECC8A',priority:2}); }
    if(mEx.length>=15){ const traits=[]; if(wkendAvg>wkdayAvg*1.5)traits.push('Weekend Warrior 🎭'); if(mTotal>0&&subAmt2/mTotal>0.2)traits.push('Subscription Hoarder 📱'); if(savingsRate!==null&&savingsRate>=30)traits.push('Disciplined Saver 💎'); if(mEx.length>=20&&mTotal/Math.max(mEx.length,1)<400)traits.push('Impulse Buyer ⚡'); if(traits.length===0)traits.push('Balanced Spender ⚖️'); result.push({icon:'🧬',title:'Your Financial DNA',text:'You are a '+traits[0]+(traits.length>1?' · '+traits.slice(1,3).join(' · '):'')+'.',color:'#7C3AED',priority:1,badge:'DNA'}); }
    if(state.expenses.length>=10){ let score=50; if(state.budget>0){const adh=mTotal/state.budget;if(adh<=0.7)score+=20;else if(adh<=0.9)score+=12;else if(adh<=1.0)score+=5;else if(adh<=1.2)score-=10;else score-=20;} if(savingsRate!==null){if(savingsRate>=30)score+=15;else if(savingsRate>=20)score+=8;else if(savingsRate>=10)score+=3;else if(savingsRate<0)score-=15;} if(streak>=14)score+=10;else if(streak>=7)score+=5; if(nearGoals.length>0||achievedGoals.length>0)score+=10; if(lmTotal>0&&mTotal<lmTotal)score+=5; score=Math.max(10,Math.min(100,score)); const grade=score>=85?'Excellent 🏆':score>=70?'Good 👍':score>=55?'Average 📊':score>=40?'Needs work ⚠️':'Critical 🚨'; result.push({icon:'⭐',title:'Savings Score: '+score+'/100',text:grade+'. Budget discipline, savings rate, logging streak and goal progress all factor in.',color:score>=70?'#2ECC8A':score>=50?'#FF9800':'#FF3B30',priority:1,badge:'SCORE'}); }
    if(state.expenses.length>=10&&lmTotal>0){const cuts=[];Object.entries(catTotals).forEach(([cat,amt])=>{const lm=catLMTotals[cat]||0;if(amt>lm+300&&lm>0)cuts.push({cat,save:Math.round((amt-lm)*0.6)});});cuts.sort((a,b)=>b.save-a.save);const totalAnnual=cuts.slice(0,3).reduce((a,c)=>a+c.save*12,0);if(totalAnnual>=12000&&cuts.length>=2){const cutList=cuts.slice(0,2).map(c=>'cut '+c.cat+' by '+fmt2(c.save)+'/mo').join(' + ');result.push({icon:'💰',title:'Save ₹'+Math.round(totalAnnual/1000)+'k this year',text:'If you '+cutList+', you will save '+fmt2(totalAnnual)+' over 12 months.',color:'#2ECC8A',priority:2,badge:'CHALLENGE'});}}
    if(state.expenses.length>=20){const dayTotals=[0,0,0,0,0,0,0];const dayCounts=[0,0,0,0,0,0,0];state.expenses.forEach(e=>{const d=new Date(e.date).getDay();dayTotals[d]+=e.amount;dayCounts[d]++;});const dayAvgs=dayTotals.map((t,i)=>dayCounts[i]>0?t/dayCounts[i]:0);const overallAvg=dayAvgs.filter(a=>a>0).reduce((a,b)=>a+b,0)/Math.max(dayAvgs.filter(a=>a>0).length,1);const peakDay=dayAvgs.indexOf(Math.max(...dayAvgs));const peakAvg=dayAvgs[peakDay];const DNAMES=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];if(peakAvg>overallAvg*1.5&&dayCounts[peakDay]>=4){result.push({icon:'🎯',title:DNAMES[peakDay]+' spending trigger',text:'You spend '+fmt2(Math.round(peakAvg))+' on average every '+DNAMES[peakDay]+' — '+Math.round((peakAvg/overallAvg-1)*100)+'% above your daily average.',color:'#9C27B0',priority:3});}}
    const BENCHMARKS={Food:0.15,Transport:0.10,Shopping:0.08,Entertainment:0.06,Subscriptions:0.08,Utilities:0.07,Health:0.05,Groceries:0.12};
    if(mTotal>5000){const bi2=[];Object.entries(BENCHMARKS).forEach(([cat,bm])=>{const ca=mEx.filter(e=>e.category===cat).reduce((a,e)=>a+e.amount,0);const cp=ca/mTotal;if(ca>500&&cp>bm*1.5)bi2.push({cat,catPct:Math.round(cp*100),avg:Math.round(bm*100),excess:fmt2(Math.round(ca-mTotal*bm))});});if(bi2.length>0){const b2=bi2.sort((a,b)=>(b.catPct-b.avg)-(a.catPct-a.avg))[0];result.push({icon:'👥',title:b2.cat+' vs average',text:'You spend '+b2.catPct+'% on '+b2.cat+'. Most people spend '+b2.avg+'%. That is '+b2.excess+' above average per month.',color:'#3F51B5',priority:3});}}
    return result.sort((a,b)=>a.priority-b.priority).slice(0,10);
  }, [
    // Recalculate on ANY financial data change — not just count
    state.expenses.length,
    state.expenses.reduce((a,e)=>a+e.amount,0),
    state.budget,
    JSON.stringify(state.featEntries?.income||[]),
    JSON.stringify(state.featEntries?.goals||[]),
    state.bills?.length,
  ]);

  const T_base = THEMES[state.theme] || THEMES.default;
  const _ap    = ACCENT_PRESETS[state.accentKey] || ACCENT_PRESETS.purple;
  // default and amoled: keep their original colours unchanged (as requested)
  // all other themes: apply accent preset on top
  const T = (state.theme === 'default' || state.theme === 'amoled')
    ? T_base
    : T_base.white
    ? {
        // White Magic — full tonal surface design
        // Canvas shifts to the accent hue, cards float as frosted panels
        ...T_base,
        accent:   _ap.accent,
        grad1:    _ap.grad1,
        grad2:    _ap.grad2,
        card:     _ap.grad1,
        // Tonal overrides — the whole canvas changes colour
        bg:      _ap.bg_white    || T_base.bg,
        nav:     _ap.nav_white   || T_base.nav,
        surface: _ap.surface_white || T_base.surface,
        surface2:_ap.surface_white || T_base.surface2,
        text:    _ap.text_white  || T_base.text,
        sub:     _ap.sub_white   || T_base.sub,
      }
    : {
        // All other non-default/amoled themes — accent only
        ...T_base,
        accent: _ap.accent,
        grad1:  _ap.grad1,
        grad2:  _ap.grad2,
        card:   T_base.matte ? _ap.card : T_base.dark ? _ap.card : _ap.card,
      };
  const aL  = DEBUG_MODE ? 'none' : (state.animLevel || 'full');
  if (DEBUG_MODE) console.log('[APP] Animations disabled in DEBUG_MODE');
  ...
  useEffect(() => {
    ...
    loop.start();
    return () => loop.stop();
  }, [state.theme, state.animLevel, state.expenses.length, state.budget, locked]);

  // ── Restart breathing when app comes to foreground ───────────────────────
  useEffect(() => {
    const sub = AppState.addEventListener('change', nextAppState => {
      if (nextAppState === 'active' && state.theme === 'glass') {
        // Force breathing effect to restart by nudging the value
        breathAnim.setValue(0);
      }
    });
    return () => sub.remove();
  ...
  const lastUnlockTime = useRef(null);
  const suppressLock = useRef(false);
  useEffect(() => {
    const sub = AppState.addEventListener('change', nextState => {
      if (!state.pin) return;
      if (suppressLock.current) return; // voice overlay or other overlay is open — ignore
      if (nextState === 'background') {
        // Only track true background (not inactive — inactive fires too often on MIUI)
        bgTime.current = Date.now();
      } else if (nextState === 'active' && bgTime.current) {
        const elapsed = (Date.now() - bgTime.current) / 1000;
        ...
        // Don't re-lock if we just unlocked within the threshold window
        const timeSinceUnlock = lastUnlockTime.current
          ? (Date.now() - lastUnlockTime.current) / 1000
          : Infinity;
        if (elapsed >= threshold && (threshold === 0 || timeSinceUnlock > threshold)) {
          setLocked(true);
        }
        bgTime.current = null;
      }
  ...
  // Auto-save live insights to savedInsights whenever they change
  // Runs on every expense/budget change — always fresh
  useEffect(() => {
    if (!loaded || !liveInsights.length) return;
    const now = new Date();
    ...
    // Replace today's insights with fresh ones — always current
    dispatch({ type:'CLEAR_TODAY_INSIGHTS', wk, mk, qk, yk });
    liveInsights.forEach(ins => dispatch({ type:'SAVE_INSIGHT', insight:{ ...ins, weekKey:wk, monthKey:mk, quarterKey:qk, yearKey:yk, savedAt:todayStr() } }));
  }, [liveInsights, loaded]);
  const handleSaveInsights = () => {}; // kept for compatibility, no longer used

  if (!loaded) {
    return (
      <View style={{ flex:1, backgroundColor: splashTheme.bg, alignItems:'center', justifyContent:'center' }}>
        {/* Glow behind logo */}
        <View style={{ position:'absolute', width:300, height:300, borderRadius:150,
          backgroundColor: splashTheme.grad1+'25', top:'25%', alignSelf:'center' }}
          pointerEvents="none" />
        {/* App icon */}
        <View style={{ width:88, height:88, borderRadius:26,
          backgroundColor: splashTheme.grad1,
          alignItems:'center', justifyContent:'center',
          shadowColor: splashTheme.grad1, shadowOpacity:0.7,
          shadowRadius:24, elevation:14, marginBottom:20 }}>
          <Text style={{ fontSize:44 }}>🌙</Text>
        </View>
        <Text style={{ fontSize:22, fontWeight:'800', color:'#fff', marginBottom:6 }}>Celestial</Text>
        <Text style={{ fontSize:11, color: splashTheme.accent+'AA',
          letterSpacing:2, textTransform:'uppercase' }}>Expense Tracker</Text>
        {/* Loading dots */}
        <View style={{ flexDirection:'row', gap:6, marginTop:52 }}>
          {[0,1,2].map(i => (
            <View key={i} style={{ width:6, height:6, borderRadius:3,
              backgroundColor: splashTheme.accent, opacity:0.5 }} />
          ))}
        </View>
      </View>
    );
  ...
  if (locked) {
    return (
      <View style={{ flex: 1 }}>
        ...
        <LockScreen pin={state.pin} biometric={state.biometric} bioType={bioType} onUnlock={() => { lastUnlockTime.current = Date.now(); setLocked(false); }} T={T} />
      </View>
    );
  }
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      <StatusBar barStyle="light-content" backgroundColor={T.bg} />
      {tab === 'home' && <HomeScreen state={state} dispatch={dispatch} onRow={handleRow} onEdit={handleEdit} onDelete={handleDelete} onQuickAdd={() => { setFormEx(null); setShowForm(true); }} T={T} animLevel={aL} collapsed={homeCollapsed} setCollapsed={setHomeCollapsed} onSaveInsights={handleSaveInsights} breathAnim={breathAnim} liveInsights={liveInsights} onVoiceTap={() => { suppressLock.current = true; setShowVoiceListen(true); }} />}
      {burst && <ParticleBurst key={burst.id} onDone={() => setBurst(null)} animLevel={aL} />}
      {singularity && <SingularityDelete key={Date.now()} onDone={() => setSingularity(false)} animLevel={aL} />}
      {tab === 'stats'    && <StatsScreen    expenses={state.expenses} featEntries={state.featEntries} bills={state.bills||[]} savedInsights={liveInsights} dispatch={dispatch} T={T} animLevel={aL} selCat={statsCat} setSelCat={setStatsCat} onEditExpense={e => { setFormEx(e); setShowForm(true); }} budget={state.budget} />}
      ...
      {tab === 'settings' && <SettingsScreen state={state} dispatch={dispatch} T={T} animLevel={aL} onRecurring={() => setShowRecurring(true)} onSubscriptions={() => setShowSubscriptions(true)} onTaxSummary={() => setShowTaxSummary(true)} onBackup={() => setShowBackup(true)} onVoice={() => setShowVoice(true)} />}
      {showSubscriptions && (
      ...
      {showVoice  && <VoiceScreen  state={state} dispatch={dispatch} T={T} animLevel={aL} onClose={() => setShowVoice(false)} onTrain={() => setShowTrain(true)} />}
      {showVoiceListen && <VoiceListenOverlay T={T} animLevel={aL} onClose={() => { suppressLock.current = false; setShowVoiceListen(false); }} onAdd={payload => { suppressLock.current = false; dispatch({ type:'ADD', payload: { ...payload, source:'voice' } }); setShowVoiceListen(false); }} onCorrect={correction => dispatch({ type:'ADD_VOICE_CORR', correction })} userKeywords={state.voiceKeywords||[]} userCorrections={state.voiceCorrections||[]} />}
      {showTrain && <TrainScreen state={state} dispatch={dispatch} T={T} animLevel={aL} onClose={() => setShowTrain(false)} />}
      {showRecurring && (
      ...
      {showForm && <FormScreen existing={formEx} onSave={handleSave} onClose={() => setShowForm(false)} T={T} animLevel={aL} voiceKeywords={state.voiceKeywords||[]} />}
    </View>
  );
}
