...
const ACCENT_PRESETS = {
  ...
  purple: { label:'Purple', swatch:'#6750A4', accent:'#B985FA', grad1:'#6750A4', grad2:'#9C68E8', card:'#6750A4',
    bg_white:'#E8E0F5', nav_white:'#E0D8F0', surface_white:'rgba(255,255,255,0.78)',
    text_white:'#2D1B6B', sub_white:'rgba(45,27,107,0.55)' },
...
// ─── HOME SCREEN ─────────────────────────────────────────────────────────────
function HomeScreen({ state, dispatch, onRow, onEdit, onQuickAdd, onDelete, T, animLevel, collapsed, setCollapsed, onSaveInsights, breathAnim, liveInsights }) {
...
function SavedInsightsView({ savedInsights, dispatch, T, animLevel }) {
  ...
  const SECTIONS = [
    { key:'week',    label:'📅 Week',    items: saved.filter(s => s.weekKey    === wk) },
    { key:'month',   label:'📆 Month',   items: saved.filter(s => s.monthKey   === mk) },
    { key:'quarter', label:'🗓️ Quarter', items: saved.filter(s => s.quarterKey === qk) },
    { key:'year',    label:'📊 Year',    items: saved.filter(s => s.yearKey    === yk) },
  ];
  const dedup = arr => {
    const seen = new Set();
  ...
  const [curSec, setCurSec]   = useState('month');
  const [stack,  setStack]    = useState([]);
  const [allItems, setAllItems] = useState([]);
  const [done, setDone]       = useState(false);

  // Build stack when section changes
  useEffect(() => {
    const items = dedup(SECTIONS.find(s=>s.key===curSec)?.items||[]);
    setAllItems(items);
    setStack(items.slice());
    setDone(false);
  }, [curSec, savedInsights]);
  ...
  return (
    <View style={{ flex:1 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {SECTIONS.map(sec => {
          const cnt = dedup(sec.items).length;
          if (cnt === 0) return null;
          const on = sec.key === curSec;
          return (
            <TouchableOpacity key={sec.key} onPress={() => setCurSec(sec.key)}
              style={{ paddingHorizontal:14, paddingVertical:7, borderRadius:99,
                backgroundColor: on ? T.accent+'25' : 'rgba(255,255,255,0.05)',
                borderWidth:1, borderColor: on ? T.accent+'60' : 'transparent' }}>
              <Text style={{ fontSize:11, fontWeight:'700', color: on ? T.accent : T.sub }}>
                {sec.label} {cnt}
              </Text>
            </TouchableOpacity>
          );
        ...
        {/* Counter */}
        {!done && stack.length > 0 && (
          <View style={{ paddingHorizontal:12, paddingVertical:7, borderRadius:99,
