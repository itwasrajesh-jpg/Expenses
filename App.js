...
// ─── SPAGHETTIFY ROW — row-level deletion animation ──────────────────────────
// The expense row itself stretches and gets sucked into a golden event horizon
function SpaghettifyRow({ children, onDelete, animLevel, T }) {
  const scaleX   = useRef(new Animated.Value(1)).current;
  const scaleY   = useRef(new Animated.Value(1)).current;
  const translateY = useRef(new Animated.Value(0)).current;
  const opacity  = useRef(new Animated.Value(1)).current;
  const rowH     = useRef(new Animated.Value(1)).current; // 1=full, 0=collapsed
  ...
  const handleDel = () => {
    ...
    // Show the golden event horizon ring
    setShowRing(true);

    if (animLevel === 'reduced') {
      // Lite: simple fast stretch and vanish
      Animated.parallel([
        Animated.timing(scaleX,    { toValue:0.02, duration:280, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        Animated.timing(scaleY,    { toValue:3,    duration:280, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        Animated.timing(opacity,   { toValue:0,    duration:240, delay:60, useNativeDriver:true }),
        Animated.timing(translateY,{ toValue:-30,  duration:280, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
      ]).start(() => {
        Animated.timing(rowH, { toValue:0, duration:180, easing:Easing.in(Easing.cubic), useNativeDriver:false }).start(onDelete);
      });
      return;
    }

    // Full: true spaghettification
    // Phase 1: row shakes — grabbed by gravity
    Animated.sequence([
      Animated.timing(scaleX, { toValue:1.04, duration:60,  useNativeDriver:true }),
      Animated.timing(scaleX, { toValue:0.96, duration:60,  useNativeDriver:true }),
      Animated.timing(scaleX, { toValue:1.02, duration:50,  useNativeDriver:true }),
    ]).start(() => {
      // Phase 2: spaghettification — narrow and stretch toward event horizon
      Animated.parallel([
        // scaleX collapses — matter gets pulled into a stream
        Animated.sequence([
          Animated.timing(scaleX, { toValue:0.6,  duration:120, easing:Easing.in(Easing.quad),  useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.2,  duration:150, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
          Animated.timing(scaleX, { toValue:0.02, duration:180, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        // scaleY elongates — spaghettification stretch
        Animated.sequence([
          Animated.timing(scaleY, { toValue:1.25, duration:140, easing:Easing.out(Easing.quad), useNativeDriver:true }),
          Animated.timing(scaleY, { toValue:2.8,  duration:200, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
          Animated.timing(scaleY, { toValue:4.5,  duration:160, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        ]),
        // translateY — flows toward event horizon (upward)
        Animated.timing(translateY, { toValue:-55, duration:480, easing:Easing.in(Easing.cubic), useNativeDriver:true }),
        // Brightness increases as matter heats up near singularity — fades last
        Animated.timing(opacity, { toValue:0, duration:320, delay:200, useNativeDriver:true }),
      ]).start(() => {
        // Phase 3: collapse row height so other rows fill the gap smoothly
        setShowRing(false);
        Animated.timing(rowH, { toValue:0, duration:200, easing:Easing.in(Easing.cubic), useNativeDriver:false }).start(onDelete);
      });
    });
  };

  // rowH interpolates from full height to 0 — animates layout space
  const rowHeightStyle = rowH.interpolate({ inputRange:[0,1], outputRange:[0,1] });

  return (
    <Animated.View style={{ transform:[{ scaleX }], overflow:'visible' }}>
      {/* Golden event horizon ring — appears at deletion point */}
      {showRing && (
        <Animated.View style={{
          position:'absolute', right:8, top:'50%',
          width:28, height:28, borderRadius:14,
          borderWidth:2, borderColor:'#FFD700',
          marginTop:-14, zIndex:99,
          shadowColor:'#FFD700', shadowOpacity:0.9, shadowRadius:8, elevation:10,
          opacity,
        }} />
      )}
      <Animated.View style={{
      ...
      {/* Height collapser — animates gap closing */}
      <Animated.View style={{ height: rowH.interpolate({ inputRange:[0,1], outputRange:[0,64], extrapolate:'clamp' }) }} />
    </Animated.View>
  );
}
