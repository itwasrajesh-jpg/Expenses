...
function TrendBar({ value, budget, maxH, color, isCur, animLevel }) {
  const fillAnim = useRef(new Animated.Value(0)).current;
  // pct of budget this bar represents (capped at 100%)
  ...
  useEffect(() => {
    ...
    Animated.timing(fillAnim, {
      toValue: targetH,
      duration: animLevel === 'reduced' ? 500 : 900,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: false,
    }).start();
  }, [targetH]);
  ...
  return (
    <View style={{ height: maxH, width: '100%', justifyContent: 'flex-end' }}>
      <Animated.View style={{
        ...
        width: '100%',
        borderRadius: 6,
        backgroundColor: isCur ? color : color + '66',
        overflow: 'hidden',
      }}>
        {/* Liquid wave overlay — only on full animation */}
        {animLevel !== 'reduced' && animLevel !== 'none' && (
          <LottieView
            source={require('./lottie-waves.json')}
            autoPlay loop
            style={{ position: 'absolute', bottom: -8, left: -10, width: 120, height: 36 }}
            speed={0.5}
          />
        )}
      </Animated.View>
