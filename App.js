...
// ─── MORE SCREEN ──────────────────────────────────────────────────────────────
function MoreScreen({ features, expenses, T, animLevel, collapsed, setCollapsed }) {
    ...
    <View style={{ flex: 1 }}>
      ...
      {showFeat && (
        <SlideScreen onClose={() => { setShowFeat(null); setSelBbl(-1); }} zIndex={60} T={T} animLevel={animLevel}>
          {close => {
            // Filter relevant expenses per feature
            const featExpenses = (() => {
              if (!expenses) return [];
              if (showFeat.id === 'income') return [];
              if (showFeat.id === 'investments') return expenses.filter(e => e.category === 'Other' || e.note?.toLowerCase().includes('invest'));
              if (showFeat.id === 'goals') return [];
              if (showFeat.id === 'emi') return expenses.filter(e => e.note?.toLowerCase().includes('emi') || e.note?.toLowerCase().includes('loan') || e.title?.toLowerCase().includes('emi'));
              if (showFeat.id === 'accounts') return expenses;
              if (showFeat.id === 'split') return expenses.filter(e => e.note?.toLowerCase().includes('split') || e.note?.toLowerCase().includes('shared'));
              if (showFeat.id === 'tax') return expenses;
              return expenses;
            })();
            const featTotal = featExpenses.reduce((a, e) => a + e.amount, 0);
            // Summary cards per feature
            const summary = (() => {
              if (showFeat.id === 'income')      return [{ label: 'Track your salary, freelance & other income', icon: '💰' }];
              if (showFeat.id === 'investments')  return [{ label: 'Monitor stocks, mutual funds & FDs', icon: '📈' }];
              if (showFeat.id === 'goals')        return [{ label: 'Set savings targets for trips, gadgets & more', icon: '🎯' }];
              if (showFeat.id === 'emi')          return expenses.filter(e => e.note?.toLowerCase().includes('emi') || e.title?.toLowerCase().includes('emi')).slice(0, 3);
              if (showFeat.id === 'accounts')     return [{ label: 'Cash', icon: '💵' }, { label: 'Bank Account', icon: '🏦' }, { label: 'UPI / Cards', icon: '💳' }];
              if (showFeat.id === 'split')        return featExpenses.slice(0, 4);
              if (showFeat.id === 'tax')          return [{ label: '80C deductions', icon: '📋' }, { label: 'HRA exemption', icon: '🏠' }, { label: 'Other deductions', icon: '📊' }];
              return [];
            })();
            return (
              <>
                <View style={[styles.detailHero, { backgroundColor: showFeat.color + '22' }]}>
                  <TouchableOpacity onPress={close} style={styles.backBtn}><Text style={{ color: T.text, fontSize: 18 }}>←</Text></TouchableOpacity>
                  <View style={{ width: 68, height: 68, borderRadius: 22, backgroundColor: showFeat.color + '30', alignItems: 'center', justifyContent: 'center', marginBottom: 12, borderWidth: 2, borderColor: showFeat.color + '55' }}>
                    <Text style={{ fontSize: 32 }}>{showFeat.icon}</Text>
                  </View>
                  ...
                  {featTotal > 0 && (
                    <Text style={{ color: showFeat.color, fontSize: 24, fontWeight: '800', marginTop: 10, letterSpacing: -0.5 }}>{fmt(featTotal)}</Text>
                  )}
                </View>
                <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
                  <View style={{ padding: 16 }}>
                    {/* Show relevant expenses if any */}
                    {featExpenses.length > 0 && (showFeat.id === 'emi' || showFeat.id === 'split' || showFeat.id === 'accounts') ? (
                      <>
                        <Text style={{ color: T.text, fontSize: 13, fontWeight: '600', marginBottom: 10 }}>
                          {showFeat.id === 'emi' ? 'EMI / Loan Expenses' : showFeat.id === 'split' ? 'Shared Expenses' : 'All Expenses by Account'}
                        </Text>
                        {featExpenses.slice(0, 8).map((e, i) => {
                          const cat = CM[e.category] ?? CM['Other'];
                          return (
                            <View key={e.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, backgroundColor: T.surface, borderRadius: 14, marginBottom: 7 }}>
                              <View style={{ width: 38, height: 38, borderRadius: 11, backgroundColor: cat.color + '28', alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ fontSize: 18 }}>{cat.icon}</Text>
                              </View>
                              <View style={{ flex: 1 }}>
                                <Text style={{ color: T.text, fontSize: 13, fontWeight: '600' }}>{e.title}</Text>
                                <Text style={{ color: T.sub, fontSize: 11 }}>{dispDate(e.date)} · {e.payment || e.category}</Text>
                              </View>
                              <Text style={{ color: '#FF8A80', fontSize: 13, fontWeight: '700' }}>−{fmt(e.amount)}</Text>
                            </View>
                          );
                        })}
                        {featExpenses.length === 0 && (
                          <Text style={{ color: T.sub, fontSize: 13, textAlign: 'center', marginTop: 8 }}>No matching expenses yet</Text>
                        )}
                      </>
                    ) : (
                      <>
                        <Text style={{ color: T.text, fontSize: 13, fontWeight: '600', marginBottom: 12 }}>What you get</Text>
                        {summary.map((item, i) => (
                          <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, backgroundColor: T.surface, borderRadius: 14, marginBottom: 8 }}>
                            <Text style={{ fontSize: 22 }}>{item.icon}</Text>
                            <Text style={{ color: T.text, fontSize: 13, flex: 1 }}>{item.label || item.title || item.name}</Text>
                          </View>
                        ))}
                      </>
                    )}
                    {/* Tax summary special case */}
                    {showFeat.id === 'tax' && expenses && expenses.length > 0 && (
                      <View style={{ marginTop: 8 }}>
                        <Text style={{ color: T.text, fontSize: 13, fontWeight: '600', marginBottom: 10 }}>This Year's Spending</Text>
                        {Object.entries(
                          expenses.reduce((acc, e) => { acc[e.category] = (acc[e.category] || 0) + e.amount; return acc; }, {})
                        ).sort((a,b) => b[1]-a[1]).slice(0, 5).map(([cat, amt]) => {
                          const c = CM[cat] ?? CM['Other'];
                          return (
                            <View key={cat} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                              <Text style={{ fontSize: 18 }}>{c.icon}</Text>
                              <Text style={{ color: T.text, fontSize: 13, flex: 1 }}>{cat}</Text>
                              <Text style={{ color: T.accent, fontSize: 13, fontWeight: '700' }}>{fmt(amt)}</Text>
                            </View>
                          );
                        })}
                      </View>
...
export default function App() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {tab === 'more'     && <MoreScreen     features={state.features} expenses={state.expenses} T={T} animLevel={aL} collapsed={moreCollapsed} setCollapsed={setMoreCollapsed} />}
